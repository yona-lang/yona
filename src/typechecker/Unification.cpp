...
Unifier::Unifier(TypeArena &arena, UnionFind &uf, DiagnosticEngine &diag) : arena_(arena), uf_(uf), diag_(diag) {}
...
bool Unifier::unify(MonoTypePtr a, MonoTypePtr b, const SourceLocation &loc, const std::string &context) {
  return unify_inner(resolve(a), resolve(b), loc, context);
}

bool Unifier::unify_inner(MonoTypePtr a, MonoTypePtr b, const SourceLocation &loc, const std::string &context) {
  if (a == b)
    return true;
  if (!a || !b)
    return false;

  // ERow is retained only as a bridge for the unconverted TypeChecker. It
  // is not unified as a value type: translate both sides to solver-owned
  // effects first, then mirror the successful result into UnionFind below.
  if (a->tag == MonoType::ERow || b->tag == MonoType::ERow) {
    const auto a_effect = arena_.legacy_effect_ref(a);
    const auto b_effect = arena_.legacy_effect_ref(b);
    if (a_effect && b_effect && !unify_effects(*a_effect, *b_effect, loc, context))
      return false;
  }
  ...
  if (a->tag == MonoType::Var) {
    ...
    if (occurs_in(a->var_id, b)) {
      diag_.error(loc, ErrorCode::E0101,
                  "infinite type: cannot construct " + pretty_print(a) + " ~ " + pretty_print(b) + (context.empty() ? "" : " " + context));
      return false;
    }
  ...
  if (b->tag == MonoType::Var) {
    if (occurs_in(b->var_id, a)) {
      diag_.error(loc, ErrorCode::E0101,
                  "infinite type: cannot construct " + pretty_print(b) + " ~ " + pretty_print(a) + (context.empty() ? "" : " " + context));
      return false;
    }
  ...
  if (a->tag != b->tag) {
    diag_.error(loc, ErrorCode::E0100,
                "type mismatch: expected " + pretty_print(a) + " but found " + pretty_print(b) + (context.empty() ? "" : " " + context));
    return false;
  }
  ...
  case MonoType::Arrow:
    if (!unify(a->param_type, b->param_type, loc, context) || !unify(a->return_type, b->return_type, loc, context) ||
        !unify_effects(a->arrow_effect, b->arrow_effect, loc, context))
      return false;
    // The mirror can represent only one open tail. Let it synchronize two
    // legacy arrows, but never let an absent/lossy projection veto solver
    // equality during the staged migration.
    if (a->has_legacy_effect_view && b->has_legacy_effect_view)
      return sync_legacy_effect_rows(a->arrow_effects, a->effect_rest,
                                     b->arrow_effects, b->effect_rest, loc,
                                     context);
    return true;

  case MonoType::ERow:
    return sync_legacy_effect_rows(a->arrow_effects, a->effect_rest, b->arrow_effects, b->effect_rest, loc, context);
  ...
  case MonoType::App: {
    ...
    auto is_collection = [](MonoTypePtr t) { return t->type_name == "Seq" || t->type_name == "Set" || t->type_name == "Dict"; };
    if (a->type_name == "ADT" && b->type_name != "ADT") {
      if (is_collection(b)) {
        diag_.error(loc, ErrorCode::E0100, "type mismatch: " + pretty_print(a) + " vs " + pretty_print(b) + (context.empty() ? "" : " " + context));
        return false;
      }
  ...
  case MonoType::MTuple:
    if (a->elements.size() != b->elements.size()) {
      diag_.error(loc, ErrorCode::E0102,
                  "tuple size mismatch: " + std::to_string(a->elements.size()) + " vs " + std::to_string(b->elements.size()) +
  ...
  case MonoType::MRecord: {
    ...
    if (!a_extras.empty() && b->row_rest) {
      ...
      diag_.error(loc, ErrorCode::E0100, "record has extra field(s) not expected" + (context.empty() ? "" : " " + context));
      return false;
    }
    ...
    if (!b_extras.empty() && a->row_rest) {
      ...
      diag_.error(loc, ErrorCode::E0100, "record missing field(s)" + (context.empty() ? "" : " " + context));
      return false;
    }
...
void Unifier::flatten_effect_row(MonoTypePtr rest, std::vector<LatentEffect> &labs, MonoTypePtr &out_rest) {
  rest = resolve(rest);
  int guard = 0;
...
bool Unifier::unify_effects(EffectRef left, EffectRef right, const SourceLocation &loc, const std::string &context) {
  const auto result = arena_.effect_solver().equate(left, right);
  if (result != EffectConstraintResult::Conflict)
    return true;
  diag_.error(loc, ErrorCode::E0100, "incompatible effect rows" + (context.empty() ? "" : " " + context));
  return false;
}

bool Unifier::sync_legacy_effect_rows(const std::vector<LatentEffect> &a_labs_in, MonoTypePtr a_rest, const std::vector<LatentEffect> &b_labs_in,
  ...
  for (auto &[k, e] : a_map)
    if (b_map.find(k) == b_map.end())
      a_extras.push_back(e);
  for (auto &[k, e] : b_map)
    if (a_map.find(k) == a_map.end())
      b_extras.push_back(e);

  auto same_rest_var = [&](MonoTypePtr x, MonoTypePtr y) {
    auto *xr = resolve(x);
    auto *yr = resolve(y);
    return xr && yr && xr->tag == MonoType::Var && yr->tag == MonoType::Var && xr->var_id == yr->var_id;
  };
...
bool Unifier::occurs_in(TypeId var_id, MonoTypePtr type) {
  ...
  if (type->tag == MonoType::Arrow)
    return occurs_in(var_id, type->param_type) || occurs_in(var_id, type->return_type) || occurs_in(var_id, type->effect_rest);
...
std::string pretty_print_rec(MonoTypePtr type, PrintCtx &ctx) {
  ...
  case MonoType::Arrow: {
    if (type->effect_solver && type->arrow_effect.valid()) {
      const auto row = pretty_print_effect(type->effect_solver->summarize(type->arrow_effect), ctx);
      return "(" + pretty_print_rec(type->param_type, ctx) + " -> " + row + pretty_print_rec(type->return_type, ctx) + ")";
    }
    std::vector<std::string> labels;
    for (auto &e : type->arrow_effects)
      labels.push_back(e.op_key);
