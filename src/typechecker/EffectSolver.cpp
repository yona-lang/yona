...
void merge_labels(std::vector<std::string> &target, const std::vector<std::string> &source) {
...
void normalize_summary(EffectNormalForm &summary) {
  ...
  std::sort(summary.tails.begin(), summary.tails.end(), [](const EffectProjection &left, const EffectProjection &right) {
    return std::tie(left.variable, left.opaque, left.excluded_labels) < std::tie(right.variable, right.opaque, right.excluded_labels);
  });
  summary.tails.erase(std::unique(summary.tails.begin(), summary.tails.end()), summary.tails.end());
}

void merge_summary(EffectNormalForm &target, const EffectNormalForm &source) {
  target.known_labels.insert(target.known_labels.end(), source.known_labels.begin(), source.known_labels.end());
  target.tails.insert(target.tails.end(), source.tails.begin(), source.tails.end());
...
struct EffectSolver::Impl {
  ...
  std::uint64_t owner = next_effect_solver_owner.fetch_add(1, std::memory_order_relaxed);
  std::vector<Node> nodes{{}};
  ...
  std::map<std::pair<std::uint32_t, std::vector<std::string>>, EffectRef> mask_cache;
  ...
  [[nodiscard]] EffectRef empty_ref() const noexcept { return EffectRef{owner, 0}; }

  void require(EffectRef effect) const {
    if (!effect.valid() || effect.owner != owner || effect.index >= nodes.size())
      throw std::invalid_argument("effect reference does not belong to this solver");
  }
  ...
  EffectRef resolve_binding(EffectRef effect) const {
    ...
    while (nodes[effect.index].kind == EffectNodeKind::FlexibleMeta && nodes[effect.index].binding) {
  ...
  bool occurs(EffectVarId variable, EffectRef effect, std::unordered_set<std::uint32_t> &seen) const {
    require(effect);
    if (!seen.insert(effect.index).second)
      return false;
  ...
  bool is_symbolic(EffectRef effect, std::unordered_set<std::uint32_t> &seen) const {
    ...
    case EffectNodeKind::Mask:
      return std::any_of(node.inputs.begin(), node.inputs.end(), [&](EffectRef input) { return is_symbolic(input, seen); });
  ...
  EffectNormalForm summarize_impl(EffectRef effect, std::unordered_set<std::uint32_t> &active, bool follow_equalities) const {
    effect = resolve_binding(effect);
    ...
    case EffectNodeKind::Mask: {
      result = node.inputs.empty() ? EffectNormalForm{} : summarize_impl(node.inputs.front(), active, follow_equalities);
      std::erase_if(result.known_labels,
                    [&](const std::string &label) { return std::binary_search(node.excluded_labels.begin(), node.excluded_labels.end(), label); });
    ...
    if (follow_equalities && is_symbolic(effect)) {
      for (const auto &[constraint_left, constraint_right] : equality_constraints) {
  ...
  bool contains_derived(EffectRef effect, std::unordered_set<std::uint32_t> &seen) const {
    require(effect);
    if (!seen.insert(effect.index).second)
      return false;
    const auto &node = nodes[effect.index];
  ...
  static BooleanAtom effect_variable_atom(EffectVarId variable) { return static_cast<BooleanAtom>(variable) << 1; }
  static BooleanAtom derived_atom(std::uint32_t index) { return (static_cast<BooleanAtom>(index) << 1) | 1; }

  static void merge_boolean_expression(BooleanExpression &target, const BooleanExpression &source) {
    target.constant = target.constant || source.constant;
    target.atoms.insert(target.atoms.end(), source.atoms.begin(), source.atoms.end());
    std::sort(target.atoms.begin(), target.atoms.end());
    target.atoms.erase(std::unique(target.atoms.begin(), target.atoms.end()), target.atoms.end());
  }

  BooleanExpression boolean_expression(EffectRef effect, const std::string &label) const {
    effect = resolve_binding(effect);
    const auto &node = nodes[effect.index];
    ...
    case EffectNodeKind::Empty:
      break;
    case EffectNodeKind::Labels:
      result.constant = std::binary_search(node.labels.begin(), node.labels.end(), label);
      break;
    case EffectNodeKind::FlexibleMeta:
    case EffectNodeKind::RigidOpaque:
    ...
    case EffectNodeKind::Join:
      ...
      break;
    case EffectNodeKind::Mask:
      if (!std::binary_search(node.excluded_labels.begin(), node.excluded_labels.end(), label) && !node.inputs.empty())
  ...
  bool equality_constraints_satisfiable(std::vector<DerivedFact> *forced_facts = nullptr) const {
    ...
    for (const auto &node : nodes) {
      ...
      labels.insert(labels.end(), node.excluded_labels.begin(), node.excluded_labels.end());
    }
    ...
    for (const auto &label : labels) {
      ...
      auto add_rule = [&](std::vector<BooleanAtom> body, std::optional<BooleanAtom> head) {
      ...
      auto add_implication = [&](const BooleanExpression &antecedent, const BooleanExpression &consequent) {
      ...
      for (std::uint32_t index = 0; index < nodes.size(); ++index) {
        ...
        definition.constant = std::binary_search(node.labels.begin(), node.labels.end(), label);
        for (const auto input : node.inputs)
          merge_boolean_expression(definition, boolean_expression(input, label));
        add_implication(definition, BooleanExpression{false, {derived_atom(index)}});
      }
      ...
      auto horn_satisfiable = [&](const std::vector<BooleanAtom> &assumed_false) {
        std::set<BooleanAtom> complemented_true(assumed_false.begin(), assumed_false.end());
        bool changed = true;
        while (changed) {
          changed = false;
          for (const auto &rule : rules) {
            if (!std::all_of(rule.body.begin(), rule.body.end(), [&](BooleanAtom atom) { return complemented_true.contains(atom); }))
              continue;
            if (!rule.head)
              return false;
            changed = complemented_true.insert(*rule.head).second || changed;
          }
        }
        return true;
      };
  ...
  EffectConstraintResult local_constraint_status(const std::pair<EffectRef, EffectRef> &constraint) const {
    const auto left = summarize_raw(constraint.first);
    const auto right = summarize_raw(constraint.second);
    if (left == right && !left.is_open() && !contains_derived(constraint.first) && !contains_derived(constraint.second))
  ...
  EffectConstraintResult settle_constraints() {
    ...
    for (const auto &constraint : equality_constraints)
      if (local_constraint_status(constraint) == EffectConstraintResult::Deferred)
        return EffectConstraintResult::Deferred;
    return EffectConstraintResult::Solved;
  }
  ...
  EffectConstraintResult establish_equality(EffectRef left, EffectRef right) {
    ...
    if (std::find(equality_constraints.begin(), equality_constraints.end(), constraint) != equality_constraints.end()) {
    ...
    auto include_in_derived = [&](EffectRef source, EffectRef sink) {
      ...
      if (node.kind != EffectNodeKind::DerivedLeast)
        return;
      if (std::find(node.inputs.begin(), node.inputs.end(), source) == node.inputs.end()) {
        node.inputs.push_back(source);
        std::sort(node.inputs.begin(), node.inputs.end(), [](EffectRef first, EffectRef second) { return first.index < second.index; });
    ...
    if (settle_constraints() == EffectConstraintResult::Conflict) {
      ...
      for (const auto &[source, sink] : added_inputs) {
        ...
        inputs.erase(std::remove(inputs.begin(), inputs.end(), source), inputs.end());
      }
      return EffectConstraintResult::Conflict;
    }
    return local_constraint_status(constraint);
  ...
  BindResult bind_flexible(EffectRef variable, EffectRef value) {
    ...
    return status == EffectConstraintResult::Deferred ? BindResult::BoundDeferred : BindResult::Bound;
  }
};

EffectGraphTemplate::EffectGraphTemplate() = default;
EffectGraphTemplate::EffectGraphTemplate(std::shared_ptr<const Data> data) : data_(std::move(data)) {}
EffectGraphTemplate::EffectGraphTemplate(const EffectGraphTemplate &) = default;
EffectGraphTemplate::EffectGraphTemplate(EffectGraphTemplate &&) noexcept = default;
EffectGraphTemplate &EffectGraphTemplate::operator=(const EffectGraphTemplate &) = default;
EffectGraphTemplate &EffectGraphTemplate::operator=(EffectGraphTemplate &&) noexcept = default;
EffectGraphTemplate::~EffectGraphTemplate() = default;
...
EffectRef EffectSolver::labels(std::vector<std::string> labels) {
  ...
  if (labels.empty())
    return empty();
  if (const auto found = impl_->label_cache.find(labels); found != impl_->label_cache.end())
    return found->second;
  Impl::Node node;
...
EffectRef EffectSolver::labels(std::initializer_list<std::string> labels) { return this->labels(std::vector<std::string>(labels)); }
...
EffectRef EffectSolver::join(std::vector<EffectRef> effects) {
  ...
  for (const auto effect : effects)
    append(effect);
  std::sort(flattened.begin(), flattened.end(), [](EffectRef left, EffectRef right) { return left.index < right.index; });
  ...
  if (const auto found = impl_->join_cache.find(key); found != impl_->join_cache.end())
    return found->second;
...
EffectRef EffectSolver::join(std::initializer_list<EffectRef> effects) { return join(std::vector<EffectRef>(effects)); }

EffectRef EffectSolver::mask(EffectRef source, std::vector<std::string> excluded_labels) {
  ...
  if (const auto found = impl_->mask_cache.find(key); found != impl_->mask_cache.end())
    return found->second;
...
EffectRef EffectSolver::mask(EffectRef source, std::initializer_list<std::string> excluded_labels) {
  return mask(source, std::vector<std::string>(excluded_labels));
}

EffectConstraintResult EffectSolver::add_label(EffectRef derived, std::string label) {
  derived = impl_->resolve_binding(derived);
  if (impl_->nodes[derived.index].kind != EffectNodeKind::DerivedLeast)
    throw std::invalid_argument("effect labels can only be added to a derived cell");
...
EffectConstraintResult EffectSolver::include(EffectRef source, EffectRef derived) {
  ...
  if (found == inputs.end()) {
    ...
    std::sort(inputs.begin(), inputs.end(), [](EffectRef left, EffectRef right) { return left.index < right.index; });
    const auto status = impl_->settle_constraints();
    if (status == EffectConstraintResult::Conflict)
      inputs.erase(std::remove(inputs.begin(), inputs.end(), source), inputs.end());
    return status;
...
std::optional<EffectVarId> EffectSolver::variable_id(EffectRef effect) const {
  ...
  const auto &node = impl_->nodes[effect.index];
  if (node.kind == EffectNodeKind::FlexibleMeta || node.kind == EffectNodeKind::RigidOpaque)
...
EffectGraphTemplate EffectSolver::freeze(std::vector<EffectRef> roots) const {
  ...
  for (const auto &[left, right] : impl_->equality_constraints)
    if (reachable.contains(left.index) && reachable.contains(right.index))
      data->equalities.emplace_back(local_index.at(left.index), local_index.at(right.index));
  return EffectGraphTemplate(std::move(data));
}

EffectInstantiation EffectSolver::instantiate(const EffectGraphTemplate &graph) {
  if (!graph.data_)
  ...
  for (const auto &[left, right] : data.equalities)
    impl_->equality_constraints.emplace_back(mapping.at(left), mapping.at(right));
