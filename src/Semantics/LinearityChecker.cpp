/// LinearityChecker â€” tracks Linear ADT values through the program.
///
...
std::string apply_callee_name(AstNode *expr) {
  if (!expr || expr->get_type() != AST_APPLY_EXPR)
    return {};
...
void LinearEnv::create(const std::string &name, const SourceRange &loc) {
  vars[name] = LinearStatus::Live;
  created_at[name] = loc;
}

bool LinearEnv::consume(const std::string &name, const SourceRange &loc) {
  auto it = vars.find(name);
  if (it == vars.end())
    return true; // not tracked
  if (it->second == LinearStatus::Consumed)
...
bool LinearityChecker::expr_produces_linear(AstNode *expr) {
  if (auto *t = type_of_expr(expr)) {
    if (is_linear_type(t))
      return true;
    // Conclusive non-Linear type: not a producer (including unresolved-as-Int,
    // etc.)
    if (t->tag != MonoType::Var)
      return false;
  }
  return is_linear_constructor(apply_callee_name(expr));
}

void LinearityChecker::track_linear_pattern(PatternNode *pat,
                                            const MonoType *ty, LinearEnv &env,
...
void LinearityChecker::warn_unconsumed(const LinearEnv &env) {
  for (auto &name : env.live_vars()) {
    ...
    SourceRange loc =
        (it != env.created_at.end()) ? it->second : SourceRange::unknown();
    warn_leak(name, loc);
  }
}

void LinearityChecker::warn_leak(const std::string &name,
                                 const SourceRange &loc) {
  diag_.warning(loc, ErrorCode::E0602,
                "linear value '" + name +
                    "' not consumed â€” possible resource leak; "
                    "use `case " +
                    name + " of Linear fd -> close fd end` to release",
    ...
    if (auto *va = dynamic_cast<ValueAlias *>(alias)) {
      ...
      if (va->expr->get_type() == AST_IDENTIFIER_EXPR) {
        ...
        if (env.is_live(src)) {
          ...
          env.consume(src, va->Range);
          env.create(name, va->Range);
        } else if (env.is_consumed(src)) {
          diag_.error(va->Range, ErrorCode::E0600,
...
void LinearityChecker::check_case(CaseExpr *node, LinearEnv &env) {
  ...
  std::vector<LinearEnv> branch_envs;
  branch_envs.reserve(node->clauses.size());
  ...
  for (auto *clause : node->clauses) {
    ...
    if (!scrut_name.empty() && env.is_live(scrut_name)) {
      ...
      // Constructor pattern `Linear fd` â€” this is the consumption point
      if (pat->get_type() == AST_CONSTRUCTOR_PATTERN) {
        ...
        if (is_linear_constructor(cp->constructor_name)) {
          branch_env.consume(scrut_name, clause->Range);
        }
      }
    }
    ...
    branch_envs.push_back(std::move(branch_env));
  }

  // Every live value has to leave all case branches in the same state. This
  // includes values consumed by nested cases in a clause body, not only the
  // case's own scrutinee. Propagate unanimous consumption to the outer
  // environment; otherwise report the branch disagreement.
  for (const auto &[name, status] : env.vars) {
    if (status != LinearStatus::Live || branch_envs.empty())
      continue;
    const size_t consumed =
        std::count_if(branch_envs.begin(), branch_envs.end(),
                      [&](const LinearEnv &branch_env) {
                        return branch_env.is_consumed(name);
                      });
    if (consumed == branch_envs.size()) {
      env.consume(name, node->Range);
    } else if (consumed != 0) {
      diag_.error(node->Range, ErrorCode::E0601,
                  "linear value '" + name +
  ...
  for (auto &arg_variant : node->args) {
    ...
    if (arg_node && arg_node->get_type() == AST_IDENTIFIER_EXPR) {
      ...
      if (env.is_consumed(arg_name)) {
        ...
        std::string consumed_loc =
            (it != env.consumed_at.end())
                ? " (consumed at " + diag_.formatRange(it->second) + ")"
                : "";
        diag_.error(node->Range, ErrorCode::E0600,
                    "linear value '" + arg_name + "' was already consumed" +
                        consumed_loc);
        error_count_++;
      }
    }
...
void LinearityChecker::check_with(WithExpr *node, LinearEnv &env) {
  ...
  for (auto &live : body_env.live_vars()) {
    if (live == name)
      continue;
    if (env.is_tracked(live))
      continue; // outer; still live in outer too
    auto it = body_env.created_at.find(live);
    SourceRange loc =
        (it != body_env.created_at.end()) ? it->second : node->Range;
    warn_leak(live, loc);
  }
}

void LinearityChecker::check_function(FunctionExpr *node,
                                      LinearEnv & /*outer*/) {
  // Function bodies are a separate linear scope and cannot capture linear
  // values.
  LinearEnv fn_env;
  ...
  if (fn_ty) {
    ...
    while (cur && cur->tag == MonoType::Arrow &&
           param_tys.size() < node->patterns.size()) {
  ...
  for (size_t i = 0; i < node->patterns.size(); ++i) {
    ...
    track_linear_pattern(node->patterns[i], pty, fn_env, node->Range);
  }
