...
argument_node(const std::variant<ast::ExprNode *, ast::ValueExpr *> &argument) {
  return std::visit([](auto *node) -> ast::AstNode * { return node; },
                    argument);
}
...
void shadow_pattern(ast::PatternNode *pattern, ShadowedNames &shadowed) {
  ...
  if (auto *value = dynamic_cast<ast::PatternValue *>(pattern)) {
    if (auto *identifier = std::get_if<ast::IdentifierExpr *>(&value->expr);
...
void shadow_generator_binding(const ast::IdentifierOrUnderscore &binding,
...
public:
  DependencyCollector(ast::ModuleDecl *module,
                      ModuleExportResolver resolve_exports)
      : module_(module), resolve_exports_(std::move(resolve_exports)) {
...
private:
  ...
  void record(const std::string &name, const ShadowedNames &shadowed,
              size_t caller) {
  ...
  void walk_generator(ast::CollectionExtractorExpr *extractor,
                      ast::AstNode *reducer, ast::AstNode *step,
                      const ShadowedNames &shadowed, size_t caller) {
  ...
  void walk(ast::AstNode *node, const ShadowedNames &shadowed, size_t caller) {
    ...
    if (auto *identifier = dynamic_cast<ast::IdentifierExpr *>(node)) {
      ...
      if (auto *named = dynamic_cast<ast::NameCall *>(apply->call)) {
        if (named->name)
          record(named->name->value, shadowed, caller);
                     ...
                     dynamic_cast<ast::ExprCall *>(apply->call)) {
        walk(expression->expr, shadowed, caller);
      } else if (auto *module_call =
                     dynamic_cast<ast::ModuleCall *>(apply->call)) {
      ...
      for (auto *clause : import->clauses) {
        if (auto *functions = dynamic_cast<ast::FunctionsImport *>(clause)) {
          for (auto *alias : functions->aliases)
            if (alias && alias->name)
              body_shadowed.insert(alias->alias ? alias->alias->value
                                                : alias->name->value);
        } else if (auto *module = dynamic_cast<ast::ModuleImport *>(clause);
                   module && module->fqn && resolve_exports_) {
      ...
      walk_generator(sequence->collectionExtractor, sequence->reducerExpr,
                     sequence->stepExpression, shadowed, caller);
    } else if (auto *set = dynamic_cast<ast::SetGeneratorExpr *>(node)) {
      walk_generator(set->collectionExtractor, set->reducerExpr,
                     set->stepExpression, shadowed, caller);
    } else if (auto *dict = dynamic_cast<ast::DictGeneratorExpr *>(node)) {
      walk_generator(dict->collectionExtractor, dict->reducerExpr,
                     dict->stepExpression, shadowed, caller);
    ...
    std::function<void(size_t)> visit = [&](size_t node) {
      ...
      std::sort(component.functions.begin(), component.functions.end(),
                [&](const auto *left, const auto *right) {
                  return std::find(module_->functions.begin(),
                                   module_->functions.end(), left) <
                         std::find(module_->functions.begin(),
                                   module_->functions.end(), right);
                });
