/*
 * macOS file I/O — submit-and-return via kqueue (see kqueue_macos.c).
 *
 * Each async function submits and returns a completion ID immediately.
 * Completion is handled by yona_rt_io_await() which does type-specific
 * post-processing (close fd, null-terminate buffer, etc). Same ABI as
 * the Linux io_uring path.
 */

#ifndef __APPLE__
#error "file_macos.c is for macOS builds only"
#endif
...
int64_t yona_rt_io_await(int64_t uring_id) {
    ...
    int32_t res = kq_await((uint64_t)uring_id);
...
int64_t yona_platform_read_file_submit(const char* path) {
    ...
    uint64_t id = kq_submit_read(fd, buf, size, 0);
    if (id == 0) {
        close(fd);
        free(ctx);
        return 0;
...
int64_t yona_platform_write_file_submit(const char* path, const char* content) {
    ...
    uint64_t id = kq_submit_write(fd, pinned, len, 0);
...
int64_t yona_platform_read_file_bytes_submit(const char* path) {
    ...
    uint64_t id = kq_submit_read(fd, (uint8_t*)(buf + 1), size, 0);
...
int64_t yona_platform_read_fd_bytes_submit(int fd, int64_t count, int64_t offset) {
    ...
    uint64_t id = kq_submit_read(fd, (uint8_t*)(buf + 1), (size_t)count, (off_t)offset);
    if (id == 0) {
...
int64_t yona_platform_write_fd_bytes_submit(int fd, void* bytes, int64_t offset) {
    ...
    uint64_t id = kq_submit_write(fd, data, (size_t)len, (off_t)offset);
    if (id == 0) {
...
static int64_t line_iter_next(int64_t* closure_env) {
    /* closure_env layout: [fn_ptr, ret_tag, arity, num_caps, heap_mask, borrow_mask, cap0]
...
int64_t yona_rt_file_line_iterator(const char* path) {
     ...
     * Closure layout: [fn_ptr, ret_tag, arity, num_caps, heap_mask, borrow_mask, cap0, ...] */
...
static int64_t chunk_iter_next(int64_t* env) {
    ...
    uint64_t id = kq_submit_read(st->fd, (uint8_t*)(buf + 1),
                                (size_t)st->chunk_size, (off_t)st->position);
    ssize_t n;
    if (id == 0) {
        n = pread(st->fd, (uint8_t*)(buf + 1), (size_t)st->chunk_size,
                  (off_t)st->position);
    } else {
        n = (ssize_t)kq_await(id);
    }
 ...
 * so a single kqueue write covers it; the buffer is free'd by the
 * completer via IO_OP_WRITE_FD_STR. Returns a completion ID. */
static int64_t write_fd_strs_submit_impl(int fd, const char* s1, const char* s2) {
    ...
    /* stdout/stderr: write on the submit path so discarded `println` in a
    ...
    uint64_t id = kq_submit_write(fd, buf, total, (off_t)-1);
    if (id == 0) {
        ssize_t n = write(fd, buf, total);
        free(buf);
        free(ctx);
