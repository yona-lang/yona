 ...
 * completion ID immediately. Completion via yona_rt_io_await().
 */
...
int64_t yona_Std_Net__tcpConnect(const char* host, int64_t port) {
    ...
    uint64_t id = kq_submit_connect(fd, addr_copy, addr_len);
...
int64_t yona_Std_Net__tcpAccept(int64_t listener_fd) {
    ...
    *addr_len_ptr = sizeof(struct sockaddr_in);
    ...
    uint64_t id = kq_submit_accept((int)listener_fd, client_addr, addr_len_ptr);
...
int64_t yona_Std_Net__send(int64_t fd, const char* data) {
    ...
    uint64_t id = kq_submit_send((int)fd, pinned, len);
...
int64_t yona_Std_Net__recv(int64_t fd, int64_t max_bytes) {
    ...
    uint64_t id = kq_submit_recv((int)fd, buf, (size_t)max_bytes);
...
int64_t yona_Std_Net__sendBytes(int64_t fd, void* bytes) {
    ...
    uint64_t id = kq_submit_send((int)fd, data, (size_t)len);
...
int64_t yona_Std_Net__recvBytes(int64_t fd, int64_t max_bytes) {
    ...
    uint64_t id = kq_submit_recv((int)fd, (uint8_t*)(buf + 1), (size_t)max_bytes);
...
int64_t yona_Std_Http__httpGet(const char* url) {
    ...
    freeaddrinfo(ai);
    {
        uint64_t id = kq_submit_connect(fd, &addr_buf, addr_len);
        if (id == 0 || kq_await(id) < 0) { close(fd); return 0; }
    }

    const char* req = yona_Std_Http__buildRequest("GET", host, path, NULL);
    {
        uint64_t id = kq_submit_send(fd, req, strlen(req));
        if (id != 0) kq_await(id);
    }
    ...
    for (;;) {
        uint64_t id = kq_submit_recv(fd, buf + total, buf_size - total - 1);
        int32_t n = (id == 0) ? 0 : kq_await(id);
        if (n <= 0) break;
        total += (size_t)n;
