...
Server::Server(std::vector<std::string> extra_paths) : extra_paths_(std::move(extra_paths)) {}
...
Json Server::initialize(const Json& params) {
    ...
    caps["signatureHelpProvider"] = Json::Object{{"triggerCharacters", Json::Array{Json(" ")}}};
    caps["inlayHintProvider"] = true;
    caps["callHierarchyProvider"] = true;
    caps["codeActionProvider"] = true;
    Json legend;
    legend["tokenTypes"] = Json::Array{Json("function"), Json("type"), Json("namespace"), Json("parameter"),
                                       Json("property"), Json("variable"), Json("keyword"), Json("number"),
                                       Json("variable")};
    legend["tokenModifiers"] = Json::Array{Json("declaration")};
    caps["semanticTokensProvider"] = Json::Object{{"legend", legend}, {"full", Json(true)}};
    Json result;
    result["capabilities"] = caps;
    result["serverInfo"] = Json::Object{{"name", Json("yls")}, {"version", Json(YONA_VERSION_STRING)}};
    return result;
}
...
Json Server::hover(const Json& params) {
    ...
    if (!h)
        return nullptr;
    Json result;
    result["contents"] = Json::Object{{"kind", Json("markdown")}, {"value", Json("```yona\n" + h->contents + "\n```")}};
...
Json Server::document_symbol(const Json& params) {
    ...
    for (const auto& s : doc(uri).document_symbols()) {
        Json it;
        it["name"] = s.name;
        it["kind"] = s.kind == "function" ? 12 : s.kind == "type" ? 5 : s.kind == "namespace" ? 3 : 13;
...
Json Server::rename(const Json& params) {
    auto uri = params.get("textDocument").get("uri").as_string();
    Json edits;
    auto old = doc(uri).rename(pos_of(params), params.get("newName").as_string(), edits);
    if (!old)
        return nullptr;
    Json we;
...
Json Server::inlay_hint(const Json& params) {
    ...
    r.start.line = static_cast<std::size_t>(rng.get("start").get("line").as_int());
    r.start.character = static_cast<std::size_t>(rng.get("start").get("character").as_int());
    r.end.line = static_cast<std::size_t>(rng.get("end").get("line").as_int());
    r.end.character = static_cast<std::size_t>(rng.get("end").get("character").as_int());
