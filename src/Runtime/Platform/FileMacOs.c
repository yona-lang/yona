 ...
 * Completion is handled by YonaRuntimeIoAwait() which does type-specific
...
int64_t YonaRuntimeIoAwait(int64_t IoId) {
  ...
  int32_t Res = YonaRuntimeKqueueAwait((uint64_t)IoId);
...
int64_t YonaRuntimePlatformSubmitFileWrite(const char *Path,
  ...
  uint64_t Id = YonaRuntimeKqueueSubmitWrite(Fd, Pinned, Len, 0);
  if (Id == 0) {
    close(Fd);
 ...
 * completer via YonaIoOperationWriteFileDescriptorString. Returns a completion
 * ID. */
