 ...
 *
 * Process module provides synchronous and asynchronous APIs:
 *   -
 * exec/execStatus: synchronous implementation scheduled as AFN work
 *   -
...
char *YonaStdProcessExecutablePath(void) {
  ...
  Buffer[Count] = '\0';
  return copyRuntimeString(Buffer);
}
...
char *YonaStdProcessTempFile(const char *Prefix, const char *Suffix) {
  ...
  size_t RequiredSize =
      strlen(Directory) + 1 + strlen(Prefix) + 6 + strlen(Suffix) + 1;
...
int64_t YonaRuntimePlatformCacheLineSize(void) {
  ...
  return Value > 0 ? (int64_t)Value : 64;
}
