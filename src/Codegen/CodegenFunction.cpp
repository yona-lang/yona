...
// Closure layout constants match YONA_CLOSURE_HDR_SIZE in the runtime core.
...
// Perceus phase 3: match YONA_MAX_FRAME_DROPS in
...
std::string Codegen::source_type_descriptor(const types::Type &type) const {
  if (const auto *builtin = std::get_if<types::BuiltinType>(&type)) {
    ...
    case types::Float128:
      return "FLOAT";
    case types::Seq:
      return "Seq(VAR(element))";
    case types::Set:
      return "Set(VAR(element))";
    default:
      return "INT";
    }
  }
  ...
  if (const auto *named =
          std::get_if<std::shared_ptr<types::NamedType>>(&type)) {
    const auto &value = *named;
    if (!value || value->name.empty())
      return "ADT";
    if (value->name == "IntArray")
      return "INT_ARRAY";
    if (value->name == "FloatArray")
      return "FLOAT_ARRAY";
    if (value->name == "ByteArray")
      return "BYTE_ARRAY";
    ...
    if (!std::holds_alternative<std::nullptr_t>(value->type)) {
      ...
      if (const auto *product =
          ...
          product && params != types_.adt_type_params.end() &&
  ...
  if (const auto *collection =
          std::get_if<std::shared_ptr<types::SingleItemCollectionType>>(
              &type)) {
    return std::string((*collection)->kind ==
                               types::SingleItemCollectionType::Seq
                           ? "Seq("
                           : "Set(") +
  ...
  if (const auto *promise =
          std::get_if<std::shared_ptr<types::PromiseType>>(&type))
    return "Promise(" + source_type_descriptor((*promise)->valueType) + ")";
  if (const auto *refined =
          std::get_if<std::shared_ptr<types::RefinedType>>(&type))
    return source_type_descriptor((*refined)->base_type);
  return "INT";
}

// Layout must match yona_frame_t in src/Runtime/Core/Exceptions.c:
...
StructType *Codegen::get_frame_type() {
  ...
  yona_frame_ty_ = StructType::create(
      *context_, {ptr_ty, i32_ty, i32_ty, drops_arr_ty}, "yona_frame_t");
  return yona_frame_ty_;
}

// Emit a YonaRuntimeFrameTransfer(ptr) call — no-op unless the ptr is in
...
void Codegen::emit_frame_transfer(Value *ptr) {
  ...
  if (!current_frame_alloca_ || !rt_.frame_transfer_ || !ptr)
    return;
...
static std::pair<CType, std::string>
function_result_identity(const types::Type &type) {
  const types::Type *current = &type;
  while (std::holds_alternative<std::shared_ptr<types::FunctionType>>(*current))
    current =
        &std::get<std::shared_ptr<types::FunctionType>>(*current)->returnType;
...
static bool contains_type_variable_annotation(const types::Type &type) {
  if (is_type_variable_annotation(type))
    return true;
  if (const auto *collection =
...
static void bind_pattern_names(AstNode *pat,
  ...
  case ast::AST_AS_DATA_STRUCTURE_PATTERN: {
    ...
    if (ap->identifier)
      bound.insert(ap->identifier->name->value);
...
void Codegen::bind_parameter_pattern(PatternNode *pat,
  ...
  if (pat->get_type() == ast::AST_TUPLE_PATTERN && value.type == CType::TUPLE) {
    ...
    for (size_t i = 0; i < tuple->patterns.size(); ++i) {
      ...
      if (field->get_type() != ast::AST_PATTERN_VALUE)
        continue;
      ...
      auto *gep = builder_->CreateGEP(i64_ty, tuple_ptr,
                                      {ConstantInt::get(i64_ty, i + 2)},
                                      "param_tuple_gep");
      auto *element = builder_->CreateLoad(i64_ty, gep, "param_tuple_elem");
      CType field_type =
          i < value.subtypes.size() ? value.subtypes[i] : CType::INT;
  ...
  for (size_t i = 0; i < ctor_pat->sub_patterns.size(); ++i) {
    ...
    if (!id)
      continue;
    CType field_type =
        i < ctor.field_types.size() ? ctor.field_types[i] : CType::INT;
    Value *field_value =
        heap_layout
            ? builder_->CreateCall(rt_.adt_get_field_,
                                   {adt_ptr, ConstantInt::get(i64_ty, i)})
            : builder_->CreateExtractValue(value.val,
                                           {static_cast<unsigned>(i + 1)});
    if (heap_layout &&
        (field_type == CType::ADT || field_type == CType::SEQ ||
      ...
      field_value =
          builder_->CreateIntToPtr(field_value, PointerType::get(*context_, 0));
...
TypedValue Codegen::codegen_function_def(FunctionExpr *node,
  ...
  std::string fn_name =
      name.empty()
          ? (node->name.empty() ? "lambda_" + std::to_string(lambda_counter_++)
                                : node->name)
          : name;
  ...
  // and the pattern's variables are added to the bound set for free var
  // analysis.
  ...
  for (size_t i = 0; i < node->patterns.size(); i++) {
    ...
    if (pat->get_type() == ast::AST_PATTERN_VALUE) {
      ...
      if (hp->tail && hp->tail->get_type() == ast::AST_PATTERN_VALUE) {
  ...
  for (auto &v : fv_set) {
    ...
    if (it != named_values_.end()) {
      ...
      // But DO capture function pointers (runtime values: parameters,
      // closures).
  ...
  if (!def.free_vars.empty()) {
    ...
    // A closure object {fn_ptr, cap0, cap1, ...} is created at the current
    // site.
    auto ptr_ty = PointerType::get(*context_, 0);
    auto i64_ty = LType::getInt64Ty(*context_);
    ...
    cf_pre.borrowed_params =
        infer_borrowed_params(fn_name, def, cf_pre.param_types);
    ...
    if (debug_.enabled && debug_.builder && debug_.file) {
      ...
      auto *di_sp = debug_.builder->createFunction(
          debug_.file, fn_name, fn_name, debug_.file, node->Range.Line,
          di_func_ty, node->Range.Line, DINode::FlagZero,
    ...
    for (size_t ci = 0; ci < def.free_vars.size(); ci++) {
      auto *cap_val = builder_->CreateCall(rt_.closure_get_cap_,
                                           {env, ConstantInt::get(i64_ty, ci)},
                                           def.free_vars[ci] + "_cap");
      ...
      if (cap_type == CType::FUNCTION || cap_type == CType::SEQ ||
        ...
        typed_val =
            builder_->CreateBitCast(cap_val, LType::getDoubleTy(*context_));
      ...
      captured.adt_type_argument_names =
    ...
    for (size_t pi = 0; pi < def.param_names.size(); pi++) {
      Value *param = &*arg_it;
      param->setName(def.param_names[pi]);
      ++arg_it;
    ...
    if (body_tv && body_tv.val) {
      ...
      if (ret_val->getType()->isPointerTy()) {
        ret_val = builder_->CreatePtrToInt(ret_val, i64_ty, "closure_ret_ptr");
      } else if (ret_val->getType()->isDoubleTy()) {
        ret_val = builder_->CreateBitCast(ret_val, i64_ty, "closure_ret_float");
      } else if (ret_val->getType()->isIntegerTy()) {
        ret_val =
            builder_->CreateZExtOrTrunc(ret_val, i64_ty, "closure_ret_int");
      } else if (ret_val->getType()->isStructTy()) {
        auto *structure = llvm::cast<llvm::StructType>(ret_val->getType());
        auto *tag =
            builder_->CreateExtractValue(ret_val, {0}, "closure_ret_tag");
        if (!tag->getType()->isIntegerTy(64))
          tag = builder_->CreateZExtOrTrunc(tag, i64_ty);
        auto *boxed = builder_->CreateCall(
            rt_.adt_alloc_,
            {tag, ConstantInt::get(i64_ty, structure->getNumElements() - 1)},
            "closure_ret_adt");
        int64_t heap_mask = 0;
        for (unsigned field = 1; field < structure->getNumElements(); ++field) {
          auto *value = builder_->CreateExtractValue(ret_val, {field},
                                                     "closure_ret_field");
        ...
        ret_val =
            builder_->CreatePtrToInt(boxed, i64_ty, "closure_ret_adt_i64");
      }
    ...
    if (debug_.enabled)
      builder_->SetCurrentDebugLocation(saved_debug_loc);
    ...
    auto *closure = builder_->CreateCall(
        ...
        {fn, ConstantInt::get(i64_ty, static_cast<int64_t>(ret_ctype)),
         ...
         ConstantInt::get(i64_ty, def.free_vars.size())},
        fn_name + "_closure");
    const int64_t parameter_borrow_mask = borrow_mask(cf_pre.borrowed_params);
    ...
    for (size_t ci = 0; ci < capture_tvs.size(); ci++) {
      bool is_self =
          (ci < def.free_vars.size() && def.free_vars[ci] == fn_name);
      ...
      if (capture_tvs[ci].type == CType::ADT &&
          cap_i64->getType()->isStructTy()) {
        ...
        auto *heap_adt = builder_->CreateCall(
            rt_.adt_alloc_, {tag_val, ConstantInt::get(i64_ty, num_fields)},
            "boxed_adt");
        // Copy fields
        for (int fi = 0; fi < num_fields; fi++) {
          Value *field =
              builder_->CreateExtractValue(cap_i64, {(unsigned)(fi + 1)});
      ...
      else if (cap_i64->getType()->isIntegerTy() &&
               cap_i64->getType() != i64_ty)
...
TypedValue Codegen::codegen_lambda_alias(LambdaAlias *node) {
  ...
  // For closures, tv has the closure pointer; for deferred, tv is {nullptr,
  // FUNCTION}
...
Codegen::compile_function(const std::string &name, const DeferredFunction &def,
  ...
  if (def.ast->type_signature.has_value()) {
    auto [ann_params, ann_ret] =
        uncurry_type_signature(*def.ast->type_signature);
    ...
    while (std::holds_alternative<std::shared_ptr<types::FunctionType>>(
      ...
      if (std::holds_alternative<std::shared_ptr<types::FunctionType>>(
        ...
        annotated_function_return_adt_names.push_back(std::move(return_adt));
      } else {
    ...
    } else if (std::holds_alternative<std::shared_ptr<types::NamedType>>(
                   *current_type))
  ...
  for (size_t i = 0; i < def.param_names.size(); i++) {
    const bool is_annotation_variable =
        i < annotated_param_is_variable.size() &&
    ...
    if (i < args.size() && args[i].val &&
        args[i].val->getType() != llvm_type(ct)) {
          ...
          if (ce->expr && ce->expr->get_type() == ast::AST_IDENTIFIER_EXPR) {
            ...
            if (id->name->value == pname) {
              for (auto *cl : ce->clauses) {
                ...
                if (pt == ast::AST_SEQ_PATTERN ||
                    pt == ast::AST_HEAD_TAILS_PATTERN)
                  return true;
              }
            }
          }
          for (auto *cl : ce->clauses)
            if (has_seq_case(cl->body, pname))
              return true;
        }
  ...
  std::function<std::pair<LType *, CType>(AstNode *)> infer_ret =
      [&](AstNode *node) -> std::pair<LType *, CType> {
    if (!node)
    ...
    if (ct == ast::AST_FIELD_ACCESS_EXPR) {
    ...
    if (ct == ast::AST_CASE_EXPR) {
      ...
      if (!ce->clauses.empty())
        return infer_ret(ce->clauses[0]->body);
    }
    if (ct == ast::AST_LET_EXPR)
      return infer_ret(static_cast<LetExpr *>(node)->expr);
    if (ct == ast::AST_IF_EXPR)
      return infer_ret(static_cast<IfExpr *>(node)->thenExpr);
    return {i64_ty, CType::INT};
  };
  ...
  // is ast::AST_MULTIPLY and infer_ret would otherwise leave i64).
  ...
  if (debug_.enabled && debug_.builder && debug_.file) {
    ...
    auto *di_sp = debug_.builder->createFunction(
        debug_.file, name, name, debug_.file, def.ast->Range.Line, di_func_ty,
        def.ast->Range.Line, DINode::FlagZero, DISubprogram::SPFlagDefinition);
    fn->setSubprogram(di_sp);
    debug_.scope = di_sp;
  }
  ...
  if (debug_.enabled && debug_.scope)
    builder_->SetCurrentDebugLocation(
        DILocation::get(*context_, def.ast->Range.Line, 0, debug_.scope));
  ...
  for (auto &arg : fn->args()) {
    ...
    if (ct == CType::FUNCTION && i < annotated_function_return_adt_names.size())
    ...
    if (i < args.size())
      param_tv.boxed_heap = args[i].boxed_heap;
    ...
    if (debug_.enabled && debug_.scope && debug_.builder &&
        i < def.param_names.size()) {
      auto *alloca =
          builder_->CreateAlloca(arg.getType(), nullptr, pname + ".dbg");
      builder_->CreateStore(&arg, alloca);
      auto *di_param = debug_.builder->createParameterVariable(
          debug_.scope, pname, i + 1, debug_.file, def.ast->Range.Line,
          di_type_for(ct));
      debug_.builder->insertDeclare(
          alloca, di_param, debug_.builder->createExpression(),
          DILocation::get(*context_, def.ast->Range.Line, 0, debug_.scope),
        ...
        while (cur) {
          if (auto *nc = dynamic_cast<NameCall *>(cur->call)) {
            if (nc->name->value == name ||
                (has_source_self_alias && nc->name->value == def.ast->name))
              return true;
            break;
          }
  ...
  // NULL their slot via YonaRuntimeFrameTransfer so we don't double-dec.
  ...
  current_frame_alloca_ = nullptr;
  {
    ...
    for (size_t pi = 0; pi < def.param_names.size(); pi++) {
      ...
      if (param->getType()->isStructTy())
        continue;
      if (!param->getType()->isPointerTy() && !param->getType()->isIntegerTy())
        continue;
      Value *p = param;
    ...
    if (!heap_param_ptrs.empty() &&
        ...
        builder_->GetInsertBlock() && !has_self_tail_call) {
      auto *i32_ty = LType::getInt32Ty(*context_);
      auto *i64_ty = LType::getInt64Ty(*context_);
      ...
      auto *depth = builder_->CreateCall(rt_.try_depth_, {}, "try_depth");
      auto *in_try =
          builder_->CreateICmpSGT(depth, ConstantInt::get(i32_ty, 0), "in_try");
      ...
      builder_->CreateStore(
          ConstantInt::get(i32_ty, (int32_t)heap_param_ptrs.size()), dc_gep);
  ...
  tco_param_names_ =
      has_self_tail_call ? def.param_names : std::vector<std::string>{};
  ...
  CType ret_ctype =
      has_annotated_ret ? annotated_ret : (body_tv ? body_tv.type : CType::INT);
  ...
  if (body_tv && body_tv.val) {
    ...
    if (body_tv.val->getType() != ret_type) {
      ...
      if (has_external_users) {
        report_error(def.ast->Range,
                     "function ABI mismatch for '" + name +
                         "': its inferred return type changed after callers "
                         "were emitted; "
    ...
    if (body_tv.val->getType() != ret_type) {
      // A body can refine an unannotated result from the conservative
      // provisional ABI. Replace that provisional function atomically after
      // migrating every compiler-owned reference; no second callable ABI is
      // retained.
      LType *ActualReturnType = body_tv.val->getType();
      auto *ObsoleteFunction = fn;
      auto NewFunctionType =
          llvm::FunctionType::get(ActualReturnType, param_types, false);
      auto *Replacement =
          Function::Create(NewFunctionType, Function::InternalLinkage,
                           name + ".abi_rebuild", module_);

      auto migrate_bindings = [&](auto &bindings) {
        for (auto &[_, value] : bindings)
          if (value.val == ObsoleteFunction)
            value.val = Replacement;
      };
      migrate_bindings(saved_values);
      migrate_function_references(ObsoleteFunction, Replacement);

      // The frame alloca and deferred drops belong to the provisional body.
      // Clear the builder before erasing that CFG, then make the replacement
      // the sole function with the source name.
      current_frame_alloca_ = nullptr;
      discard_pending_transfer_drops(ObsoleteFunction);
      builder_->ClearInsertionPoint();
      ObsoleteFunction->replaceAllUsesWith(Replacement);
      ObsoleteFunction->eraseFromParent();
      ...
      if (debug_.enabled && debug_.builder && debug_.file) {
        auto *di_func_ty = di_func_type(param_ctypes, preliminary_ret);
        auto *di_sp = debug_.builder->createFunction(
            debug_.file, name, name, debug_.file, def.ast->Range.Line,
            di_func_ty, def.ast->Range.Line, DINode::FlagZero,
            DISubprogram::SPFlagDefinition);
        fn->setSubprogram(di_sp);
        debug_.scope = di_sp;
      }
        ...
        if (i < def.param_names.size()) {
          ...
          if (st.empty() && i < annotated_adt_arguments_by_param.size() &&
              (ct == CType::SEQ || ct == CType::SET || ct == CType::DICT ||
               ct == CType::TUPLE))
            st = annotated_adt_arguments_by_param[i];
          if (ct == CType::FUNCTION && st.empty() && i < args.size() &&
              args[i].val) {
            if (auto *fn_val = dyn_cast<Function>(args[i].val)) {
          ...
          pname = def.free_vars[i - def.param_names.size()];
          size_t ci = i - def.param_names.size();
          ct = (ci < capture_values.size()) ? capture_values[ci].type
                                            : CType::INT;
        }
        arg.setName(pname);
        TypedValue param_tv{&arg, ct, st};
      ...
      ret_ctype = has_annotated_ret ? annotated_ret
                                    : (body_tv ? body_tv.type : CType::INT);
    }
    if (!current_block_terminated()) {
      ...
      if (body_tv.val && !tco_cleanup_done_) {
        ...
        for (size_t pi = 0; pi < def.param_names.size(); pi++) {
          ...
          if (ct == CType::SEQ && is_transferred(param, TransferDomain::Seq))
            continue;
          ...
          if (param_ptr->getType()->isPointerTy() &&
              ret_ptr->getType()->isPointerTy()) {
            auto *is_same =
                builder_->CreateICmpEQ(param_ptr, ret_ptr, "param_is_ret");
  ...
  for (size_t i = 0;
       i < cf.param_adt_names.size() && i < annotated_adt_names.size(); ++i) {
    if (param_ctypes[i] == CType::ADT && !annotated_adt_names[i].empty() &&
        !std::islower(
            static_cast<unsigned char>(annotated_adt_names[i].front()))) {
  ...
  if (def.ast->type_signature.has_value()) {
    ...
    while (std::holds_alternative<std::shared_ptr<types::FunctionType>>(
        *annotated)) {
      const auto &function =
          std::get<std::shared_ptr<types::FunctionType>>(*annotated);
  ...
  // an io_uring submit (direct call + YonaRuntimeIoAwait) rather than a
