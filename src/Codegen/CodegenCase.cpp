...
bool Codegen::codegen_pattern_headtail(HeadTailsPattern *htp, CaseExpr *node,
  ...
  if (htp->heads.size() == 1) {
    auto *count_ptr =
        builder_->CreateGEP(i64_ty, seq_ptr, {ConstantInt::get(i64_ty, 0)});
    auto *count = builder_->CreateLoad(i64_ty, count_ptr, "seq_count");
    builder_->CreateCondBr(
        builder_->CreateICmpSGT(count, ConstantInt::get(i64_ty, 0)), body_bb,
        next_bb);
  } else {
    auto len = builder_->CreateCall(rt_.seq_length_, {seq_ptr});
    auto min_len = ConstantInt::get(i64_ty, htp->heads.size());
    builder_->CreateCondBr(builder_->CreateICmpSGE(len, min_len), body_bb,
                           next_bb);
  }
  ...
  CType elem_type =
      !scrutinee.semantic_subtypes.empty()
          ...
          : (!scrutinee.subtypes.empty() ? scrutinee.subtypes.front()
                                         : CType::INT);
  ...
  bool owned = true;
  if (node->expr->get_type() == ast::AST_IDENTIFIER_EXPR) {
  ...
  if (htp->tail) {
    ...
    if (htp->tail->get_type() == ast::AST_PATTERN_VALUE) {
    ...
    } else if (htp->tail->get_type() == ast::AST_HEAD_TAILS_PATTERN) {
      auto *nested_tail_bb =
          BasicBlock::Create(*context_, "tail.pat.match", body_bb->getParent());
...
bool Codegen::codegen_pattern_seq(SeqPattern *sp, const TypedValue &scrutinee,
  ...
  if (sp->patterns.empty()) {
    auto *count_ptr =
        builder_->CreateGEP(i64_ty, seq_val, {ConstantInt::get(i64_ty, 0)});
    auto *count = builder_->CreateLoad(i64_ty, count_ptr, "seq_count");
    builder_->CreateCondBr(
        builder_->CreateICmpEQ(count, ConstantInt::get(i64_ty, 0)), body_bb,
        next_bb);
    ...
    auto *count =
        builder_->CreateCall(rt_.seq_length_, {seq_val}, "seq_exact_count");
    auto *bind_bb = BasicBlock::Create(*context_, "seq.exact.bind",
                                       builder_->GetInsertBlock()->getParent());
    builder_->CreateCondBr(
        builder_->CreateICmpEQ(count,
                               ConstantInt::get(i64_ty, sp->patterns.size())),
        bind_bb, next_bb);
    ...
    const SemanticTypeIdentity *element_identity =
        scrutinee.semantic_subtypes.empty()
            ? nullptr
            : &scrutinee.semantic_subtypes.front();
    ...
    bind_pattern = [&](PatternNode *pattern, Value *carrier,
      ...
      if (!pattern || pattern->get_type() == ast::AST_UNDERSCORE_PATTERN)
        return;
        ...
        if (auto *id = std::get_if<IdentifierExpr *>(&value_pattern->expr)) {
          ...
          if (is_heap_type(type))
            value = builder_->CreateIntToPtr(
                carrier, PointerType::get(*context_, 0), "seq_pattern_ptr");
          else if (type == CType::FLOAT)
            value = builder_->CreateBitCast(
                carrier, LType::getDoubleTy(*context_), "seq_pattern_float");
      ...
      if (tuple_ptr->getType()->isIntegerTy())
        tuple_ptr = builder_->CreateIntToPtr(
            tuple_ptr, PointerType::get(*context_, 0), "seq_pattern_tuple");
      for (size_t i = 0; i < tuple->patterns.size(); ++i) {
        ...
        auto *field =
            builder_->CreateLoad(i64_ty, slot, "seq_pattern_tuple_field");
        const SemanticTypeIdentity *field_identity =
            identity && i < identity->arguments.size() ? &identity->arguments[i]
                                                       : nullptr;
...
bool Codegen::codegen_pattern_tuple(TuplePattern *tp,
                                    const TypedValue &scrutinee,
                                    BasicBlock *body_bb, BasicBlock *next_bb) {
  auto i64_ty = LType::getInt64Ty(*context_);
  ...
  if (tuple_val->getType()->isIntegerTy()) {
    tuple_val =
        builder_->CreateIntToPtr(tuple_val, PointerType::get(*context_, 0));
    is_ptr = true;
  }
  ...
  for (size_t ti = 0; ti < tp->patterns.size(); ti++) {
    ...
    if (is_ptr) {
      auto *gep = builder_->CreateGEP(
          i64_ty, tuple_val, {ConstantInt::get(i64_ty, ti + 2)}, "tuple_gep");
      elem = builder_->CreateLoad(i64_ty, gep, "tuple_elem");
    } else {
      elem = builder_->CreateExtractValue(tuple_val, {(unsigned)ti});
    }
    CType et =
        (ti < scrutinee.subtypes.size()) ? scrutinee.subtypes[ti] : CType::INT;
    auto *sub = tp->patterns[ti];
    if (sub->get_type() == ast::AST_PATTERN_VALUE) {
      auto *pv = static_cast<PatternValue *>(sub);
...
bool Codegen::codegen_pattern_constructor(ConstructorPattern *cp,
                                          const TypedValue &scrutinee_in,
                                          BasicBlock *body_bb,
                                          BasicBlock *next_bb) {
  ...
  std::function<void(const std::vector<PatternNode *> &, Value *,
                     const FieldShape &)>
      bind_tuple;
  bind_tuple = [&](const std::vector<PatternNode *> &patterns,
                   Value *tuple_value, const FieldShape &shape) {
    if (!tuple_value)
      return;
    Value *tuple_ptr = tuple_value;
    if (tuple_ptr->getType()->isIntegerTy())
      tuple_ptr = builder_->CreateIntToPtr(
          tuple_ptr, PointerType::get(*context_, 0), "ctor_tuple_field_ptr");
    for (size_t ti = 0;
         ti < patterns.size() && ti < shape.tuple_elements.size(); ++ti) {
      auto *gep = builder_->CreateGEP(i64_ty, tuple_ptr,
                                      {ConstantInt::get(i64_ty, ti + 2)},
                                      "ctor_tuple_gep");
  ...
  bind_pattern = [&](PatternNode *pattern, Value *raw_value,
                     const FieldShape &shape) {
    if (!pattern || !raw_value)
      return;
    if (shape.type == CType::TUPLE &&
        pattern->get_type() == ast::AST_TUPLE_PATTERN) {
      auto *tuple = static_cast<TuplePattern *>(pattern);
      std::vector<PatternNode *> elements(tuple->patterns.begin(),
                                          tuple->patterns.end());
    ...
    if (shape.type == CType::FLOAT && raw_value->getType()->isIntegerTy())
      typed_value =
          builder_->CreateBitCast(raw_value, LType::getDoubleTy(*context_));
    else if (is_heap_type(shape.type) && raw_value->getType()->isIntegerTy())
      typed_value = builder_->CreateIntToPtr(
          raw_value, PointerType::get(*context_, 0), "ctor_tuple_element_ptr");
  ...
  auto bind_field_pattern = [&](size_t field_index, Value *field_value) {
    ...
    const auto &shape = field_index < ctor_it->second.field_shapes.size()
                            ? ctor_it->second.field_shapes[field_index]
                            : fallback;
  ...
  // `YonaRuntimeTaskAwait` for a CAF returning an Option). Coerce back to
  ...
  if (scrutinee.val && scrutinee.val->getType()->isIntegerTy()) {
    scrutinee.val = builder_->CreateIntToPtr(
        scrutinee.val, PointerType::get(*context_, 0), "adt.scrutinee.ptr");
  ...
  auto concrete_field_type = [&](size_t field_index) -> CType {
      ...
      if (!declared.is_function_type && !declared.is_tuple_type &&
        ...
        if (params != types_.adt_type_params.end()) {
          for (size_t i = 0; i < params->second.size() &&
                             i < scrutinee.adt_type_arguments.size();
               ++i) {
    ...
    return field_index < scrutinee.subtypes.size()
               ? scrutinee.subtypes[field_index]
               : CType::INT;
  };
  ...
  auto concrete_field_adt_name = [&](size_t field_index) -> std::string {
    ...
    if (params == types_.adt_type_params.end())
      return {};
    for (size_t i = 0; i < params->second.size() &&
                       i < scrutinee.adt_type_argument_names.size();
         ++i)
      if (params->second[i] == declared.name)
        return scrutinee.adt_type_argument_names[i];
    return {};
  };
  auto concrete_field_identity =
      [&](size_t field_index) -> const SemanticTypeIdentity * {
    if (field_index >= ctor_it->second.declared_field_types.size())
      return nullptr;
    const auto &declared = ctor_it->second.declared_field_types[field_index];
    if (declared.is_function_type || declared.is_tuple_type ||
    ...
    if (params == types_.adt_type_params.end())
      return nullptr;
    for (size_t i = 0; i < params->second.size() &&
                       i < scrutinee.adt_semantic_arguments.size();
         ++i)
      if (params->second[i] == declared.name)
  ...
  if (use_heap_layout) {
    ...
    builder_->CreateCondBr(
        builder_->CreateICmpEQ(scr_tag, ConstantInt::get(i64_ty, tag)), body_bb,
        next_bb);
    builder_->SetInsertPoint(body_bb);
    const size_t field_count = std::min(
        cp->sub_patterns.size(), static_cast<size_t>(ctor_it->second.arity));
    for (size_t fi = 0; fi < field_count; fi++) {
      auto field_val = builder_->CreateCall(
          rt_.adt_get_field_, {scrutinee.val, ConstantInt::get(i64_ty, fi)});
      auto *sub_pat = cp->sub_patterns[fi];
      if (sub_pat->get_type() == ast::AST_PATTERN_VALUE) {
        ...
        if (auto *id = std::get_if<IdentifierExpr *>(&pv->expr)) {
          ...
          CType ftype = declared_parameter && fi < scrutinee.subtypes.size()
                            ? runtime_st
          ...
          if (is_heap_type(ftype))
            ...
            typed_val = builder_->CreateBitCast(field_val,
          ...
          if (fi < ctor_it->second.declared_field_types.size()) {
            const auto &declared = ctor_it->second.declared_field_types[fi];
            const auto params =
                types_.adt_type_params.find(ctor_it->second.type_name);
    ...
    builder_->CreateCondBr(
        builder_->CreateICmpEQ(scr_tag, ConstantInt::get(tag_ty, tag)), body_bb,
        next_bb);
    builder_->SetInsertPoint(body_bb);
    const size_t field_count = std::min(
        cp->sub_patterns.size(), static_cast<size_t>(ctor_it->second.arity));
    for (size_t fi = 0; fi < field_count; fi++) {
      auto field_val =
          builder_->CreateExtractValue(scrutinee.val, {(unsigned)(fi + 1)});
      auto *sub_pat = cp->sub_patterns[fi];
      if (sub_pat->get_type() == ast::AST_PATTERN_VALUE) {
        auto *pv = static_cast<PatternValue *>(sub_pat);
        if (auto *id = std::get_if<IdentifierExpr *>(&pv->expr)) {
...
Codegen::CasePatternAnalysis
Codegen::analyze_case_patterns(CaseExpr *node) const {
  CasePatternAnalysis result;
  if (!node)
    return result;
  const pattern_analysis::ConstructorCatalog constructors{
      .lookup = [this](std::string_view name)
          -> std::optional<pattern_analysis::ConstructorInfo> {
  ...
  if (analysis.incomplete)
    result.incomplete =
        FiniteCaseCoverage{std::move(analysis.incomplete->family),
                           std::move(analysis.incomplete->missing)};
  return result;
  ...
  auto collect = [&](const auto &self, PatternNode *pat) -> void {
    ...
    if (pat->get_type() == ast::AST_CONSTRUCTOR_PATTERN) {
      ...
      if (std::get_if<IdentifierExpr *>(&pv->expr)) {
        ...
        if (literal_type == ast::AST_TRUE_LITERAL_EXPR) {
          saw_bool = true;
          covered_atoms.insert("True");
        }
        if (literal_type == ast::AST_FALSE_LITERAL_EXPR) {
          saw_bool = true;
          covered_atoms.insert("False");
        }
        if (literal_type == ast::AST_UNIT_EXPR) {
          saw_unit = true;
          covered_atoms.insert("()");
        }
      }
    } else if (pat->get_type() == ast::AST_OR_PATTERN) {
      for (const auto &alternative : static_cast<OrPattern *>(pat)->patterns)
  ...
  for (size_t index = 0; index < node->clauses.size(); ++index) {
    ...
    if (!clause->guard) {
      ...
      if (clause->pattern->get_type() == ast::AST_OR_PATTERN) {
        covered_by_prior = true;
        for (const auto &alternative :
             static_cast<OrPattern *>(clause->pattern)->patterns) {
          bool covered = false;
          for (auto *previous : prior_unguarded)
            covered = covered ||
                      pattern_analysis::covers(previous, alternative.get());
          covered_by_prior = covered_by_prior && covered;
        }
      } else {
        for (auto *previous : prior_unguarded)
          covered_by_prior = covered_by_prior || pattern_analysis::covers(
                                                     previous, clause->pattern);
      }
  ...
  if (!adt_type_name.empty() && !missing.empty())
    result.incomplete =
        FiniteCaseCoverage{std::move(adt_type_name), std::move(missing)};
  return result;
}

std::optional<Codegen::FiniteCaseCoverage>
Codegen::finite_case_coverage(CaseExpr *node) const {
...
TypedValue Codegen::codegen_case(CaseExpr *node) {
  ...
  if (scrutinee.type != CType::SUM && !node->clauses.empty()) {
    for (auto *clause : node->clauses) {
      if (clause->pattern->get_type() == ast::AST_TYPED_PATTERN) {
  ...
  if ((scrutinee.type == CType::INT ||
    ...
    std::string ctor_name;
    if (first_pat->get_type() == ast::AST_CONSTRUCTOR_PATTERN)
      ctor_name =
          static_cast<ConstructorPattern *>(first_pat)->constructor_name;
    else if (first_pat->get_type() == ast::AST_RECORD_PATTERN)
      ctor_name = static_cast<RecordPattern *>(first_pat)->recordType;
    if (!ctor_name.empty()) {
      auto ctor_it = types_.adt_constructors.find(ctor_name);
      if (ctor_it != types_.adt_constructors.end()) {
        if (!(ctor_it->second.max_arity == 0 &&
              !ctor_it->second.is_recursive)) {
          scrutinee.val = builder_->CreateIntToPtr(
              scrutinee.val, PointerType::get(*context_, 0));
          scrutinee.type = CType::ADT;
    ...
    for (size_t index : analysis.unreachable_clauses) {
      if (index < node->clauses.size() && node->clauses[index])
        Session->diagnostics().warning(
            node->clauses[index]->Range,
            "unreachable pattern: earlier unguarded arms already cover every "
            "value it can match",
            compiler::WarningFlag::OverlappingPatterns);
    }
    if (auto coverage = analysis.incomplete) {
      std::string message = "non-exhaustive pattern match on " +
  ...
  for (auto *clause : node->clauses) {
    if (!clause || clause->guard ||
        clause->pattern->get_type() != ast::AST_CONSTRUCTOR_PATTERN) {
  ...
  BasicBlock *impossible_match_bb =
      finite_constructor_match
          ? BasicBlock::Create(*context_, "case.impossible", fn)
          : nullptr;
  ...
  for (size_t i = 0; i < node->clauses.size(); i++) {
    ...
    const bool catch_all =
        !clause->guard &&
        (pat->get_type() == ast::AST_UNDERSCORE_PATTERN ||
         (pat->get_type() == ast::AST_PATTERN_VALUE &&
          std::get_if<IdentifierExpr *>(
              &static_cast<PatternValue *>(pat)->expr) != nullptr));
    auto arm_named_values = named_values_;
    ActiveNamedValueSnapshot arm_named_values_snapshot(*this, arm_named_values);
    auto body_bb =
        BasicBlock::Create(*context_, "case.body." + std::to_string(i), fn);
    auto next_bb =
        (!catch_all && i + 1 < node->clauses.size())
            ? BasicBlock::Create(*context_,
                                 "case.next." + std::to_string(i + 1), fn)
    ...
    if (pat->get_type() == ast::AST_UNDERSCORE_PATTERN) {
      ...
      if (!seq_ptr->getType()->isPointerTy())
        seq_ptr =
            builder_->CreateIntToPtr(seq_ptr, PointerType::get(*context_, 0));
      body_inline = codegen_pattern_headtail(
          static_cast<HeadTailsPattern *>(pat), node, clause, scrutinee,
          seq_ptr, body_bb, next_bb);
    } else if (pat->get_type() == ast::AST_SEQ_PATTERN) {
      ...
      // For each alternative in the or-pattern, test and branch to body_bb on
      // match
      for (size_t oi = 0; oi < op->patterns.size(); oi++) {
        auto *alt = op->patterns[oi].get();
        auto alt_next =
            (oi + 1 < op->patterns.size())
                ? BasicBlock::Create(*context_,
                                     "case.or." + std::to_string(i) + "." +
                                         std::to_string(oi + 1),
                                     fn)
                : next_bb;

        if (alt->get_type() == ast::AST_UNDERSCORE_PATTERN) {
          builder_->CreateBr(body_bb);
        } else if (alt->get_type() == ast::AST_PATTERN_VALUE) {
          auto *pv = static_cast<PatternValue *>(alt);
          if (auto *sym = std::get_if<SymbolExpr *>(&pv->expr)) {
            int64_t sym_id = intern_symbol((*sym)->value);
            auto sym_val =
                ConstantInt::get(LType::getInt64Ty(*context_), sym_id);
            auto cmp = builder_->CreateICmpEQ(scrutinee.val, sym_val);
    ...
    } else if (pat->get_type() == ast::AST_CONSTRUCTOR_PATTERN) {
      body_inline = codegen_pattern_constructor(
          static_cast<ConstructorPattern *>(pat), scrutinee, body_bb, next_bb);
    } else if (pat->get_type() == ast::AST_RECORD_PATTERN) {
      ...
      if (ctor_it != types_.adt_constructors.end()) {
        ...
        bind_field = [&](PatternNode *field_pattern, Value *raw_value,
                         const FieldShape &shape) {
          if (!field_pattern || !raw_value)
            return;
          if (shape.type == CType::TUPLE &&
              field_pattern->get_type() == ast::AST_TUPLE_PATTERN) {
            auto *tuple_pattern = static_cast<TuplePattern *>(field_pattern);
            Value *tuple_ptr = raw_value;
            if (tuple_ptr->getType()->isIntegerTy())
              tuple_ptr = builder_->CreateIntToPtr(
                  tuple_ptr, PointerType::get(*context_, 0),
                  "record_tuple_field_ptr");
            for (size_t element = 0; element < tuple_pattern->patterns.size() &&
                                     element < shape.tuple_elements.size();
                 ++element) {
              auto *slot = builder_->CreateGEP(
                  i64_ty, tuple_ptr, {ConstantInt::get(i64_ty, element + 2)},
                  "record_tuple_gep");
              auto *value =
                  builder_->CreateLoad(i64_ty, slot, "record_tuple_element");
              bind_field(tuple_pattern->patterns[element], value,
                         shape.tuple_elements[element]);
            }
            return;
          }
          if (field_pattern->get_type() != ast::AST_PATTERN_VALUE)
            return;
          ...
          if (shape.type == CType::FLOAT && raw_value->getType()->isIntegerTy())
            typed_value = builder_->CreateBitCast(
                raw_value, LType::getDoubleTy(*context_));
          else if (is_heap_type(shape.type) &&
                   raw_value->getType()->isIntegerTy())
            typed_value = builder_->CreateIntToPtr(
                raw_value, PointerType::get(*context_, 0), "record_field_ptr");
          TypedValue bound{typed_value, shape.type};
          if (shape.type == CType::FUNCTION) {
            bound.subtypes = {shape.function_return_type};
            bound.adt_type_name = shape.function_return_adt_name;
        ...
        if (use_heap_layout) {
          auto scr_tag =
              builder_->CreateCall(rt_.adt_get_tag_, {scrutinee.val});
          builder_->CreateCondBr(
    ...
    } else if (pat->get_type() == ast::AST_TYPED_PATTERN) {
    ...
    if (incoming->getType() != phi_type) {
      ...
      if (tv.type == CType::ADT && incoming->getType()->isStructTy() &&
        ...
        auto *boxed = builder_->CreateCall(
            rt_.adt_alloc_,
            {tag,
             ConstantInt::get(i64_type, struct_type->getNumElements() - 1)},
            "case_adt_box");
        int64_t heap_mask = 0;
        for (unsigned field = 1; field < struct_type->getNumElements();
             ++field) {
          ...
          if (subtype < tv.subtypes.size() &&
              is_heap_type(tv.subtypes[subtype]))
...
TypedValue Codegen::box_as_sum(const TypedValue &value) {
  ...
  builder_->CreateCall(rt_.tuple_set_, {tuple_ptr, ConstantInt::get(i64_ty, 0),
                                        ConstantInt::get(i64_ty, tag)});
...
bool Codegen::codegen_pattern_typed(TypedPattern *pat,
                                    const TypedValue &scrutinee,
                                    BasicBlock *body_bb, BasicBlock *next_bb) {
  auto i64_ty = LType::getInt64Ty(*context_);
  ...
  auto *tag_gep = builder_->CreateGEP(
      i64_ty, tuple_ptr, {ConstantInt::get(i64_ty, 2)}, "sum_tag_gep");
  ...
  auto *cmp = builder_->CreateICmpEQ(
      tag_val, ConstantInt::get(i64_ty, expected_tag), "sum_tag_match");
  builder_->CreateCondBr(cmp, body_bb, next_bb);

  // In the body block, extract the value and bind it
  builder_->SetInsertPoint(body_bb);
  auto *val_gep = builder_->CreateGEP(
      i64_ty, tuple_ptr, {ConstantInt::get(i64_ty, 3)}, "sum_val_gep");
  ...
  if (expected_ct == CType::FLOAT)
    ...
    typed_val =
        builder_->CreateIntToPtr(raw_val, PointerType::get(*context_, 0));
  else if (expected_ct == CType::SEQ || expected_ct == CType::SET ||
           expected_ct == CType::DICT)
    typed_val =
        builder_->CreateIntToPtr(raw_val, PointerType::get(*context_, 0));
