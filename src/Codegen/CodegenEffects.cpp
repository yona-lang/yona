...
TypedValue Codegen::codegen_perform(PerformExpr *node) {
  ...
  if (node->effect_name == "Cancel" && node->operation_name == "check") {
    if (current_group_) {
      ...
      auto *cancelled = builder_->CreateCall(rt_.group_is_cancelled_,
                                             {current_group_}, "cancelled");
      auto *is_cancelled = builder_->CreateICmpNE(
          cancelled, ConstantInt::get(i64_ty, 0), "is_cancelled");
  ...
  if (!handler_fn) {
    ...
    report_error(node->Range, "unhandled effect operation: " + op_key);
    return {};
  }
  ...
  for (auto *arg : node->args) {
    ...
    if (!tv)
      return {};
    if (tv.type == CType::UNIT)
      continue;
    Value *v = tv.val;
    if (v->getType()->isPointerTy())
      v = builder_->CreatePtrToInt(v, i64_ty);
    call_args.push_back(v);
  }

  // Identity continuation
  auto *id_type = llvm::FunctionType::get(i64_ty, {i64_ty}, false);
  auto *id_fn = Function::Create(
      id_type, Function::InternalLinkage,
      "resume_id_" + std::to_string(lambda_counter_++), module_);
  {
...
TypedValue Codegen::codegen_handle(HandleExpr *node) {
  ...
  for (auto *clause : node->clauses) {
    ...
    named_values_[clause->resume_name] = {
        &*arg_it, CType::FUNCTION, {CType::INT}};
    ...
    if (!current_block_terminated()) {
      ...
      if (rv->getType()->isPointerTy())
        rv = builder_->CreatePtrToInt(rv, i64_ty);
