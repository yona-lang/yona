...
equality_type_argument(typechecker::MonoTypePtr type) {
  if (!type)
    return std::nullopt;
...
static SemanticTypeIdentity
semantic_type_identity(typechecker::TypeChecker &checker,
...
static Value *coerce_for_phi(Value *val, LType *target, IRBuilder<> &builder,
                             LLVMContext &ctx) {
...
static LType *common_phi_type(LType *a, LType *b, LLVMContext &ctx) {
  ...
  if (a->isFloatingPointTy() || b->isFloatingPointTy())
    return a->isDoubleTy() || b->isDoubleTy() ? LType::getDoubleTy(ctx)
...
TypedValue Codegen::codegen_string(StringExpr *node) {
  ...
  auto *as_i8 =
      builder_->CreateBitCast(bytes_ptr, PointerType::get(*context_, 0));
...
TypedValue Codegen::codegen_binary(AstNode *left_node, AstNode *right_node,
                                   const std::string &op) {
  set_debug_loc(left_node->Range);
  ...
  if (op == "+" || op == "-" || op == "*" || op == "/" || op == "%") {
    ...
    bool right_numeric =
        (right.type == CType::INT || right.type == CType::FLOAT);
    bool both_string =
        (left.type == CType::STRING && right.type == CType::STRING);
    if (!left_numeric && !right_numeric && !both_string) {
      report_error(left_node->Range,
                   "type error: operator '" + op +
  ...
  if (left.type == CType::STRING && right.type == CType::STRING && op == "+") {
    return {builder_->CreateCall(rt_.string_concat_, {left.val, right.val}),
            CType::STRING};
  }
  ...
  if (left.type == CType::FLOAT && right.type == CType::INT) {
    right.val =
        builder_->CreateSIToFP(right.val, LType::getDoubleTy(*context_));
  ...
  if (op == "+")
    result = is_float ? builder_->CreateFAdd(left.val, right.val)
                      : builder_->CreateAdd(left.val, right.val);
  else if (op == "-")
    result = is_float ? builder_->CreateFSub(left.val, right.val)
                      : builder_->CreateSub(left.val, right.val);
  else if (op == "*")
    result = is_float ? builder_->CreateFMul(left.val, right.val)
                      : builder_->CreateMul(left.val, right.val);
  else if (op == "/")
    result = is_float ? builder_->CreateFDiv(left.val, right.val)
                      : builder_->CreateSDiv(left.val, right.val);
  else if (op == "%")
    result = builder_->CreateSRem(left.val, right.val);

  return {result, is_float ? CType::FLOAT : CType::INT};
}

TypedValue Codegen::codegen_comparison(AstNode *left_node, AstNode *right_node,
                                       const std::string &op) {
  set_debug_loc(left_node->Range);
  auto left = auto_await(codegen(left_node));
  auto right = auto_await(codegen(right_node));
  ...
  auto attach_adt_arguments = [&](TypedValue &value, AstNode *source) {
    ...
    if (inferred->tag == typechecker::MonoType::App) {
      if (value.adt_type_name.empty())
        value.adt_type_name = inferred->type_name;
    ...
    for (auto *argument : arguments) {
      ...
      concrete_argument_names.push_back(semantic_argument->tag ==
                                                typechecker::MonoType::App
                                            ? semantic_argument->type_name
                                            : std::string{});
  ...
  auto resolve_adt_method =
      [&](const std::string &method_name) -> ResolvedMethod {
    std::vector<TypedValue> args = {left, right};
    const std::string instance_symbol =
        resolve_trait_method(method_name, left.type, left.adt_type_name,
      ...
      if (source != imports_.imported_sources.end() &&
        ...
        for (size_t i = 0; i < semantic_arguments.size(); ++i) {
          if (i < left.adt_semantic_arguments.size())
            append_semantic_type_key(specialization,
                                     left.adt_semantic_arguments[i]);
          else {
            specialization +=
                std::to_string(static_cast<int>(semantic_arguments[i]));
            if (i < left.adt_type_argument_names.size() &&
                !left.adt_type_argument_names[i].empty())
              specialization += "_" + left.adt_type_argument_names[i];
          }
          specialization += "_";
        }
        ...
        if (instance != compiled_functions_.end())
          callable_symbol = specialization;
      }
      if (instance == compiled_functions_.end()) {
    ...
    return instance == compiled_functions_.end()
               ? ResolvedMethod{}
               : ResolvedMethod{callable_symbol, &instance->second};
  };
  ...
  const bool aggregate_equality =
      left.type == CType::ADT || left.type == CType::SEQ ||
      left.type == CType::TUPLE || left.type == CType::SET ||
      left.type == CType::DICT;
  if ((op == "==" || op == "!=") && aggregate_equality &&
      left.type == right.type) {
    auto resolved = resolve_adt_method("eq");
    if (!resolved.function) {
      const std::string type_name = !left.adt_type_name.empty()
                                        ? left.adt_type_name
                                        : ctype_to_type_name(left.type);
      report_error(left_node->Range,
                   "no Eq instance for '" + type_name +
                       "'; add `deriving Eq`, define an instance, or use an "
                       "explicit comparator");
      return {};
    }
    auto equal =
        emit_direct_call(resolved.symbol, *resolved.function, {left, right});
    if (!equal)
      return {};
    ...
    return {op == "==" ? eq_bool : builder_->CreateNot(eq_bool, "neq"),
            CType::BOOL};
  }
  ...
  if ((op == "<" || op == ">" || op == "<=" || op == ">=") &&
    ...
    if (!resolved.function) {
      ...
      report_error(left_node->Range,
                   "no Ord instance for '" + type_name +
                       "'; add `deriving Ord`, define an instance, or use an "
                       "explicit comparator");
      return {};
    }
    auto ordering =
    ...
    if (op == "<")
      comparison = builder_->CreateICmpULT(tag, equal_tag);
    else if (op == ">")
      comparison = builder_->CreateICmpUGT(tag, equal_tag);
    else if (op == "<=")
      comparison = builder_->CreateICmpULE(tag, equal_tag);
    else
      comparison = builder_->CreateICmpUGE(tag, equal_tag);
    return {comparison, CType::BOOL};
  }

  bool is_float = (left.type == CType::FLOAT || right.type == CType::FLOAT);
  if (is_float) {
    if (left.type == CType::INT)
      left.val =
          builder_->CreateSIToFP(left.val, LType::getDoubleTy(*context_));
    if (right.type == CType::INT)
      right.val =
          builder_->CreateSIToFP(right.val, LType::getDoubleTy(*context_));
  }
  ...
  if (is_float) {
    if (op == "==")
      result = builder_->CreateFCmpOEQ(left.val, right.val);
    else if (op == "!=")
      result = builder_->CreateFCmpONE(left.val, right.val);
    else if (op == "<")
      result = builder_->CreateFCmpOLT(left.val, right.val);
    else if (op == ">")
      result = builder_->CreateFCmpOGT(left.val, right.val);
    else if (op == "<=")
      result = builder_->CreateFCmpOLE(left.val, right.val);
    else if (op == ">=")
      result = builder_->CreateFCmpOGE(left.val, right.val);
    ...
    auto *eq =
        builder_->CreateCall(rt_.string_eq_, {as_str(left), as_str(right)});
    ...
    if (left.val->getType() != right.val->getType()) {
      if (right.val->getType()->isPointerTy() &&
          left.val->getType()->isIntegerTy())
        left.val = builder_->CreateIntToPtr(left.val, right.val->getType());
      else if (left.val->getType()->isPointerTy() &&
               right.val->getType()->isIntegerTy())
        right.val = builder_->CreateIntToPtr(right.val, left.val->getType());
      else if (left.val->getType()->isIntegerTy() &&
               right.val->getType()->isIntegerTy())
        left.val = builder_->CreateZExtOrTrunc(left.val, right.val->getType());
    }
    if (op == "==")
      result = builder_->CreateICmpEQ(left.val, right.val);
    else if (op == "!=")
      result = builder_->CreateICmpNE(left.val, right.val);
    else if (op == "<")
      result = builder_->CreateICmpSLT(left.val, right.val);
    else if (op == ">")
      result = builder_->CreateICmpSGT(left.val, right.val);
    else if (op == "<=")
      result = builder_->CreateICmpSLE(left.val, right.val);
    else if (op == ">=")
      result = builder_->CreateICmpSGE(left.val, right.val);
  }
...
void Codegen::clear_transferred(TransferDomain domain) {
  ...
  for (auto it = transferred_values_.begin();
       it != transferred_values_.end();) {
...
Codegen::snapshot_transferred(TransferDomain domain) const {
...
void Codegen::restore_transferred(
    TransferDomain domain, const std::unordered_set<llvm::Value *> &snapshot) {
  clear_transferred(domain);
  for (auto *v : snapshot)
    mark_transferred(v, domain);
}

bool Codegen::is_cross_branch_droppable(llvm::Value *v,
                                        uint64_t pre_scope_block_ordinal) {
  if (llvm::isa<llvm::Argument>(v))
    return true;
  if (auto *inst = llvm::dyn_cast<llvm::Instruction>(v)) {
    auto it = transfer_block_ordinals_.find(inst->getParent());
    if (it == transfer_block_ordinals_.end())
      return false;
...
void Codegen::refresh_transfer_block_ordinals(llvm::Function *fn) {
  ...
  for (auto it = all_blocks.rbegin(); it != all_blocks.rend(); ++it) {
    if (transfer_block_ordinals_.count(*it))
      break;
...
void Codegen::transfer_branch_begin() {
  if (transfer_scope_stack_.empty())
    return;
  restore_transferred(TransferDomain::Seq,
                      transfer_scope_stack_.back().entry_snapshot);
}
...
void Codegen::transfer_scope_exit() {
  ...
  for (auto *v : any_transferred) {
    if (!is_cross_branch_droppable(v, scope.pre_scope_block_ordinal))
      continue;
    for (auto &b : scope.branches) {
      if (!b.exit_bb)
        continue;
      if (b.transfers.count(v))
        continue;
  ...
  for (auto &b : scope.branches) {
    if (!b.exit_bb)
      continue;
    for (auto *v : b.transfers)
      mark_transferred(v, TransferDomain::Seq);
  }
}
...
void Codegen::discard_pending_transfer_drops(llvm::Function *function) {
  pending_transfer_drops_.erase(
      std::remove_if(
          pending_transfer_drops_.begin(), pending_transfer_drops_.end(),
...
void Codegen::flush_pending_transfer_drops() {
  std::unordered_map<llvm::Function *, std::vector<PendingTransferDrop>>
      by_function;
  for (const auto &drop : pending_transfer_drops_) {
    if (!drop.value || !drop.exit_bb)
      continue;
...
static bool contains_raise_expr(AstNode *node) {
  ...
  if (ty == ast::AST_CASE_EXPR) {
    ...
    if (contains_raise_expr(e->expr))
      return true;
    for (auto *clause : e->clauses) {
      if (contains_raise_expr(clause->guard))
        return true;
      if (contains_raise_expr(clause->body))
        return true;
    }
    return false;
  }
  if (ty == ast::AST_DO_EXPR) {
    ...
    for (auto *step : e->steps)
      if (contains_raise_expr(step))
        return true;
    return false;
  }
  if (ty == ast::AST_TRY_CATCH_EXPR) {
    auto *e = static_cast<TryCatchExpr *>(node);
    if (contains_raise_expr(e->tryExpr))
      return true;
    for (auto *cp : e->catchExpr->patterns) {
      if (auto *bwg = std::get_if<PatternWithoutGuards *>(&cp->pattern))
        if (*bwg && contains_raise_expr((*bwg)->expr))
          return true;
      if (auto *guarded =
              std::get_if<std::vector<PatternWithGuards *>>(&cp->pattern)) {
        for (auto *pg : *guarded) {
          if (contains_raise_expr(pg->guard))
            return true;
          if (contains_raise_expr(pg->expr))
            return true;
        }
      }
    }
  ...
  if (ty == ast::AST_APPLY_EXPR) {
    ...
    for (auto &arg : e->args) {
      if (auto *expr = std::get_if<ExprNode *>(&arg))
        if (*expr && contains_raise_expr(*expr))
          return true;
    }
    return false;
  }
...
bool Codegen::has_escaping_use(AstNode *node, const std::string &name,
  ...
  if (ty == ast::AST_RECORD_INSTANCE_EXPR) {
    auto *e = static_cast<RecordInstanceExpr *>(node);
    for (auto &[field, value] : e->items)
      if (count_identifier_refs(value, name) > 0)
        return true;
    return false;
  }
  if (ty == ast::AST_RECORD_LITERAL_EXPR) {
  ...
  if (ty == ast::AST_LET_EXPR) {
    ...
    for (auto *a : e->aliases) {
      if (auto *va = dynamic_cast<ValueAlias *>(a)) {
        if (has_escaping_use(va->expr, name, false, context))
          return true;
        if (va->identifier->name->value == name)
          return false;
      } else if (auto *la = dynamic_cast<LambdaAlias *>(a)) {
        if (has_escaping_use(la->lambda, name, false, context))
          return true;
  ...
  if (ty == ast::AST_IMPORT_EXPR)
    return has_escaping_use(static_cast<ImportExpr *>(node)->expr, name,
                            is_return_position, context);
  ...
  if (ty == ast::AST_APPLY_EXPR) {
    ...
    if (auto *ec = dynamic_cast<ExprCall *>(e->call)) {
      if (has_escaping_use(ec->expr, name, false, context))
        return true;
    }
    auto cf_it = callee_name.empty() ? compiled_functions_.end()
    ...
    for (size_t ai = 0; ai < e->args.size(); ai++) {
      ...
      if (!arg)
        continue;
      if (has_escaping_use(arg, name, false, context))
        return true;
...
std::vector<bool>
Codegen::infer_borrowed_params(const std::string &function_name,
  ...
  if (!bwg)
    return borrowed;
  if (contains_raise_expr(bwg->expr))
    return borrowed;
...
std::unordered_set<std::string> Codegen::analyze_let_escaping(LetExpr *node) {
  ...
  for (auto &[name, _] : deferred_functions_)
    local_fns.insert(name);
  for (auto &[name, _] : compiled_functions_)
    local_fns.insert(name);
  ...
  std::function<void(AstNode *, bool)> check_escape = [&](AstNode *n,
                                                          bool ret_pos) {
    if (!n)
      return;
    if (n->get_type() == ast::AST_IDENTIFIER_EXPR) {
      auto *id = static_cast<IdentifierExpr *>(n);
      if (ret_pos && let_names.count(id->name->value))
        escaping.insert(id->name->value);
      return;
    }
    if (n->get_type() == ast::AST_LET_EXPR) {
      check_escape(static_cast<LetExpr *>(n)->expr, ret_pos);
      return;
    }
    if (n->get_type() == ast::AST_IF_EXPR) {
      auto *ie = static_cast<IfExpr *>(n);
      check_escape(ie->thenExpr, ret_pos);
      check_escape(ie->elseExpr, ret_pos);
      return;
    }
    if (n->get_type() == ast::AST_CASE_EXPR) {
      for (auto *clause : static_cast<CaseExpr *>(n)->clauses)
        check_escape(clause->body, ret_pos);
      return;
    }
    if (n->get_type() == ast::AST_DO_EXPR) {
    ...
    if (n->get_type() == ast::AST_FUNCTION_EXPR) {
      ...
      std::function<void(AstNode *)> walk = [&](AstNode *nd) {
        if (!nd)
          return;
        if (nd->get_type() == ast::AST_IDENTIFIER_EXPR) {
          if (let_names.count(static_cast<IdentifierExpr *>(nd)->name->value))
            escaping.insert(static_cast<IdentifierExpr *>(nd)->name->value);
          return;
        }
        if (nd->get_type() == ast::AST_LET_EXPR) {
          walk(static_cast<LetExpr *>(nd)->expr);
          return;
        }
        if (nd->get_type() == ast::AST_IF_EXPR) {
          auto *i = static_cast<IfExpr *>(nd);
          walk(i->condition);
          walk(i->thenExpr);
          walk(i->elseExpr);
          return;
        }
        if (nd->get_type() == ast::AST_CASE_EXPR) {
          auto *c = static_cast<CaseExpr *>(nd);
          walk(c->expr);
          for (auto *cl : c->clauses)
            walk(cl->body);
          return;
        }
        if (nd->get_type() == ast::AST_APPLY_EXPR) {
          auto *a = static_cast<ApplyExpr *>(nd);
          for (auto &arg : a->args) {
  ...
  for (auto &name : let_names) {
    ...
    for (auto *alias : node->aliases) {
      if (auto *va = dynamic_cast<ValueAlias *>(alias)) {
        ...
        if (ty == ast::AST_VALUES_SEQUENCE_EXPR || ty == ast::AST_TUPLE_EXPR ||
            ty == ast::AST_SET_EXPR || ty == ast::AST_DICT_EXPR ||
            ty == ast::AST_SEQ_GENERATOR_EXPR ||
            ty == ast::AST_SET_GENERATOR_EXPR ||
            ty == ast::AST_DICT_GENERATOR_EXPR || ty == ast::AST_FUNCTION_EXPR)
...
llvm::Value *
Codegen::setup_let_arena(const std::unordered_set<std::string> &non_escaping) {
  if (non_escaping.size() < 2)
    return nullptr;
  auto i64_ty = LType::getInt64Ty(*context_);
  return builder_->CreateCall(rt_.arena_create_,
                              {ConstantInt::get(i64_ty, ARENA_DEFAULT_SIZE)},
                              "arena");
}
...
static bool let_apply_is_spawn(AstNode *n) {
  if (!n || n->get_type() != ast::AST_APPLY_EXPR)
    return false;
...
void Codegen::codegen_let_aliases(
    LetExpr *node, llvm::Value *arena,
  ...
  for (size_t ai = 0; ai < node->aliases.size(); ai++) {
    ...
    if (auto *va = dynamic_cast<ValueAlias *>(alias)) {
      ...
      if (va->expr->get_type() == ast::AST_SEQ_GENERATOR_EXPR) {
        int refs = count_identifier_refs(node->expr, vname);
        if (refs == 1) {
          deferred_generators_[vname] =
              static_cast<SeqGeneratorExpr *>(va->expr);
          continue;
        }
      }
      ...
      if (tv) {
        ...
        if (debug_.enabled && debug_.scope && debug_.builder && tv.val) {
          auto *alloca =
              builder_->CreateAlloca(tv.val->getType(), nullptr, vname);
          builder_->CreateStore(tv.val, alloca);
          auto *di_var = debug_.builder->createAutoVariable(
              debug_.scope, vname, debug_.file, va->Range.Line,
              di_type_for(tv.type));
          ...
          for (size_t i = 0; i < tp->patterns.size(); i++) {
            ...
            if (tuple_ptr->getType()->isPointerTy()) {
              auto *gep = builder_->CreateGEP(
                  i64_local, tuple_ptr, {ConstantInt::get(i64_local, i + 2)},
                  "let_tuple_gep");
...
// Clean up scope: RC decrement bindings, destroy arena, clean deferred
// generators.
...
TypedValue Codegen::codegen_let(LetExpr *node) {
  ...
  const bool has_local_function = std::any_of(
      node->aliases.begin(), node->aliases.end(), [](AliasExpr *alias) {
        return dynamic_cast<LambdaAlias *>(alias) != nullptr;
      });
...
TypedValue Codegen::codegen_if(IfExpr *node) {
  ...
  if (!cond)
    return {};
  // Ensure condition is i1 for branch — closures return i64 even for bool
  // results
  if (cond.val->getType() != LType::getInt1Ty(*context_)) {
    if (cond.val->getType()->isIntegerTy())
      cond.val = builder_->CreateICmpNE(
          cond.val, ConstantInt::get(cond.val->getType(), 0));
...
TypedValue Codegen::codegen_identifier(IdentifierExpr *node) {
  ...
  // Materialize deferred generator if referenced outside fusion context
  {
    ...
    if (dg_it != deferred_generators_.end()) {
      ...
      if (tv)
        named_values_[node->name->value] = tv;
      return tv;
    }
  }
  ...
  if (it != named_values_.end()) {
    ...
    if (it->second.type == CType::FUNCTION) {
      ...
      if (cf_it != compiled_functions_.end()) {
        ...
        if (user_arity == 0 &&
            cf.extern_promise == ast::ExternPromiseKind::Sync &&
          ...
          if (ext_it != imports_.extern_functions.end()) {
            ...
            if (genfn_it != imports_.imported_sources.end()) {
              ...
              if (reparsed && !reparsed->functions.empty()) {
                ...
                if (local_cf.closure_env)
                  local_args.push_back(local_cf.closure_env);
                auto *local_call =
                    local_cf.fn->getReturnType()->isVoidTy()
                ...
                if (!local_cf.return_adt_name.empty())
                  result.adt_type_name = local_cf.return_adt_name;
                if (!local_cf.return_subtypes.empty())
                  result.subtypes = local_cf.return_subtypes;
                return result;
              }
            }
          }
      ...
      if (!it->second.val) {
        if (imports_.extern_functions.count(node->name->value)) {
          auto imported =
              materialize_imported_function_value(node->name->value);
          if (imported)
            return imported;
        }
      ...
      if (ext_it != imports_.extern_functions.end()) {
        ...
        if (genfn_it != imports_.imported_sources.end()) {
          ...
          if (reparsed && !reparsed->functions.empty()) {
            ...
            if (local_def_it != deferred_functions_.end()) {
              auto local_cf =
                  compile_function(node->name->value, local_def_it->second, {});
              iso.restore();
              std::vector<llvm::Value *> local_args;
              if (local_cf.closure_env)
                local_args.push_back(local_cf.closure_env);
  ...
  // Check if it's a non-zero-arity ADT constructor (used as a function
  // reference)
...
TypedValue Codegen::codegen_main_node(MainNode *node) {
...
TypedValue Codegen::codegen_do(DoExpr *node) {
  ...
  return last ? last
              : TypedValue{ConstantInt::get(LType::getInt64Ty(*context_), 0),
                           CType::INT};
}
...
TypedValue Codegen::codegen_try_catch(TryCatchExpr *node) {
  ...
  // slot 2 = SP. YonaRuntimeRaise restores those and branches to slot 1.
  //
  ...
  auto *fa_fn = llvm::Intrinsic::getOrInsertDeclaration(
      module_, llvm::Intrinsic::frameaddress, {ptr_ty});
  auto *ss_fn = llvm::Intrinsic::getOrInsertDeclaration(
      module_, llvm::Intrinsic::stacksave, {ptr_ty});
  auto *fa =
      builder_->CreateCall(fa_fn, {ConstantInt::get(i32_ty, 0)}, "sjlj.fp");
  builder_->CreateStore(fa, jmp_buf_ptr);
  auto *sp = builder_->CreateCall(ss_fn, {}, "sjlj.sp");
  auto *sp_slot = builder_->CreateGEP(
      ptr_ty, jmp_buf_ptr, ConstantInt::get(i32_ty, 2), "sjlj.sp.slot");
  builder_->CreateStore(sp, sp_slot);
  fn->addFnAttr(llvm::Attribute::NoInline);
#if defined(__aarch64__)
  // Match include/yona/Runtime/Platform/SjLj.h — Clang rejects
  // __builtin_setjmp here
  // and llvm.eh.sjlj.setjmp is a no-op on AArch64 Darwin.
  auto *sj_ty = llvm::FunctionType::get(i32_ty, {ptr_ty}, false);
  auto *sj_asm = llvm::InlineAsm::get(sj_ty,
                                      "str x29, [$1]\n\t"
                                      "adr x2, 1f\n\t"
                                      "str x2, [$1, #8]\n\t"
                                      "mov x2, sp\n\t"
  ...
  auto *sj_fn = llvm::Intrinsic::getOrInsertDeclaration(
      module_, llvm::Intrinsic::eh_sjlj_setjmp);
  ...
  if (!try_terminated) {
    ...
    if (!try_val)
      try_val = {ConstantInt::get(i64_ty, 0), CType::INT};
    builder_->CreateBr(merge_bb);
  ...
  if (node->catchExpr) {
    for (size_t ci = 0; ci < node->catchExpr->patterns.size(); ci++) {
      ...
      if (ci + 1 < node->catchExpr->patterns.size())
        next_bb = BasicBlock::Create(
            *context_, "catch.next." + std::to_string(ci + 1), fn);
      else
        next_bb = BasicBlock::Create(*context_, "catch.reraise", fn);

      auto *pat = cp->matchPattern;

      if (pat->get_type() == ast::AST_CONSTRUCTOR_PATTERN) {
      ...
      } else if (pat->get_type() == ast::AST_UNDERSCORE_PATTERN) {
        builder_->CreateBr(body_bb);
        builder_->SetInsertPoint(body_bb);
      } else if (pat->get_type() == ast::AST_PATTERN_VALUE) {
        auto *pv = static_cast<PatternValue *>(pat);
        if (auto *id = std::get_if<IdentifierExpr *>(&pv->expr)) {
          // Bind exception as a tuple (tag, payload)
          auto *st =
              StructType::get(*context_, {i64_ty, llvm_type(CType::STRING)});
  ...
  // Determine result type: use catch type if try body always raises
  // (terminated)
  CType result_type =
      try_end_bb
          ? try_val.type
  ...
  LType *result_llvm = try_end_bb ? try_val.val->getType()
                                  : (!catch_results.empty()
                                         ? catch_results[0].first.val->getType()
                                         : LType::getInt64Ty(*context_));
...
TypedValue Codegen::codegen_with(WithExpr *node) {
  ...
  auto resolved =
      resolve_trait_method("close", resource.type, adt_name, "Closeable");
  if (resolved.empty()) {
    ...
    report_error(node->Range,
                 "type '" + type_name +
                     "' does not implement Closeable (required by 'with')");
    return {};
  }
  ...
  if (close_cf == compiled_functions_.end()) {
    ...
    if (!close_fn) {
      ...
      close_fn = Function::Create(fn_type, Function::ExternalLinkage, resolved,
                                  module_);
    }
    CompiledFunction cf;
    cf.fn = close_fn;
  ...
  if (!body_val)
    body_val = {ConstantInt::get(i64_ty, 0), CType::INT};
