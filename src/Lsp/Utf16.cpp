...
std::uint32_t decode_utf8(std::string_view s, std::size_t &i) {
  ...
  if ((b0 & 0xE0) == 0xC0 && i + 1 < s.size()) {
    const auto cp =
        ((b0 & 0x1F) << 6) | (static_cast<unsigned char>(s[i + 1]) & 0x3F);
    i += 2;
    return cp;
  }
...
std::size_t utf16_width(std::uint32_t cp) { return cp > 0xFFFF ? 2 : 1; }
...
Range source_to_range(std::string_view utf8, const SourceRange &Source) {
  Range r;
  r.start = offset_to_position(utf8, Source.Offset);
  r.end = offset_to_position(utf8, Source.Offset + Source.Length);
  if (Source.Length == 0 && Source.Line > 0) {
    r.start.line = Source.Line - 1;
    r.start.character = Source.Column > 0 ? Source.Column - 1 : 0;
    r.end = r.start;
  }
  return r;
}
