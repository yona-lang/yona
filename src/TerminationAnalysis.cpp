...
AstNode *argument_node(const std::variant<ast::ExprNode *, ast::ValueExpr *> &argument) {
...
std::optional<DirectCall> direct_call(ast::ApplyExpr *outer) {
  ...
  while (current) {
    ...
    if (auto *target = dynamic_cast<ast::NameCall *>(current->call)) {
      DirectCall call;
      call.target = target;
      for (auto group_it = argument_groups.rbegin(); group_it != argument_groups.rend(); ++group_it)
        call.arguments.insert(call.arguments.end(), group_it->begin(), group_it->end());
      return call;
    }
    auto *expression_call = dynamic_cast<ast::ExprCall *>(current->call);
    current = expression_call ? dynamic_cast<ast::ApplyExpr *>(expression_call->expr) : nullptr;
  }
  return std::nullopt;
}
...
Relations unknown_relations(size_t arity) { return Relations(arity, Relation::Unknown); }
...
private:
  ...
  size_t register_function(FunctionExpr *function, const std::string &binding_name) {
    if (const auto found = function_indices_.find(function); found != function_indices_.end()) {
  ...
  LexicalBindings bind_function_group(const FunctionDefinitions &functions, const LexicalBindings &outer_bindings) {
  ...
  void discover_function_group(const FunctionDefinitions &functions, const LexicalBindings &bindings) {
    for (const auto &[_, function] : functions) {
      ...
      body_bindings[functions_[function_indices_.at(function)].name] = function_indices_.at(function);
  ...
  LexicalBindings register_function_group(const FunctionDefinitions &functions, const LexicalBindings &outer_bindings) {
    auto bindings = bind_function_group(functions, outer_bindings);
    discover_function_group(functions, bindings);
    return bindings;
  }

  static void shadow_pattern(ast::PatternNode *pattern, LexicalBindings &bindings) {
    if (!pattern)
      return;
  ...
  static std::optional<size_t> callable_alias(AstNode *expression, const LexicalBindings &bindings) {
  ...
  static void shadow_generator_binding(const ast::IdentifierOrUnderscore &binding, LexicalBindings &bindings) {
  ...
  void discover_generator(ast::CollectionExtractorExpr *extractor, AstNode *reducer, AstNode *step, const LexicalBindings &bindings) {
    ...
    } else if (auto *key_value = dynamic_cast<ast::KeyValueCollectionExtractorExpr *>(extractor)) {
  ...
  void discover(AstNode *node, const LexicalBindings &bindings = {}) {
    ...
    if (auto *main = dynamic_cast<ast::MainNode *>(node)) {
      ...
      const auto module_bindings = register_function_group(module_functions, bindings);
      ...
      for (auto *alias : let->aliases) {
        if (auto *value = dynamic_cast<ast::ValueAlias *>(alias)) {
          ...
          if (value->identifier && value->identifier->name)
            body_bindings[value->identifier->name->value] = callable_alias(value->expr, body_bindings);
      ...
      for (auto *clause : import->clauses)
        if (auto *functions = dynamic_cast<ast::FunctionsImport *>(clause))
          for (auto *alias : functions->aliases)
            if (alias && alias->name)
              imported_bindings[alias->alias ? alias->alias->value : alias->name->value] = std::nullopt;
      ...
      for (const auto &[_, value] : update->updates)
        discover(value, bindings);
    } else if (auto *sequence_generator = dynamic_cast<ast::SeqGeneratorExpr *>(node)) {
      discover_generator(sequence_generator->collectionExtractor, sequence_generator->reducerExpr, sequence_generator->stepExpression, bindings);
    } else if (auto *set_generator = dynamic_cast<ast::SetGeneratorExpr *>(node)) {
      discover_generator(set_generator->collectionExtractor, set_generator->reducerExpr, set_generator->stepExpression, bindings);
    } else if (auto *dict_generator = dynamic_cast<ast::DictGeneratorExpr *>(node)) {
      discover_generator(dict_generator->collectionExtractor, dict_generator->reducerExpr, dict_generator->stepExpression, bindings);
    ...
    } else if (auto *extractor = dynamic_cast<ast::ValueCollectionExtractorExpr *>(node)) {
      discover(extractor->collection, bindings);
      discover(extractor->condition, bindings);
    } else if (auto *extractor = dynamic_cast<ast::KeyValueCollectionExtractorExpr *>(node)) {
      discover(extractor->collection, bindings);
      discover(extractor->condition, bindings);
    ...
    } else if (auto *pattern = dynamic_cast<ast::PatternWithoutGuards *>(node)) {
      discover(pattern->expr, bindings);
      ...
      std::visit(
          [&](auto &body) {
            ...
            } else if constexpr (std::is_same_v<Body, std::vector<ast::PatternWithGuards *>>) {
              for (auto *guarded : body)
      ...
      if (lambda->name)
        register_function_group({{lambda->name->value, lambda->lambda}}, bindings);
  ...
  void bind_unknown(ast::PatternNode *pattern, Facts &facts, LexicalBindings &bindings, size_t arity) {
    bind_pattern(pattern, unknown_relations(arity), facts, bindings, false);
  }

  void bind_pattern(ast::PatternNode *pattern, const Relations &source, Facts &facts, LexicalBindings &bindings, bool structural_field) {
    if (!pattern)
      return;
    const Relations relation = structural_field ? strict_descendant(source) : source;
    if (auto name = pattern_identifier(pattern)) {
      facts[*name] = relation;
      bindings[*name] = std::nullopt;
    } else if (auto *constructor = dynamic_cast<ast::ConstructorPattern *>(pattern)) {
  ...
  Relations expression_relations(AstNode *expression, const Facts &facts, size_t arity) const {
    ...
    for (auto *body : function.node->bodies) {
      ...
      for (size_t index = 0; index < arity; ++index) {
        ...
        if (guarded)
          ...
          bind_pattern(function.node->patterns[index], parameter, facts, bindings, false);
      }
      // A function name remains callable even though parameter/value bindings shadow
      // other local function names.
  ...
  void add_call_edges(ast::ApplyExpr *apply, const DirectCall &call, const Facts &facts, const LexicalBindings &bindings, FunctionInfo &caller) {
    ...
    for (size_t position = 0; position < relations.size() && position < call.arguments.size(); ++position) {
      const auto argument = expression_relations(call.arguments[position], facts, caller.node->patterns.size());
      if (position < argument.size())
        relations[position] = argument[position];
    }
    caller.outgoing.push_back({&caller, &callee, apply->source_context, std::move(relations)});
  }
  ...
  void bind_generator_name(const ast::IdentifierOrUnderscore &binding, Facts &facts, LexicalBindings &bindings, size_t arity) {
    if (const auto *identifier = std::get_if<ast::IdentifierExpr *>(&binding); identifier && *identifier && (*identifier)->name) {
  ...
  void walk_generator(ast::CollectionExtractorExpr *extractor, AstNode *reducer, AstNode *step, const Facts &facts, const LexicalBindings &bindings,
                      FunctionInfo &function) {
    Facts body_facts = facts;
    LexicalBindings body_bindings = bindings;
    if (auto *value = dynamic_cast<ast::ValueCollectionExtractorExpr *>(extractor)) {
      walk(value->collection, facts, bindings, function);
      bind_generator_name(value->expr, body_facts, body_bindings, function.node->patterns.size());
      walk(value->condition, body_facts, body_bindings, function);
    } else if (auto *key_value = dynamic_cast<ast::KeyValueCollectionExtractorExpr *>(extractor)) {
      walk(key_value->collection, facts, bindings, function);
      bind_generator_name(key_value->keyExpr, body_facts, body_bindings, function.node->patterns.size());
      bind_generator_name(key_value->valueExpr, body_facts, body_bindings, function.node->patterns.size());
  ...
  void walk(AstNode *node, const Facts &facts, const LexicalBindings &bindings, FunctionInfo &function) {
    if (!node)
      return;
    if (auto *apply = dynamic_cast<ast::ApplyExpr *>(node)) {
      ...
      const Relations scrutinee = expression_relations(case_expr->expr, facts, function.node->patterns.size());
      for (auto *clause : case_expr->clauses)
        if (clause) {
          Facts arm_facts = facts;
          LexicalBindings arm_bindings = bindings;
          if (clause->guard)
            bind_unknown(clause->pattern, arm_facts, arm_bindings, function.node->patterns.size());
          else
            bind_pattern(clause->pattern, scrutinee, arm_facts, arm_bindings, false);
      ...
      for (auto *alias : let->aliases) {
        if (auto *value = dynamic_cast<ast::ValueAlias *>(alias)) {
          ...
          if (value->identifier && value->identifier->name) {
            ...
            body_facts[name] = expression_relations(value->expr, facts, function.node->patterns.size());
            body_bindings[name] = callable_alias(value->expr, body_bindings);
          }
        } else if (auto *pattern = dynamic_cast<ast::PatternAlias *>(alias)) {
          walk(pattern->expr, facts, body_bindings, function);
          bind_unknown(pattern->pattern, body_facts, body_bindings, function.node->patterns.size());
        }
      }
      walk(let->expr, body_facts, body_bindings, function);
    } else if (dynamic_cast<FunctionExpr *>(node)) {
      // Nested functions are collected and analysed independently. Walking their
      // bodies here would misattribute captured calls to the enclosing function.
    } else if (auto *import = dynamic_cast<ast::ImportExpr *>(node)) {
      ...
      for (auto *clause : import->clauses) {
        if (auto *functions = dynamic_cast<ast::FunctionsImport *>(clause))
          for (auto *alias : functions->aliases)
            if (alias && alias->name)
              imported[alias->alias ? alias->alias->value : alias->name->value] = std::nullopt;
      }
      walk(import->expr, facts, imported, function);
    } else if (auto *external = dynamic_cast<ast::ExternDeclExpr *>(node)) {
      Facts body_facts = facts;
      LexicalBindings body_bindings = bindings;
      body_facts[external->name] = unknown_relations(function.node->patterns.size());
      body_bindings[external->name] = std::nullopt;
      ...
      LexicalBindings body_bindings = bindings;
      if (with->name) {
        body_facts[with->name->value] = unknown_relations(function.node->patterns.size());
        body_bindings[with->name->value] = std::nullopt;
      }
      ...
      for (auto *clause : handle->clauses)
        if (clause) {
          ...
          const auto bind_clause_name = [&](const std::string &name) {
            if (name.empty())
              return;
            clause_facts[name] = unknown_relations(function.node->patterns.size());
            clause_bindings[name] = std::nullopt;
          };
          for (const auto &name : clause->arg_names)
      ...
      if (try_catch->catchExpr)
        for (auto *item : try_catch->catchExpr->patterns)
          if (item) {
            ...
            bind_unknown(item->matchPattern, catch_facts, catch_bindings, function.node->patterns.size());
            std::visit(
                [&](auto &body) {
                  using Body = std::remove_cvref_t<decltype(body)>;
                  if constexpr (std::is_same_v<Body, ast::PatternWithoutGuards *>) {
                    ...
                    for (auto *guarded : body)
                      if (guarded) {
                        walk(guarded->guard, catch_facts, catch_bindings, function);
                        walk(guarded->expr, catch_facts, catch_bindings, function);
                      }
                  }
                },
                item->pattern);
      ...
      walk_generator(sequence_generator->collectionExtractor, sequence_generator->reducerExpr, sequence_generator->stepExpression, facts, bindings,
                     function);
    } else if (auto *set_generator = dynamic_cast<ast::SetGeneratorExpr *>(node)) {
      walk_generator(set_generator->collectionExtractor, set_generator->reducerExpr, set_generator->stepExpression, facts, bindings, function);
    } else if (auto *dict_generator = dynamic_cast<ast::DictGeneratorExpr *>(node)) {
      walk_generator(dict_generator->collectionExtractor, dict_generator->reducerExpr, dict_generator->stepExpression, facts, bindings, function);
    } else if (auto *reducer = dynamic_cast<ast::DictGeneratorReducer *>(node)) {
      ...
      Facts clause_facts = facts;
      LexicalBindings clause_bindings = bindings;
      bind_unknown(clause->pattern, clause_facts, clause_bindings, function.node->patterns.size());
      walk(clause->guard, clause_facts, clause_bindings, function);
      walk(clause->body, clause_facts, clause_bindings, function);
    } else if (auto *pattern = dynamic_cast<ast::CatchPatternExpr *>(node)) {
      Facts catch_facts = facts;
      LexicalBindings catch_bindings = bindings;
      bind_unknown(pattern->matchPattern, catch_facts, catch_bindings, function.node->patterns.size());
      std::visit(
          [&](auto &body) {
            using Body = std::remove_cvref_t<decltype(body)>;
            if constexpr (std::is_same_v<Body, ast::PatternWithoutGuards *>) {
  ...
  void emit_failure(const Edge &edge, const std::string &component, const std::string &reason, std::unordered_set<const Edge *> &emitted) {
    if (!emitted.insert(&edge).second)
      return;
    result_.failures.push_back({edge.location, edge.caller->name, edge.callee->name, component, reason});
  }
  ...
  void prove_component(const std::vector<size_t> &component) {
    ...
    if (!recursive) {
      ...
      recursive = std::any_of(functions_[only].outgoing.begin(), functions_[only].outgoing.end(),
    ...
    const bool compatible =
        std::all_of(component.begin(), component.end(), [&](size_t index) { return functions_[index].node->patterns.size() == arity; });
    std::unordered_set<const Edge *> emitted;
    if (!compatible) {
      for (size_t index : component)
        for (const auto &edge : functions_[index].outgoing)
          if (members.contains(edge.callee->index))
            emit_failure(edge, component_text, "recursive component members have incompatible arity", emitted);
      return;
    }

    const std::string reason = component.size() > 1 ? "mutual recursion has no provable lexicographic structural descent"
    ...
    for (size_t start : component) {
      ...
      std::function<void(size_t, const Relations &)> enumerate = [&](size_t current, const Relations &prefix) {
