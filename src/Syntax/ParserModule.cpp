...
bool ParserImpl::try_consume_borrow_annotation() {
  if (!check(TokenType::YAT) || is_at_end())
    return false;
...
unique_ptr<ModuleDecl> ParserImpl::parse_module_internal() {
    ...
    if (match(TokenType::YEXPORT)) {
      ...
      if (match(TokenType::YTYPE)) {
        if (!check(TokenType::YIDENTIFIER)) {
          error(ParseError::Type::INVALID_SYNTAX,
                "Expected type name after 'export type'");
        } else {
          string type_name(advance().lexeme);
          bool opaque =
              check(TokenType::YIDENTIFIER) && current().lexeme == "opaque";
          if (opaque)
            advance();
          if (std::find(exported_types.begin(), exported_types.end(),
                        type_name) != exported_types.end()) {
            error(ParseError::Type::INVALID_SYNTAX,
                  "Type '" + type_name + "' is exported more than once");
          } else {
            exported_types.push_back(type_name);
            if (opaque)
              opaque_exported_types.push_back(type_name);
          }
        }
        ...
        if (!check(TokenType::YIDENTIFIER)) {
          error(ParseError::Type::INVALID_SYNTAX,
                "Expected trait name after 'export trait'");
        } else {
      ...
      if (auto adt = parse_adt_declaration()) {
        ...
        for (size_t ci = 0; ci < adt->variants.size(); ci++) {
          ...
          constructor_registry_[ctor->name] = {
              adt->name, static_cast<int>(ci),
              static_cast<int>(ctor->field_type_names.size()),
              ctor->field_names};
        }
      ...
      //   `native` — C returns YonaTask*; await via YonaRuntimeTaskAwait
      //   (GPU, etc.) neither — synchronous C call
      ...
      if (!check(TokenType::YIDENTIFIER)) {
        ...
        SourceRange eloc = current_location();
        ...
        if (etype) {
          auto *decl =
              new ExternDeclExpr(eloc, ename, *etype, nullptr, ext_promise);
  ...
  if (name->parts.size() == 1) {
    ...
    delete name.release();
    auto fqn =
        std::unique_ptr<FqnExpr>(new FqnExpr(source_ctx, nullopt, module_name));
    return make_unique<ModuleDecl>(
        start_loc, fqn.release(), exports, exported_types,
        opaque_exported_types, exported_traits, re_exports, functions,
        function_declarations, adt_declarations, trait_declarations,
        instance_declarations, extern_declarations);
  }
  ...
  return make_unique<ModuleDecl>(
      start_loc, fqn.release(), exports, exported_types, opaque_exported_types,
      exported_traits, re_exports, functions, function_declarations,
      adt_declarations, trait_declarations, instance_declarations,
      extern_declarations);
}
...
unique_ptr<PackageNameExpr> ParserImpl::parse_module_name() {
  ...
  do {
    ...
    auto token = advance();
    auto name_expr =
        make_unique<NameExpr>(token_location(token), string(token.lexeme));
...
void ParserImpl::parse_export_list(vector<string> &exports,
                                   vector<ReExport> &re_exports) {
...
unique_ptr<FunctionExpr> ParserImpl::parse_function() {
  ...
  skip_newlines();
  while (check(TokenType::YIDENTIFIER) && peek().lexeme == name &&
         !is_at_end()) {
  ...
  auto func = make_unique<FunctionExpr>(
      start_loc, name, patterns, function_bodies,
      type_signature ? std::optional<compiler::types::Type>(*type_signature)
                     : std::nullopt);
...
ParserImpl::parse_function_clause_with_patterns(const string &expected_name) {
  ...
  if (match(TokenType::YIF)) {
    do {
      ...
      if (guard_expr && body_expr) {
        bodies.push_back(make_unique<BodyWithGuards>(
            start_loc, guard_expr.release(), body_expr.release()));
      }
    ...
    if (body) {
      bodies.push_back(
          make_unique<BodyWithoutGuards>(start_loc, body.release()));
    }
  }
...
ParsedFunctionClause ParserImpl::parse_function_clause_patterns_and_body() {
  ...
  if (check(TokenType::YIF)) {
    for (size_t off = 1; off < 20 && !is_at_end(); off++) {
      ...
      if (t == TokenType::YARROW) {
        is_guard = true;
        break;
      }
      if (t == TokenType::YTHEN)
        break;
      if (t == TokenType::YEOF_TOKEN || t == TokenType::YEND)
        break;
    }
  }
  if (is_guard) {
    while (match(TokenType::YIF)) {
      ...
      if (!guard) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected guard expression after 'if'");
        break;
      }
      ...
      if (body) {
        bodies.push_back(make_unique<BodyWithGuards>(start_loc, guard.release(),
                                                     body.release()));
      }
    }
  } else {
...
ParserImpl::parse_function_clause(const string &expected_name) {
...
unique_ptr<AdtDeclNode> ParserImpl::parse_adt_declaration() {
  ...
  SourceRange loc = previous_location();
  ...
  do {
    ...
    std::function<FieldType()> parse_field_type = [&]() -> FieldType {
      ...
      const auto can_apply_type = [](const string &name) {
        static const std::unordered_set<string> nullary_types = {
            ...
            "Symbol",     "Byte",     "Char",       "Unit",
            "ByteArray",  "IntArray", "FloatArray", "Channel",
      ...
      if (check(TokenType::YLPAREN)) {
        ...
        if (check(TokenType::YRPAREN)) {
          ...
          bool head_is_ctor =
              !tname.empty() && isupper(tname[0]) && can_apply_type(tname);
          advance();
    ...
    variants.push_back(
        new AdtConstructor(loc, ctor_name, field_types, field_names));
  ...
  if (variants.empty()) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected at least one constructor in ADT declaration");
    return nullptr;
  }
...
unique_ptr<TraitDeclNode> ParserImpl::parse_trait_declaration() {
  ...
  vector<pair<string, string>> superclasses;
  {
    ...
    if (check(TokenType::YIDENTIFIER)) {
      ...
      while (lookahead < 20) {
        ...
        if (t == TokenType::YFAT_ARROW) {
          found_fat_arrow = true;
          break;
        }
        if (t == TokenType::YEND || t == TokenType::YEOF_TOKEN ||
            t == TokenType::YNEWLINE)
          break;
        lookahead++;
      }
    }
    ...
    if (found_fat_arrow) {
      ...
      if (!expect(TokenType::YFAT_ARROW,
  ...
  if (type_params.empty()) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected type parameter after trait name");
    return nullptr;
  }
  ...
  while (!check(TokenType::YEND) && !is_at_end()) {
    ...
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected method name in trait declaration");
      break;
    }
    ...
    if (match(TokenType::YCOLON)) {
      ...
      if (!type_sig) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected type signature after ':'");
        break;
      }

      methods.push_back({method_name, *type_sig, nullptr});
    } else {
      // Phase 3: This is a default method implementation (no ':' → it's a
      // function def) We need to find the method signature already declared and
      // attach the default impl Back up: the method_name was consumed, now
      // parse patterns and body Construct a FunctionExpr for the default
      // implementation

      vector<PatternNode *> patterns;
      vector<bool> param_borrow;
      while (!check(TokenType::YASSIGN) && !is_at_end() &&
             !check(TokenType::YNEWLINE)) {
        bool borrow = try_consume_borrow_annotation();
        auto pattern = parse_pattern();
        if (pattern) {
          patterns.push_back(pattern.release());
      ...
      expect(TokenType::YASSIGN,
             "Expected '=' in default method implementation");

      auto body = parse_expr();
      if (!body) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected body for default method");
        break;
      }
      ...
      auto default_fn = make_unique<FunctionExpr>(loc, method_name, patterns,
                                                  bodies, std::nullopt);
      ...
      if (!found) {
        ...
        // This allows trait declarations where default comes after sig in same
        // block
  ...
  return make_unique<TraitDeclNode>(loc, trait_name, std::move(type_params),
                                    std::move(methods),
                                    std::move(superclasses));
}
...
unique_ptr<InstanceDeclNode> ParserImpl::parse_instance_declaration() {
  ...
  vector<pair<string, string>> constraints;
  {
    ...
    if (found_fat_arrow) {
      ...
      do {
        ...
        if (!check(TokenType::YIDENTIFIER)) {
          error(ParseError::Type::INVALID_SYNTAX,
                "Expected type variable after constraint trait name");
          break;
        }
  ...
  if (!check(TokenType::YIDENTIFIER)) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected trait name after 'instance'");
    return nullptr;
  }
  string trait_name(advance().lexeme);
  ...
  if (match(TokenType::YLPAREN)) {
    ...
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected type name inside parentheses");
      return nullptr;
    }
    ...
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected type name after trait name in instance");
      return nullptr;
    }
    type_name = string(advance().lexeme);
    ...
    while (check(TokenType::YIDENTIFIER) && isupper(peek().lexeme[0])) {
      ...
      // (don't consume lowercase — those are type_params for parameterized
      // types)
  ...
  while (check(TokenType::YIDENTIFIER) || check(TokenType::YLPAREN)) {
    if (match(TokenType::YLPAREN)) {
      ...
      if (match(TokenType::YCOMMA)) {
        ...
        if (islower(head[0]) && find(type_params.begin(), type_params.end(),
                                     head) == type_params.end())
          type_params.push_back(head);
        if (islower(second[0]) && find(type_params.begin(), type_params.end(),
                                       second) == type_params.end())
          type_params.push_back(second);
        head = "Tuple";
      } else {
        while (check(TokenType::YIDENTIFIER) && islower(peek().lexeme[0])) {
          string parameter(advance().lexeme);
          if (find(type_params.begin(), type_params.end(), parameter) ==
              type_params.end())
  ...
  while (!check(TokenType::YEND) && !is_at_end()) {
    ...
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected method implementation in instance declaration");
      break;
    }
    ...
    if (current_ == method_start && !is_at_end()) {
      ...
      do {
        advance();
      } while (!check(TokenType::YNEWLINE) && !check(TokenType::YEND) &&
               !is_at_end());
    }
    skip_newlines();
  }

  if (!expect(TokenType::YEND,
              "Expected 'end' to close instance declaration")) {
    return nullptr;
  }

  return make_unique<InstanceDeclNode>(
      loc, trait_name, std::move(type_names), std::move(methods),
      std::move(constraints), std::move(type_params));
}

unique_ptr<RecordNode> ParserImpl::parse_record_definition() {
  SourceRange loc = previous().Range;

  if (!check(TokenType::YIDENTIFIER)) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected record name after 'record'");
    return nullptr;
  }
  ...
  if (!check(TokenType::YRPAREN)) {
    do {
      ...
      SourceRange field_loc = current_location();
      string field_name(current().lexeme);
      advance();

      auto field_id =
          new IdentifierExpr(field_loc, new NameExpr(field_loc, field_name));
      ...
      if (expect(TokenType::YCOLON, "Expected ':' after field name")) {
        ...
        field_type =
            new TypeDefinition(field_loc, type_node.release(), empty_types);
      }
