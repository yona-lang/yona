    ...
    if (check(TokenType::YIDENTIFIER)) {
      ...
      if (check(TokenType::YCOLON)) {
        ...
        if (base_type && check(TokenType::YPIPE)) {
          ...
          if (predicate) {
            if (!expect(TokenType::YRBRACE,
                        "Expected '}' after refinement type"))
              return nullptr;
...
unique_ptr<compiler::types::Type> ParserImpl::parse_primary_type() {
  if (check(TokenType::YIDENTIFIER)) {
    ...
    static const unordered_map<string, compiler::types::BuiltinType>
        builtin_types = {{"Bool", compiler::types::Bool},
    ...
    if (it != builtin_types.end()) {
      if (!type_arguments.empty()) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Built-in type '" + type_name +
                  "' does not accept a type argument");
        return nullptr;
      }
    ...
    if (type_name == "Seq" || type_name == "Set") {
      ...
      auto collection =
          make_shared<compiler::types::SingleItemCollectionType>();
...
unique_ptr<TypeNameNode> ParserImpl::parse_type_name() {
  ...
  if (check(TokenType::YIDENTIFIER)) {
    ...
    if (it != builtin_types.end()) {
      ...
      return make_unique<UserDefinedTypeNode>(loc,
                                              new NameExpr(loc, type_name));
    }
  }
...
ParserImpl::parse_refinement_predicate(const string &default_var) {
...
shared_ptr<compiler::types::RefinePredicate>
ParserImpl::parse_refinement_atom(const string &default_var) {
  ...
  if (check(TokenType::YIDENTIFIER) && string(current().lexeme) == "length") {
    ...
    if (match(TokenType::YGT))
      op = compiler::types::RefinePredicate::LengthGt;
    else {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected '>' after 'length' in refinement");
      return nullptr;
    }
    if (!check(TokenType::YINTEGER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected integer literal in refinement predicate");
      return nullptr;
    }
  ...
  compiler::types::RefinePredicate::Op op;
  if (match(TokenType::YGT))
    op = compiler::types::RefinePredicate::Gt;
  else if (match(TokenType::YLT))
    op = compiler::types::RefinePredicate::Lt;
  else if (match(TokenType::YGTE))
    op = compiler::types::RefinePredicate::Ge;
  else if (match(TokenType::YLTE))
    op = compiler::types::RefinePredicate::Le;
  else if (match(TokenType::YEQ))
    op = compiler::types::RefinePredicate::Eq;
  else if (match(TokenType::YNEQ))
    op = compiler::types::RefinePredicate::Ne;
  else {
    error(ParseError::Type::INVALID_SYNTAX,
