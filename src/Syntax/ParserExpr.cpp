//
// ParserExpr.cpp — Expression parsing (Pratt parser, control flow, generators,
// imports).
//
...
unique_ptr<ExprNode>
ParserImpl::parse_juxtaposition_apply(unique_ptr<ExprNode> func) {
  SourceRange loc = current_location();
  ...
  if (auto id = dynamic_cast<IdentifierExpr *>(func.get())) {
    auto name_copy = new NameExpr(id->Range, id->name->value);
    auto name_call = new NameCall(id->Range, name_copy);
    return make_unique<ApplyExpr>(loc, name_call, args);
  }

  if (auto call = dynamic_cast<CallExpr *>(func.get())) {
    func.release();
    return make_unique<ApplyExpr>(loc, call, args);
  }
...
unique_ptr<ExprNode> ParserImpl::parse_expr(Precedence min_prec) {
  ...
  if (recursion_depth > 50) {
    ...
    error(ParseError::Type::INVALID_SYNTAX,
          "Maximum recursion depth exceeded in parse_expr");
    return nullptr;
  }
  ...
  while (!is_at_end()) {
    if (++loop_count > 100) {
      std::cerr << "parse_expr: Infinite loop detected at position " << current_
                << "/" << tokens_.size() << ", token: "
                << (current_ < tokens_.size() ? peek().lexeme : "EOF")
                << ", type: "
                << (current_ < tokens_.size() ? static_cast<int>(peek().type)
                                              : -1)
                << std::endl;
      if (current_ > 0 && current_ - 1 < tokens_.size()) {
        std::cerr << "  Previous token: " << tokens_[current_ - 1].lexeme
                  << std::endl;
      }
      if (current_ + 1 < tokens_.size()) {
        std::cerr << "  Next token: " << tokens_[current_ + 1].lexeme
                  << std::endl;
      }
    ...
    if (check(TokenType::YLBRACE) && peek(1).type == TokenType::YIDENTIFIER &&
      ...
      auto sloc = left->Range;
      auto *id_expr = dynamic_cast<IdentifierExpr *>(left.get());
      if (!id_expr) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Field update target must be an identifier");
        return nullptr;
      }
      left.release();
      advance(); // consume {
    ...
    bool prev_closes_block =
        current_ > 0 && (tokens_[current_ - 1].type == TokenType::YEND ||
                         tokens_[current_ - 1].type == TokenType::YIN);
    if (!prev_closes_block && min_prec <= Precedence::CALL &&
        could_be_argument(peek().type)) {
...
unique_ptr<ExprNode> ParserImpl::parse_expr_until_in() {
  ...
  while (!is_at_end() && peek().type != TokenType::YIN) {
    if (++loop_count > 1000) {
      ...
      error(ParseError::Type::INVALID_SYNTAX,
            "Infinite loop in parse_expr_until_in");
      return nullptr;
    }
...
unique_ptr<ExprNode> ParserImpl::parse_prefix_expr() {
  ...
  // A package-qualified module call is unambiguous once the first
  // backslash is present: `Std\Gpu::available`. Parse the complete FQN
  // before the shared `::` token can be interpreted as sequence cons.
  if (check_fqn_start()) {
    auto fqn = parse_fqn();
    if (!fqn || !expect(TokenType::YCONS, "Expected '::' after module name")) {
      return nullptr;
    }
    ...
    variant<FqnExpr *, ExprNode *> module_fqn{in_place_index<0>, fqn.release()};
    return make_unique<ModuleCall>(loc, module_fqn, function_name.release());
  }
  ...
  case TokenType::YIDENTIFIER: {
    ...
    if (!id_name.empty() && isupper(id_name[0]) && check(TokenType::YLBRACE)) {
      ...
      if (ctor_it != constructor_registry_.end() &&
          !ctor_it->second.field_names.empty()) {
        advance(); // consume {
        ...
        while (!check(TokenType::YRBRACE) && !is_at_end()) {
          ...
          auto expr = parse_expr();
          if (expr)
            field_exprs[fname] = std::move(expr);
          if (!match(TokenType::YCOMMA))
            break;
          skip_newlines();
        }
  ...
  case TokenType::YNOT:
    advance();
    return make_unique<LogicalNotOpExpr>(
        loc, parse_expr(Precedence::UNARY).release());

  case TokenType::YBIT_NOT:
    advance();
    return make_unique<BinaryNotOpExpr>(
        loc, parse_expr(Precedence::UNARY).release());
  ...
  case TokenType::YMINUS: {
    ...
    return make_unique<SubtractExpr>(
        loc, make_unique<IntegerExpr>(loc, 0).release(), expr.release());
  }
  ...
  case TokenType::YLPAREN: {
    ...
    do {
      auto expr = parse_expr();
      if (expr)
        elements.push_back(expr.release());
  ...
  case TokenType::YLBRACKET: {
    ...
    if (match(TokenType::YPIPE)) {
      ...
      if (!body) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected expression in parallel comprehension");
        return nullptr;
      }
      if (!match(TokenType::YFOR)) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected 'for' in parallel comprehension [| expr for var = "
              "source ]");
        return nullptr;
      }
      ...
      auto result = make_unique<SeqGeneratorExpr>(loc, body.release(),
    ...
    if (match(TokenType::YFOR)) {
      ...
      if (!end) {
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected numeric expression after '..' in range");
        return nullptr;
      }
      ...
      if (match(TokenType::YDOTDOT)) {
        ...
        if (step_expr) {
          ...
          error(ParseError::Type::INVALID_SYNTAX,
                "Expected numeric expression for range step");
        }
      }

      expect(TokenType::YRBRACKET, "Expected ']' after range");
      auto range_expr = make_unique<RangeSequenceExpr>(loc, first.release(),
                                                       end.release(), step);
      return range_expr;
    } else {
      vector<ExprNode *> elements;
  ...
  case TokenType::YLBRACE: {
    ...
    if (check(TokenType::YIDENTIFIER)) {
      ...
      // Records have lowercase field names; uppercase would be a set of
      // constructors
      if (!first_name.empty() && islower(first_name[0])) {
        ...
        if (check(TokenType::YASSIGN)) {
          ...
          if (first_val)
            fields.push_back({first_name, first_val.release()});
          ...
          while (match(TokenType::YCOMMA)) {
            ...
            string fname(advance().lexeme);
            if (!expect(TokenType::YASSIGN, "Expected '=' after field name"))
              break;
            auto fval = parse_expr();
            if (fval)
              fields.push_back({fname, fval.release()});
          }
          skip_newlines();
  ...
  default:
    error(ParseError::Type::UNEXPECTED_TOKEN, "Unexpected token in expression");
    return nullptr;
  }
}

unique_ptr<ExprNode> ParserImpl::parse_infix_expr(unique_ptr<ExprNode> left,
                                                  Precedence prec) {
  SourceRange loc = current_location();
  auto op = advance();
...
#define BINARY_OP(TokenType, ExprType, OpStr)                                  \
  case TokenType: {                                                            \
    ...
    if (!right) {                                                              \
      error(ParseError::Type::INVALID_SYNTAX,                                  \
            "Expected expression after '" OpStr "'");                          \
      return nullptr;                                                          \
    }                                                                          \
...
#define BINARY_OP_RIGHT_ASSOC(TokenType, ExprType, OpStr)                      \
  ...
  case TokenType::YDOT: {
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX, "Expected field name after '.'");
      return left;
    }
    ...
    if (left->get_type() == AST_IDENTIFIER_EXPR) {
      ...
      auto name_expr =
          make_unique<NameExpr>(token_location(field_token), field_name);

      if (!id_name.empty() && isupper(id_name[0])) {
        left = make_unique<ModuleCall>(token_location(op),
                                       static_cast<ExprNode *>(left.release()),
                                       name_expr.release());
      } else {
        left = make_unique<FieldAccessExpr>(
            token_location(op), static_cast<IdentifierExpr *>(left.release()),
            name_expr.release());
      }
    } else {
      error(ParseError::Type::INVALID_SYNTAX,
  ...
  case TokenType::YLPAREN: {
    ...
    if (auto id = dynamic_cast<IdentifierExpr *>(left.get())) {
      auto name_copy = new NameExpr(id->Range, id->name->value);
      call_expr = new NameCall(id->Range, name_copy);
    } else if (auto call = dynamic_cast<CallExpr *>(left.get())) {
      left.release();
      call_expr = call;
    } else {
...
unique_ptr<ExprNode> ParserImpl::parse_if_expr(bool stop_at_in) {
  ...
  return make_unique<IfExpr>(loc, condition.release(), then_expr.release(),
                             else_expr.release());
}
...
unique_ptr<AliasExpr> ParserImpl::parse_alias() {
  ...
  while (check(TokenType::YAT) || check(TokenType::YIDENTIFIER) ||
         check(TokenType::YUNDERSCORE)) {
    ...
    if (!check(TokenType::YIDENTIFIER) && !check(TokenType::YUNDERSCORE)) {
      if (check(TokenType::YAT))
        ...
        error(ParseError::Type::INVALID_SYNTAX,
              "Expected parameter name after `@borrow`");
      return nullptr;
    }
  ...
  if (!params.empty()) {
    ...
    auto type_sig = type_annotation
                        ...
                        : std::nullopt;
    auto func_expr =
        new FunctionExpr(loc, name, patterns, bodies, std::move(type_sig));
...
unique_ptr<ExprNode> ParserImpl::parse_case_expr() {
  ...
  while (!check(TokenType::YEND) && !is_at_end()) {
    while (check(TokenType::YNEWLINE) || check(TokenType::YSEMICOLON))
      advance();
...
unique_ptr<ExprNode> ParserImpl::parse_do_expr() {
  ...
  while (!check(TokenType::YEND) && !is_at_end()) {
    ...
    if (check(TokenType::YIDENTIFIER) && peek(1).type == TokenType::YASSIGN) {
      ...
      SourceRange bind_loc = current_location();
      string name(advance().lexeme);
      advance(); // consume '='
      auto value = parse_expr();
      skip_newlines();
      ...
      while (!check(TokenType::YEND) && !is_at_end()) {
        ...
        if (check(TokenType::YIDENTIFIER) &&
          ...
          if (inner_do)
            rest.push_back(inner_do.release());
          break;
        }
        size_t pos_before = current_;
        auto expr = parse_expr();
        if (expr)
          rest.push_back(expr.release());
...
// Helper: parse remaining do-block entries when a binding is encountered
// mid-block
...
unique_ptr<ExprNode> ParserImpl::parse_try_expr() {
  ...
  while (!check(TokenType::YEND) && !is_at_end()) {
    ...
    if (!error_pattern) {
      error(ParseError::Type::INVALID_PATTERN,
            "Expected pattern in catch clause");
      break;
    }
    ...
    patterns.push_back(
        new CatchPatternExpr(current_location(), error_pattern.release(), var));
  ...
  if (patterns.empty()) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected catch clause after 'catch'");
    return nullptr;
  }
...
unique_ptr<ExprNode> ParserImpl::parse_raise_expr(bool stop_at_in) {
  ...
  if (!value) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected expression after 'raise'");
    return nullptr;
  }
...
unique_ptr<ExprNode> ParserImpl::parse_with_expr(bool stop_at_in) {
  ...
  if (!check(TokenType::YIDENTIFIER)) {
    error(ParseError::Type::INVALID_SYNTAX, "Expected identifier after 'with'");
    return nullptr;
  }

  string name(advance().lexeme);

  expect(TokenType::YASSIGN,
         "Expected '=' after identifier in with expression");
  ...
  auto name_expr = new NameExpr(loc, name);
  return make_unique<WithExpr>(loc, false, resource.release(), name_expr,
                               body.release());
}
...
unique_ptr<ExprNode>
ParserImpl::parse_sequence_generator(SourceRange loc,
                                     unique_ptr<ExprNode> expr) {
  auto extractor = parse_collection_extractor();

  expect(TokenType::YRBRACKET, "Expected ']' after sequence generator");

  return make_unique<SeqGeneratorExpr>(loc, expr.release(), extractor.release(),
                                       nullptr);
}

unique_ptr<ExprNode>
ParserImpl::parse_set_generator(SourceRange loc, unique_ptr<ExprNode> expr) {
  auto extractor = parse_collection_extractor();

  expect(TokenType::YRBRACE, "Expected '}' after set generator");

  return make_unique<SetGeneratorExpr>(loc, expr.release(), extractor.release(),
                                       nullptr);
}
...
unique_ptr<CollectionExtractorExpr> ParserImpl::parse_collection_extractor() {
  ...
  if (auto underscore = dynamic_cast<UnderscoreNode *>(pattern.get())) {
    pattern.release();
    return make_unique<ValueCollectionExtractorExpr>(loc, underscore, coll,
                                                     condition);
  } else if (auto pat_val = dynamic_cast<PatternValue *>(pattern.get())) {
    if (auto id = get_if<IdentifierExpr *>(&pat_val->expr)) {
      pattern.release();
      return make_unique<ValueCollectionExtractorExpr>(loc, *id, coll,
                                                       condition);
    }
  }
...
unique_ptr<ExprNode> ParserImpl::parse_dict(SourceRange loc,
  ...
  if (match(TokenType::YFOR)) {
    ...
    return make_unique<DictGeneratorExpr>(
        loc,
        new DictGeneratorReducer(loc, first_key.release(),
                                 first_value.release()),
        extractor.release(), nullptr);
  }
...
unique_ptr<ExprNode> ParserImpl::parse_extern_decl() {
  ...
  if (!check(TokenType::YIDENTIFIER)) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected function name after 'extern'");
    return nullptr;
  }
  string name(advance().lexeme);
  ...
  if (match(TokenType::YASSIGN)) {
    ...
    c_symbol = get<string>(advance().value);
    skip_newlines();
  }
  ...
  if (!body) {
    error(ParseError::Type::INVALID_SYNTAX,
          "Expected expression after 'in' in extern");
    return nullptr;
  }
...
unique_ptr<ExprNode> ParserImpl::parse_import_expr() {
  ...
  while (!check(TokenType::YIN) && !is_at_end()) {
    ...
    if (current_ == old_pos) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Import clause parsing made no progress");
      return nullptr;
    }
...
unique_ptr<ExprNode> ParserImpl::parse_perform_expr(bool stop_at_in) {
  ...
  vector<ExprNode *> args;
  while (!is_at_end() && !check(TokenType::YNEWLINE) &&
         !check(TokenType::YSEMICOLON) && !check(TokenType::YIN) &&
         !check(TokenType::YEND) && !check(TokenType::YWITH) &&
         !check(TokenType::YELSE) && !check(TokenType::YTHEN) &&
         !check(TokenType::YRBRACKET) && !check(TokenType::YRPAREN) &&
         !check(TokenType::YCOMMA)) {
...
unique_ptr<ExprNode> ParserImpl::parse_handle_expr() {
  ...
  while (!is_at_end() && !check(TokenType::YEND)) {
    ...
    SourceRange clause_loc = current_location();
    ...
    if (check(TokenType::YIDENTIFIER) && current().lexeme == "return") {
      ...
      if (!ret_body)
        break;
      clauses.push_back(
          new HandlerClause(clause_loc, binding, ret_body.release()));
      skip_newlines();
      continue;
    }
    ...
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected effect name or 'return' in handler clause");
      break;
    }
    string effect_name(advance().lexeme);
    if (!match(TokenType::YDOT)) {
      error(ParseError::Type::INVALID_SYNTAX,
            "Expected '.' after effect name in handler");
      break;
    }
    if (!check(TokenType::YIDENTIFIER)) {
      error(ParseError::Type::INVALID_SYNTAX,
    ...
    while (check(TokenType::YIDENTIFIER) || check(TokenType::YLPAREN)) {
      ...
      // Check if next token after this identifier is '->' (makes this the
      // resume name)
      string name(advance().lexeme);
      skip_newlines();
