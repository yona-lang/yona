//
// ParserPattern.cpp — Pattern parsing for case expressions and function
// parameters.
//
...
unique_ptr<PatternNode> ParserImpl::parse_pattern_or() {
  ...
  return make_unique<OrPattern>(peek().Range, std::move(patterns));
}
...
unique_ptr<PatternNode> ParserImpl::parse_pattern_primary() {
  ...
  if (check(TokenType::YINTEGER)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(literal_expr));
  }
  ...
  if (check(TokenType::YBYTE)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(byte_expr));
  }
  ...
  if (check(TokenType::YFLOAT)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(float_expr));
  }
  ...
  if (check(TokenType::YSTRING)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(string_expr));
  }
  ...
  if (check(TokenType::YTRUE)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(true_expr));
  }
  ...
  if (check(TokenType::YFALSE)) {
    ...
    return make_unique<PatternValue>(
        loc, static_cast<LiteralExpr<void *> *>(false_expr));
  }
  ...
  if (check(TokenType::YIDENTIFIER)) {
      ...
      if (check(TokenType::YLBRACE)) {
        ...
        while (!check(TokenType::YRBRACE) && !is_at_end()) {
          ...
          expect(TokenType::YASSIGN,
                 "Expected '=' after field name in pattern");
          auto pat = parse_pattern();
          if (pat)
            named_fields.push_back({new NameExpr(loc, fname), pat.release()});
          if (!match(TokenType::YCOMMA))
            break;
          skip_newlines();
        }
    ...
    const bool next_is_borrow_marker = check(TokenType::YAT) &&
                                       check_ahead(TokenType::YIDENTIFIER) &&
    ...
    if (!next_is_borrow_marker && match(TokenType::YAT)) {
      ...
      return make_unique<AsDataStructurePattern>(loc, id_expr,
                                                 pattern.release());
    }
