...
std::string gpu_std_module_fqn() { return std::string("Std") + PACKAGE_DELIMITER + "GPU"; }
...
bool inferred_mono_informative(typechecker::MonoTypePtr t) {
    if (!t) return false;
    if (t->tag == typechecker::MonoType::Var) return false;
    return true;
}

bool inferred_pretty_informative(const std::string& pp) {
    if (pp.empty()) return false;
    if (pp.size() == 1 && pp[0] >= 'a' && pp[0] <= 'z') return false;
    return true;
}
...
std::optional<std::pair<std::string, std::string>> resolve_std_gpu_apply(ApplyExpr* start) {
    ApplyExpr* cur = start;
    for (;;) {
        if (auto* nc = dynamic_cast<NameCall*>(cur->call)) {
            const std::string& n = nc->name->value;
            if (!is_gpu_kernel_name(n)) return std::nullopt;
...
static void walk_collection_extractor(CollectionExtractorExpr* ce,
    ...
    if (auto* v = dynamic_cast<ValueCollectionExtractorExpr*>(ce)) {
        if (v->collection) walk_ast(v->collection, on_apply);
        if (v->condition) walk_ast(v->condition, on_apply);
        return;
    }
    if (auto* kv = dynamic_cast<KeyValueCollectionExtractorExpr*>(ce)) {
        if (kv->collection) walk_ast(kv->collection, on_apply);
        if (kv->condition) walk_ast(kv->condition, on_apply);
    }
}
...
static void walk_ast(AstNode* node, const std::function<void(ApplyExpr*)>& on_apply) {
    ...
    case AST_CASE_EXPR: {
        ...
        for (auto* cl : ce->clauses) {
            if (cl->guard) walk_ast(cl->guard, on_apply);
    ...
    case AST_DO_EXPR: {
        ...
        for (auto* s : de->steps) walk_ast(s, on_apply);
        return;
    }
        ...
        if (tce->catchExpr) {
            for (auto* cp : tce->catchExpr->patterns) {
                if (auto* pwog = std::get_if<PatternWithoutGuards*>(&cp->pattern)) {
                    if (*pwog && (*pwog)->expr) walk_ast((*pwog)->expr, on_apply);
                }
            }
        }
        return;
    ...
    case AST_RECORD_LITERAL_EXPR: {
        ...
        for (auto& [_, e] : re->fields) walk_ast(e, on_apply);
        return;
    }
    case AST_RECORD_INSTANCE_EXPR: {
        auto* ri = static_cast<RecordInstanceExpr*>(node);
        for (auto& [_, e] : ri->items) walk_ast(e, on_apply);
        return;
    }
    ...
    case AST_FIELD_UPDATE_EXPR: {
        ...
        for (auto& [_, e] : fu->updates) walk_ast(e, on_apply);
        return;
    }
...
static void walk_apply_children(ApplyExpr* ae, const std::function<void(ApplyExpr*)>& on_apply) {
    if (auto* ec = dynamic_cast<ExprCall*>(ae->call)) walk_ast(ec->expr, on_apply);
...
static void walk_module_decl(ModuleDecl* mod, const std::function<void(ApplyExpr*)>& on_apply) {
    if (!mod) return;
    for (auto* fe : mod->functions) walk_ast(fe, on_apply);
    for (auto* ex : mod->extern_declarations) {
        if (ex->body) walk_ast(ex->body, on_apply);
    }
    for (auto* inst : mod->instance_declarations) {
        for (auto* m : inst->methods) walk_ast(m, on_apply);
    }
    for (auto* tr : mod->trait_declarations) {
        for (const auto& ms : tr->methods) {
            if (ms.default_impl) walk_ast(ms.default_impl, on_apply);
        }
    }
}
...
static std::function<void(ApplyExpr*)> make_accel_site_collector(std::vector<AccelSite>& sites,
...
static void write_accel_json(std::ostream& out, const std::vector<AccelSite>& sites,
                             std::string_view input_filename, std::string_view report_kind) {
    out << "{\"schema\":\"yona.accelerator_diag.v1\"";
...
bool typecheck_module_for_accelerator_report(ModuleDecl* mod, typechecker::TypeChecker& tc) {
    if (!mod) return true;
    for (auto* fe : mod->functions) {
        if (fe) tc.check(fe);
    }
    for (auto* ex : mod->extern_declarations) {
        if (ex && ex->body) tc.check(ex->body);
    }
    for (auto* inst : mod->instance_declarations) {
    ...
    for (auto* tr : mod->trait_declarations) {
        for (const auto& ms : tr->methods) {
            if (ms.default_impl) tc.check(ms.default_impl);
        }
    }
    if (tc.has_direct_errors()) return false;
    if (!tc.solve_constraints()) return false;
    if (tc.has_errors()) return false;
    return true;
}
