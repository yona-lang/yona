...
static parser::ParseResult parse_expr(parser::Parser& parser, const char* source) {
...
foldl (\a b -> a + b) 0 doubled
    ...
    auto matches = collect_transparent_matches(parsed.node.get());
    ...
    CHECK(std::string(matches[0].abi_symbol) == "yona_Std_GPU_raw__mapAdd");
    CHECK(matches[0].binding.find("IntArray") != std::string::npos);
}

TEST_CASE("transparent lowering matches IntArray filter greater-than") {
    parser::Parser parser;
    const char* source = R"YT(import map, filter, fromSeq, foldl from Std\IntArray in
...
foldl (\a b -> a + b) 0 (filter (\x -> x < 3) (fromSeq [1, 2, 3, 4, 5]))
    ...
    for (auto& m : matches) {
        if (m.kernel == AccelKernel::IntFilterLt) {
            saw_filter = true;
            CHECK(std::string(m.abi_symbol) == "yona_Std_GPU_raw__filterLessThan");
        }
    }
    CHECK(saw_filter);
}
...
foldl (\a b -> a + b) 0 (map (\x -> x + x * x) (fromSeq [1, 2, 3]))
    ...
    bool saw_reduce = false;
    for (auto& m : matches) {
        if (m.kernel == AccelKernel::IntMapMul || m.kernel == AccelKernel::IntMapAdd)
            saw_map = true;
        if (m.kernel == AccelKernel::IntReduceSum)
            saw_reduce = true;
    }
...
foldl (\a b -> a + b) 0 (map (\x -> x + 1) xs)
    ...
    CHECK(ir.find("yona_Std_GPU_raw__mapAdd") != std::string::npos);
    CHECK(ir.find("yona_Std_GPU_raw__reduceSum") != std::string::npos);
    CHECK(ir.find("yona_Std_IntArray__map") == std::string::npos);
}
...
foldl (\a b -> a + b) 0 (map (\x -> x * x) (fromSeq [1, 2, 3]))
    ...
    REQUIRE(mod);
    const std::string ir = codegen.emit_ir();
    CHECK(ir.find("yona_Std_GPU_raw__mapSquare") != std::string::npos);
    CHECK(ir.find("yona_Std_GPU_raw__reduceSum") != std::string::npos);
    CHECK(ir.find("yona_Std_IntArray__map") == std::string::npos);
}

TEST_CASE("transparent lowering leaves compound map on the IntArray closure path") {
    parser::Parser parser;
    DiagnosticEngine diag;
    typechecker::TypeChecker type_checker(diag);
    Codegen codegen("yona_program");
