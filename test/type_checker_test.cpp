...
TEST_SUITE("TypeChecker") {
  ...
  TEST_CASE("EffectSolver: joins are associative commutative and idempotent") {
    ...
    CHECK(summary.known_labels == std::vector<std::string>{"Log.log", "State.get"});
  ...
  TEST_CASE("EffectSolver: flexible equality binds an exact row") {
    ...
    CHECK(solver.summarize(flexible).known_labels == std::vector<std::string>{"State.get"});
    CHECK_FALSE(solver.summarize(flexible).is_open());
    CHECK(solver.equate(flexible, solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
  }
  ...
  TEST_CASE("EffectSolver: independent callback rows join without equality") {
    ...
    CHECK(solver.equate(first, solver.labels({"State.get"})) == EffectConstraintResult::Solved);
    CHECK(solver.equate(second, solver.labels({"Log.log"})) == EffectConstraintResult::Solved);
  ...
  TEST_CASE("EffectSolver: derived cycles use the least fixed point") {
    ...
    CHECK(solver.summarize(left).known_labels == std::vector<std::string>{"State.get"});
    CHECK(solver.summarize(right).known_labels == std::vector<std::string>{"State.get"});
  }
  ...
  TEST_CASE("EffectSolver: opaque leaves cannot be rebound or closed") {
    ...
    CHECK(solver.equate(opaque, solver.empty()) == EffectConstraintResult::Deferred);
  ...
  TEST_CASE("EffectSolver: masks apply to effects learned after construction") {
    ...
    const auto source = solver.join({solver.labels({"State.get", "Log.log"}), callback});
    ...
    CHECK(open_summary.tails[0].excluded_labels == std::vector<std::string>{"State.get"});

    CHECK(solver.equate(callback, solver.labels({"Net.post", "State.get"})) == EffectConstraintResult::Solved);
    const auto closed_summary = solver.summarize(masked);
    CHECK(closed_summary.known_labels == std::vector<std::string>{"Log.log", "Net.post"});
    CHECK_FALSE(closed_summary.is_open());
  }

  TEST_CASE("EffectSolver: graph templates clone independent variables and cells") {
    EffectSolver solver;
    const auto first = solver.flexible();
    const auto second = solver.flexible();
    const auto body = solver.derived();
    ...
    CHECK(solver.equate(one.variables.at(first_id), solver.labels({"State.get"})) == EffectConstraintResult::Solved);
    CHECK(solver.equate(one.variables.at(second_id), solver.labels({"Log.log"})) == EffectConstraintResult::Solved);
    CHECK(solver.summarize(one.root).known_labels == std::vector<std::string>{"Log.log", "State.get"});

    solver.add_label(one.root, "Net.post");
    CHECK(solver.summarize(one.root).known_labels == std::vector<std::string>{"Log.log", "Net.post", "State.get"});
  ...
  TEST_CASE("EffectSolver: deferred recursive equality rejects a contradictory binding") {
    EffectSolver solver;
    ...
    CHECK(solver.equate(row, solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
    ...
    CHECK(solver.equate(clone.variables.at(row_id), solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
    CHECK(solver.summarize(clone.root).is_open());
  }

  TEST_CASE("EffectSolver: derived equality rejects incompatible callback binding in either order") {
    SUBCASE("constraint before callback binding") {
      ...
      CHECK(solver.equate(body, solver.labels({"State.get"})) == EffectConstraintResult::Deferred);
      CHECK(solver.equate(callback, solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
      CHECK(solver.summarize(body).known_labels == std::vector<std::string>{"State.get"});
    ...
    SUBCASE("callback binding before constraint") {
      ...
      CHECK(solver.equate(callback, solver.labels({"Log.log"})) == EffectConstraintResult::Solved);
      CHECK(solver.equate(body, solver.labels({"State.get"})) == EffectConstraintResult::Conflict);
    }
  }

  TEST_CASE("EffectSolver: derived equality constraints remain live after cloning") {
    EffectSolver solver;
    const auto callback = solver.flexible();
    ...
    CHECK(solver.equate(clone.variables.at(callback_id), solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
    CHECK(solver.summarize(clone.root).known_labels == std::vector<std::string>{"State.get"});
    CHECK(solver.summarize(clone.root).is_open());
  }

  TEST_CASE("EffectSolver: derived equality permits compatible direct and nested growth") {
    ...
    SUBCASE("derived cell nested under a join can grow to equality") {
      ...
      CHECK(solver.equate(expression, expected) == EffectConstraintResult::Deferred);
      solver.add_label(body, "State.get");
      CHECK(solver.summarize(expression) == solver.summarize(expected));
    }
  }

  TEST_CASE("EffectSolver: derived mutations reject conflicts transactionally") {
    SUBCASE("local label") {
      EffectSolver solver;
      const auto body = solver.derived();
      const auto state = solver.labels({"State.get"});
      ...
      CHECK(solver.add_label(body, "Log.log") == EffectConstraintResult::Conflict);
      CHECK(solver.summarize(body) == solver.summarize(state));
    }
    ...
    SUBCASE("inclusion edge") {
      ...
      CHECK(solver.include(solver.labels({"Log.log"}), body) == EffectConstraintResult::Conflict);
      CHECK(solver.summarize(body) == solver.summarize(state));
    }
  }

  TEST_CASE("EffectSolver: derived equality constraints are validated jointly") {
    EffectSolver solver;
    const auto body = solver.derived();
    const auto state = solver.labels({"State.get"});
    ...
    const auto incompatible = solver.labels({"Log.log", "Net.post", "State.get"});
    CHECK(solver.equate(expression, incompatible) == EffectConstraintResult::Conflict);
    CHECK(solver.summarize(expression).known_labels == std::vector<std::string>{"Net.post", "State.get"});

    const auto graph = solver.freeze(body);
    const auto clone = solver.instantiate(graph);
    const auto cloned_expression = solver.join({clone.root, solver.labels({"Net.post"})});
    CHECK(solver.equate(cloned_expression, incompatible) == EffectConstraintResult::Conflict);
  }

  TEST_CASE("EffectSolver: nested equality saturates derived facts and templates") {
    EffectSolver solver;
    const auto body = solver.derived();
    const auto expression = solver.join({body, solver.labels({"Log.log"})});
    const auto expected = solver.labels({"Log.log", "State.get"});

    REQUIRE(solver.equate(expression, expected) == EffectConstraintResult::Deferred);
    CHECK(solver.summarize(body).known_labels == std::vector<std::string>{"State.get"});
  ...
  TEST_CASE("EffectSolver: ambiguous derived equality has no least solution") {
    ...
    REQUIRE(solver.add_label(first, "State.get") != EffectConstraintResult::Conflict);
    CHECK(solver.equate(either, state) != EffectConstraintResult::Conflict);
  }

  TEST_CASE("EffectSolver: symbolic rows carry deferred derived choices through templates") {
    EffectSolver solver;
    const auto body = solver.derived();
    const auto callback = solver.flexible();
    const auto callback_id = solver.variable_id(callback).value();
    ...
    REQUIRE(solver.equate(expression, solver.labels({"State.get"})) == EffectConstraintResult::Deferred);
    ...
    CHECK(clone_solver.equate(clone.variables.at(callback_id), clone_solver.empty()) == EffectConstraintResult::Deferred);
    CHECK(clone_solver.summarize(clone.root).known_labels == std::vector<std::string>{"State.get"});
  ...
  TEST_CASE("EffectSolver: opaque equality constraints are validated jointly") {
    ...
    REQUIRE(solver.equate(opaque, solver.labels({"State.get"})) == EffectConstraintResult::Deferred);
    CHECK(solver.equate(opaque, solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);

    const auto graph = solver.freeze(opaque);
    const auto clone = solver.instantiate(graph);
    REQUIRE(clone.variables.contains(opaque_id));
    CHECK(solver.equate(clone.root, solver.labels({"Log.log"})) == EffectConstraintResult::Conflict);
    CHECK(solver.summarize(clone.root).is_open());
  }
  ...
  TEST_CASE("EffectSolver: foreign handles are rejected") {
    ...
    CHECK_THROWS_AS(first_solver.join({local, foreign}), const std::invalid_argument &);
  }
...
TEST_CASE("TypeArena: effect owner address remains stable") {
  ...
  TEST_CASE("Unifier: arrow effect equality is delegated to the effect solver") {
    TypeArena arena;
    UnionFind uf;
    DiagnosticEngine diag;
    Unifier u(arena, uf, diag);
    ...
    auto *open = arena.make_arrow_with_effect(int_t, int_t, open_effect);
    auto *state = arena.make_arrow_with_effect(int_t, int_t, state_effect);

    REQUIRE(u.unify(open, state, SourceLocation::unknown()));
    CHECK(arena.effect_solver().summarize(open->arrow_effect) == EffectNormalForm{{"State.get"}, {}});
    CHECK(arena.effect_solver().summarize(state->arrow_effect) == EffectNormalForm{{"State.get"}, {}});
  }
...
TEST_CASE("Unifier: incompatible closed arrow effects fail") {
    ...
    Unifier u(arena, uf, diag);
    auto *int_t = arena.make_con(TyCon::Int);
    auto *state = arena.make_arrow_with_effect(int_t, int_t, arena.effect_solver().labels({"State.get"}));
    auto *log = arena.make_arrow_with_effect(int_t, int_t, arena.effect_solver().labels({"Log.log"}));

    CHECK_FALSE(u.unify(state, log, SourceLocation::unknown()));
  CHECK(diag.has_errors());
}

TEST_CASE("Unifier: equivalent modern and legacy closed effects unify") {
  TypeArena arena;
  UnionFind uf;
  DiagnosticEngine diag;
  Unifier u(arena, uf, diag);
  auto *int_t = arena.make_con(TyCon::Int);
  auto *modern = arena.make_arrow_with_effect(
      int_t, int_t, arena.effect_solver().labels({"State.get"}));
  auto *legacy = arena.make_arrow(
      int_t, int_t, {{"State.get", SourceLocation::unknown()}});

  CHECK(u.unify(modern, legacy, SourceLocation::unknown()));
  CHECK_FALSE(diag.has_errors());
}

TEST_CASE("Unifier: a modern open effect learns from a legacy closed effect") {
  TypeArena arena;
  UnionFind uf;
  DiagnosticEngine diag;
  Unifier u(arena, uf, diag);
  auto *int_t = arena.make_con(TyCon::Int);
  auto *modern = arena.make_arrow_with_effect(
      int_t, int_t, arena.effect_solver().flexible());
  auto *legacy = arena.make_arrow(
      int_t, int_t, {{"State.get", SourceLocation::unknown()}});

  REQUIRE(u.unify(modern, legacy, SourceLocation::unknown()));
  CHECK(arena.effect_solver().summarize(modern->arrow_effect) ==
  ...
  TEST_CASE("Unifier: open effect rest absorbs a closed set") {
    ...
    Unifier u(arena, uf, diag);
    auto *int_t = arena.make_con(TyCon::Int);
    auto *rest = arena.fresh_var(0);
    uf.add_var(rest->var_id, 0);
  ...
  TEST_CASE("pretty_print formats types correctly") {
    ...
    auto *eff = arena.make_arrow(arena.make_con(TyCon::Int), arena.make_con(TyCon::Bool), {{"Fs.read", SourceLocation::unknown()}});
  ...
  TEST_CASE("pretty_print normalizes a multi-source effect row") {
    ...
    const auto row = effects.join({effects.flexible(), effects.labels({"Log.log", "Fs.read"}), effects.flexible(), effects.labels({"Fs.read"})});
    auto *fn = arena.make_arrow_with_effect(arena.make_con(TyCon::Int), arena.make_con(TyCon::Bool), row);
...
TEST_SUITE("TypeChecker") { // reopen suite
  ...
  TEST_CASE("Inference: integer literal") { CHECK(check_expr_str("42") == "Int"); }

  TEST_CASE("Inference: float literal") { CHECK(check_expr_str("3.14") == "Float"); }

  TEST_CASE("Inference: string literal") { CHECK(check_expr_str("\"hello\"") == "String"); }
  ...
  TEST_CASE("Inference: let with function") { CHECK(check_expr_str("let f x = x + 1 in f 5") == "Int"); }
  ...
  TEST_CASE("Inference: sequence") { CHECK(check_expr_str("[1, 2, 3]") == "Seq Int"); }

  TEST_CASE("Inference: string concat") { CHECK(check_expr_str("\"a\" ++ \"b\"") == "String"); }
  ...
  TEST_CASE("Inference: nested let") { CHECK(check_expr_str("let x = 1 in let y = x + 1 in y") == "Int"); }

  TEST_CASE("Inference: lambda") { CHECK(check_expr_str("let f = \\x -> x + 1 in f 5") == "Int"); }
...
instance Merge (Dict key value)
    ...
    const auto &signature = result.value()->instance_declarations[0]->methods[0]->type_signature;
    REQUIRE(signature.has_value());
    const auto *first_arrow = std::get_if<std::shared_ptr<yona::compiler::types::FunctionType>>(&*signature);
    REQUIRE(first_arrow != nullptr);
    CHECK(std::holds_alternative<std::shared_ptr<yona::compiler::types::DictCollectionType>>((*first_arrow)->argumentType));
  }
...
run _ =
    ...
    for (const auto &record : diag.records()) {
      if (record.code == ErrorCode::E0100 && record.message.find("constructor pattern 'Box' has 2 fields") != std::string::npos &&
          record.message.find("declares 1 field of type (Int, Int)") != std::string::npos)
        ++mismatch_count;
      if (record.level == DiagLevel::Note && record.message.find("Box ((first, second))") != std::string::npos &&
...
run _ =
    ...
    for (const auto &record : diag.records()) {
      if (record.code == ErrorCode::E0100 && record.message.find("field 1 of constructor pattern 'Box'") != std::string::npos &&
          record.message.find("String") != std::string::npos && record.message.find("Int") != std::string::npos)
        ++mismatch_count;
      if (record.level == DiagLevel::Note && record.message.find("tuple field; match it with") != std::string::npos)
...
available () = true
  ...
  TEST_CASE("Inference: case with integer patterns") { CHECK(check_expr_str("case 42 of 0 -> \"zero\"; _ -> \"other\" end") == "String"); }

  TEST_CASE("Inference: case with identifier binding") { CHECK(check_expr_str("case 42 of x -> x + 1 end") == "Int"); }

  TEST_CASE("Inference: case with head-tail pattern") { CHECK(check_expr_str("case [1, 2, 3] of [h|t] -> h end") == "Int"); }

  TEST_CASE("Inference: case with empty seq pattern") { CHECK(check_expr_str("case [1, 2] of [] -> 0; [h|t] -> h end") == "Int"); }

  TEST_CASE("Inference: case with tuple pattern") { CHECK(check_expr_str("case (1, \"hello\") of (a, b) -> a end") == "Int"); }
  ...
  TEST_CASE("Inference: cons operator") { CHECK(check_expr_str("1 :: [2, 3]") == "Seq Int"); }

  TEST_CASE("Inference: recursive sum via case") {
    CHECK(check_expr_str("let foldl fn acc seq = case seq of [] -> acc; [h|t] -> foldl fn (fn acc h) t end in "
  ...
  TEST_CASE("Type error: adding string to int") { CHECK(check_has_error("1 + \"hello\"")); }
  ...
  TEST_CASE("Type error: if condition not bool") { CHECK(check_has_error("if 42 then 1 else 2")); }

  TEST_CASE("Type error: if branches mismatch") { CHECK(check_has_error("if true then 1 else \"hello\"")); }

  TEST_CASE("Type error: undefined variable") { CHECK(check_has_error("x + 1")); }

  TEST_CASE("Type error: heterogeneous sequence") { CHECK(check_has_error("[1, \"hello\", 3]")); }
  ...
  TEST_CASE("Type error: typo in let binding suggests correct name") { CHECK(check_has_error("let myVariable = 42 in myVarible + 1")); }

  TEST_CASE("No error: valid arithmetic") { CHECK(!check_has_error("1 + 2 + 3")); }

  TEST_CASE("No error: valid let binding") { CHECK(!check_has_error("let x = 42 in x + 1")); }

  TEST_CASE("No error: valid case expression") { CHECK(!check_has_error("case 1 of 0 -> 10; _ -> 20 end")); }
...
TEST_SUITE("TypeChecker") { // reopen
  ...
  TEST_CASE("ADT: Some constructor applied") { CHECK(check_with_adt("Some 42") == "Option Int"); }
  ...
  TEST_CASE("ADT: constructor used in let binding") { CHECK(check_with_adt("let opt = Some 42 in opt") == "Option Int"); }

  TEST_CASE("ADT: Some applied to string") { CHECK(check_with_adt("Some \"hello\"") == "Option String"); }
...
TEST_SUITE("TypeChecker") { // reopen
  ...
  TEST_CASE("Trait: abs applied to Int") { CHECK(check_with_traits("abs 42") == "Int"); }

  TEST_CASE("Trait: abs applied to Float") { CHECK(check_with_traits("abs 3.14") == "Float"); }

  TEST_CASE("Trait error: abs applied to String") { CHECK(check_with_traits("abs \"hello\"", true) == "ERROR"); }

  TEST_CASE("Trait: no error on valid usage") { CHECK(check_with_traits("abs 42", false) != "ERROR"); }
  ...
  TEST_CASE("Equality operator requires an Eq instance") {
    ...
    size_t missing_eq = 0;
    for (const auto &record : diag.records()) {
      if (record.code == yona::compiler::ErrorCode::E0105 || record.code == yona::compiler::ErrorCode::E0106) {
  ...
  TEST_CASE("Lifted instance constraints are solved for concrete element types") {
    ...
    checker.register_trait_method("Eq", "eq",
                                  arena.make_arrow(element, arena.make_arrow(element, arena.make_con(yona::compiler::typechecker::TyCon::Bool))));
    checker.register_instance("Eq", "Seq", {"element"}, {{"Eq", "element"}}, {"Seq"});
  ...
  TEST_CASE("Effect: perform with registered effect returns correct type") {
    ...
    auto *resume_call = new yona::ast::ApplyExpr(sc, new yona::ast::NameCall(sc, new yona::ast::NameExpr(sc, "resume")), resume_args);
...
with
  ...
  TEST_CASE("Effect: HOF apply of perform lambda is E0202") {
    CHECK(check_source_has_direct_errors(R"(let apply = \f x -> f x in apply (\() -> perform Fs.read "/etc/shadow") ())"));
  }
...
handle f "ok" with
  ...
  TEST_CASE("Effect row: handle subtracts covered ops from latent row") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("(\\x -> handle perform State.get () with State.get () resume -> resume 1 end)");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node != nullptr);
    auto *t = checker.check(result.node.get());
    REQUIRE(t != nullptr);
  ...
  TEST_CASE("Effect row: partial handler leaves remaining ops") {
    ...
    std::istringstream stream("(\\x -> handle\n"
  ...
  TEST_CASE("Effect row: applying effectful function inside matching handle is OK") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let f = (\\x -> perform State.get ()) in\n"
  ...
  TEST_CASE("Effect row: HOF apply threads an open rest") {
    ...
    // (a -> !{|r} b) -> a -> !{|r} b — both arrows retain a symbolic
    // solver projection instead of an obsolete single legacy row tail.
  ...
  TEST_CASE("Effect row: applying a returned effectful function is E0202") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let mk = (\\x -> (\\y -> perform State.get ())) in mk 0 1");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node != nullptr);
    checker.check(result.node.get());
    CHECK(checker.has_errors());
  ...
  TEST_CASE("Effect row: recursive function captures body effects, not an empty row") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let loop n = if n <= 0 then 0 else (perform State.get ()) + loop (n - 1) in loop");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node != nullptr);
    auto *t = checker.check(result.node.get());
    REQUIRE(t != nullptr);
  ...
  TEST_CASE("Effect row: recursive effectful function applied at top level is E0202") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let loop n = if n <= 0 then 0 else (perform State.get ()) + loop (n - 1) in loop 3");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node != nullptr);
    checker.check(result.node.get());
    CHECK(checker.has_errors());
  ...
  TEST_CASE("Effect row: recursive effectful function inside handle is OK") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let loop n = if n <= 0 then 0 else (perform State.get ()) + loop (n - 1) in\n"
  ...
  TEST_CASE("Effect row: pure mutually recursive module functions close their rows") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
...
odd n = even n
    ...
    CHECK(checker.is_effect_free(checker.type_of(result.value()->functions[0])));
    CHECK(checker.is_effect_free(checker.type_of(result.value()->functions[1])));
  }

  TEST_CASE("Effect row: pure mutually recursive local functions close their rows") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
    yona::parser::Parser parser;
    auto result = parser.parse_module(R"(
...
odd k = case k of
  ...
  TEST_CASE("Effect row: mutually recursive local functions retain callback effects") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
    yona::parser::Parser parser;
    auto result = parser.parse_module(R"(
...
in left n
    ...
    REQUIRE_FALSE(diag.has_errors());
    REQUIRE(result.value()->functions.size() == 1);
    CHECK_FALSE(checker.is_effect_free(checker.type_of(result.value()->functions[0])));
    CHECK(checker.effect_row_info(
        checker.type_of(result.value()->functions[0])).open_rest);
  }

  TEST_CASE("Effect row: mutually recursive module functions retain concrete effects") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
...
second n = (perform State.get ()) + first n
    ...
    REQUIRE(result.value()->functions.size() == 2);
    CHECK_FALSE(checker.is_effect_free(checker.type_of(result.value()->functions[0])));
    CHECK_FALSE(checker.is_effect_free(checker.type_of(result.value()->functions[1])));
    CHECK(checker.closed_effect_ops(checker.type_of(result.value()->functions[0])) == std::vector<std::string>{"State.get"});
    CHECK(checker.closed_effect_ops(checker.type_of(result.value()->functions[1])) == std::vector<std::string>{"State.get"});
  }

  TEST_CASE("Effect row: module forwarding preserves a higher-order parameter rest") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
...
apply f x = f x
  ...
  TEST_CASE("Effect row: structurally recursive HOF preserves its parameter rest") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
...
app f n = case n of Zero -> f n; Succ rest -> app f rest end
  ...
  TEST_CASE("Effect row: recursive HOF preserves its parameter rest in either arm order") {
...
app f n = case n of Succ rest -> app f rest; Zero -> f n end
    ...
    for (const auto &[order, source] : modules) {
      ...
      yona::compiler::typechecker::TypeChecker checker(diag);
      yona::parser::Parser parser;
      auto result = parser.parse_module(source, order + ".yona");
      REQUIRE(result.has_value());
...
result = use get log 0
  ...
  TEST_CASE("Effect row: independent HOF callbacks retain all concrete effects") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
    auto &arena = checker.arena();
    auto *int_type = arena.make_con(TyCon::Int);
...
result = use get log 0
    ...
    CHECK(checker.closed_effect_ops(checker.type_of(result.value()->functions[3])) == std::vector<std::string>{"Log.log", "State.get"});
  }

  TEST_CASE("Effect row: three independent callbacks form a source-order-invariant union") {
    ...
    for (const auto& [name, use_body] : bodies) {
      ...
      auto result = parser.parse_module(
          "module Test\\ThreeCallback" + name + "\n\n"
          "export result\n\n" + use_body + "\n"
          "get x = do perform State.get (); x end\n"
          "log x = do perform Log.log (); x end\n"
  ...
  TEST_CASE("Effect row: a handler symbolically masks a callback effect after instantiation") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
    auto* int_type = checker.arena().make_con(TyCon::Int);
    checker.register_effect("State", "s", {{"get", {}, int_type}});
...
run n = handle use get n with State.get () resume -> resume 0 end
    ...
    CHECK(checker.closed_effect_ops(checker.type_of(result.value()->functions[2])).empty());
    CHECK(checker.is_effect_free(checker.type_of(result.value()->functions[2])));
  }

  TEST_CASE("Effect scheme: normalized interface graph preserves independent callback projections") {
    yona::compiler::DiagnosticEngine diag;
    yona::compiler::typechecker::TypeChecker checker(diag);
    yona::parser::Parser parser;
    auto result = parser.parse_module(R"(
...
use f g n = (f n, g n)
    ...
    const auto encoded = checker.serialize_effect_scheme(checker.zonk(original));
    CHECK(encoded.starts_with("v2;$#"));
    CHECK(encoded == checker.serialize_effect_scheme(checker.zonk(original)));

    auto* restored = checker.apply_effect_scheme(checker.zonk(original), encoded);
    ...
    CHECK(restored_summary.tails[0].variable != restored_summary.tails[1].variable);
  }

  TEST_CASE("Effect row: module forwarding preserves an imported open rest") {
    namespace fs = std::filesystem;
    const auto dir = fs::temp_directory_path() / "yona_module_forward_open_effect";
    fs::create_directories(dir / "Test");
    {
      std::ofstream out(dir / "Test" / "Open.yonai");
      out << "FN yona_Test_Open__run 1 INT -> INT effects |\n";
    }
...
both = (forward get 0, forward log 1)
  ...
  TEST_CASE("Module dependency SCCs include function values and respect binders") {
    yona::parser::Parser parser;
    auto result = parser.parse_module(R"(
...
consumer x = saved x
  ...
  TEST_CASE("Effect row: recursive HOF still threads the parameter rest") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let app f n = if n <= 0 then f n else app f (n - 1) in\n"
                              "let g = (\\y -> perform State.get ()) in\n"
                              "app g 3");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node != nullptr);
...
TEST_SUITE("RefinementChecker") {
  ...
  TEST_CASE("RefinePredicate: to_string") {
    ...
    auto p3 =
        RefinePredicate::make_and(RefinePredicate::make_cmp(RefinePredicate::Gt, "n", 0), RefinePredicate::make_cmp(RefinePredicate::Lt, "n", 65536));
  ...
  TEST_CASE("RefinementChecker: module function head on unknown seq is E0500") {
    ...
    yona::parser::Parser parser;
    auto result = parser.parse_module("module Test\\RefineMod\n\nexport bad\n\nbad xs = head xs\n", "RefineMod.yona");
    REQUIRE(result.has_value());
...
TEST_SUITE("LinearityChecker") {
  ...
  static void register_linear_adt(yona::compiler::typechecker::TypeChecker & tc) { tc.register_adt("Linear", {"a"}, {{"Linear", 1}}); }

  static bool diag_has_code(const yona::compiler::DiagnosticEngine &diag, yona::compiler::ErrorCode code) {
    for (auto &rec : diag.records()) {
  ...
  TEST_CASE("LinearityChecker: use after consume is error") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let makeHandle x = Linear x, conn = makeHandle 0, conn2 = conn, conn3 = conn in conn3");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node);
    tc.check(result.node.get());
    REQUIRE(!tc.has_direct_errors());
  ...
  TEST_CASE("LinearityChecker: transfer via alias is OK") {
    ...
    yona::parser::Parser parser;
    std::istringstream stream("let makeHandle x = Linear x, conn = makeHandle 0, conn2 = conn in conn2");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node);
    tc.check(result.node.get());
    REQUIRE(!tc.has_direct_errors());
  ...
  TEST_CASE("LinearityChecker: imported openFile creates obligation") {
    CHECK(imported_linear_leaks("import openFile from Std\\File in let h = openFile \"f\" Read in h"));
  }
  ...
  static std::vector<DiagnosticEngine::Record> concurrency_diagnostics(const std::string &source) {
...
let message = "immutable" in spawn (\_ -> if message == "immutable" then 1 else 0)
    ...
    CHECK(std::none_of(records.begin(), records.end(), [](const auto &record) { return record.level == DiagLevel::Error; }));
  }
...
let bytes = alloc 4 in spawn (\_ -> bytes)
    ...
    CHECK(std::count_if(records.begin(), records.end(), [](const auto &record) {
            return record.level == DiagLevel::Error &&
                   (record.message.find("Shareable ByteArray") != std::string::npos || record.message.find("Send ByteArray") != std::string::npos);
          }) >= 1);
    CHECK(std::any_of(records.begin(), records.end(), [](const auto &record) {
      return record.level == DiagLevel::Note && record.message.find("mutable native arrays require a snapshot") != std::string::npos;
    }));
  }
...
import channel, send from Std\Channel in case channel 1 of
    ...
    CHECK(std::none_of(accepted.begin(), accepted.end(), [](const auto &record) { return record.level == DiagLevel::Error; }));
...
case channel 1 of
    ...
    CHECK(std::none_of(moved_array.begin(), moved_array.end(), [](const auto &record) { return record.level == DiagLevel::Error; }));
...
case channel 1 of
    ...
    CHECK(std::any_of(rejected.begin(), rejected.end(), [](const auto &record) {
      return record.level == DiagLevel::Error && record.message.find("Send") != std::string::npos && record.message.find("->") != std::string::npos;
    }));
  }
...
type MutablePacket = MutablePacket ByteArray deriving Send, Shareable
    ...
    REQUIRE(parsed.has_value());
    checker.check_module(parsed.value().get());
    CHECK(std::any_of(diagnostics.records().begin(), diagnostics.records().end(), [](const auto &record) {
      return record.level == DiagLevel::Error && record.message.find("cannot derive Shareable") != std::string::npos;
    }));
  }
  ...
  TEST_CASE("TypeChecker: imported Linear payloads remain distinct") {
    ...
    yona::parser::Parser parser;
    auto parsed = parser.parse_expression("import open, needProcess from Test\\Resource in needProcess (open ())", "<test>");
    REQUIRE(parsed.has_value());
    DiagnosticEngine diag;
    TypeChecker checker(diag);
    checker.add_module_path(dir.string());
  ...
  TEST_CASE("LinearityChecker: wildcard openFile creates obligation") {
    CHECK(imported_linear_leaks("import Std\\File in let h = openFile \"f\" Read in h"));
  }

  TEST_CASE("LinearityChecker: imported channel tuple is tracked") {
    CHECK(imported_linear_leaks("import channel from Std\\Channel in let (a, b) = channel 16 in 0"));
  }

  TEST_CASE("LinearityChecker: imported non-linear File function has no leak") {
    CHECK(!imported_linear_leaks("import exists from Std\\File in exists \"f\""));
  }
  ...
  TEST_CASE("LinearityChecker: module function use-after-consume is E0600") {
    ...
    yona::parser::Parser parser;
    auto result = parser.parse_module("module Test\\LinMod\n\nexport bad\n\n"
                                      "bad x =\n"
...
TEST_SUITE("BorrowParam") {
  ...
  TEST_CASE("consecutive @borrow parameters remain distinct identifiers") {
    ...
    yona::compiler::typechecker::TypeChecker tc(diag);
    yona::parser::Parser parser;
    std::istringstream stream("let same @borrow left @borrow right = left == right in same 1 1");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node);
    tc.check(result.node.get());
    CHECK(!tc.has_direct_errors());
  ...
  TEST_CASE("@borrow rejected on non-identifier pattern") {
    ...
    yona::compiler::typechecker::TypeChecker tc(diag);
    yona::parser::Parser parser;
    std::istringstream stream(R"(let g = \ @borrow (a, b) -> a + b in g (1, 2))");
    auto result = parser.parse_input(stream);
    REQUIRE(result.node);
    tc.check(result.node.get());
    CHECK(tc.has_direct_errors());
