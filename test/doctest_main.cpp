...
class JunitFileListener : public doctest::IReporter {
  ...
  void flush() const {
    ...
    for (const auto &s : suites) {
      out << "  <testsuite name=\"" << xml_escape(s.name.c_str()) << "\" tests=\"" << s.tests << "\" failures=\"" << s.failures << "\" errors=\""
          << s.errors << "\" skipped=\"" << s.skipped << "\">\n";
      for (const auto *c : s.cases) {
        out << "    <testcase classname=\"" << xml_escape(c->suite.c_str()) << "\" name=\"" << xml_escape(c->name.c_str()) << "\" time=\""
            << c->seconds << "\"";
        if (c->status == "passed") {
          out << "/>\n";
          continue;
        ...
        if (c->status == "skipped")
          ...
          out << "      <error message=\"" << xml_escape(c->message.empty() ? "incomplete" : c->message.c_str()) << "\"/>\n";
        out << "    </testcase>\n";
      }
      out << "  </testsuite>\n";
    }
...
public:
  ...
  void test_case_start(const doctest::TestCaseData &in) override { start_case(in); }
  ...
  void test_case_end(const doctest::CurrentTestCaseStats &st) override {
    ...
    } else if (pending_error_ || (st.failure_flags & doctest::TestCaseFailureReason::Exception)) {
      c.status = "error";
      if (c.message.empty())
        c.message = pending_message_.empty() ? "exception" : pending_message_;
    } else if (!st.testCaseSuccess || st.numAssertsFailedCurrentTest > 0 || (st.failure_flags & doctest::TestCaseFailureReason::AssertFailure)) {
      c.status = "failed";
      if (c.message.empty())
  ...
  void subcase_start(const doctest::SubcaseSignature &in) override { subcases_.push_back(in.m_name.c_str() ? in.m_name.c_str() : ""); }
