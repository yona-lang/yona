#ifndef YONA_TEST_SUPPORT_REPOPATHS_H
#define YONA_TEST_SUPPORT_REPOPATHS_H

#include "YonaTestConfig.h"
...
inline std::filesystem::path repo_root() {
  return std::filesystem::path(YONA_SOURCE_DIR);
}
...
inline std::filesystem::path codegen_fixtures_dir() {
  return repo_root() / "test" / "Fixtures" / "Codegen";
}
...
#endif /* YONA_TEST_SUPPORT_REPOPATHS_H */
