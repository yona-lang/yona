...
TEST_CASE("JSON rejects trailing non-whitespace") {
  std::string error = "stale";
  auto valid = Json::parse("true \r\n\t", &error);
  CHECK(valid.as_bool());
  CHECK(error.empty());

  auto invalid = Json::parse("true trailing", &error);
...
instance Eq Int
  ...
  const SymbolInfo *instance_method = nullptr;
  for (const auto &symbol : symbols) {
    ...
    if (symbol.name == "eq" && symbol.kind == "method" &&
      ...
      method = &symbol;
    if (symbol.name == "eq" && symbol.kind == "method" &&
        symbol.container == "instance Eq Int")
...
instance Eq Int
  ...
  REQUIRE(signature);
  CHECK(
      signature->get("signatures").as_array().at(0).get("label").as_string() ==
      "eq : a -> a -> Bool");
  ...
  Json instance_edits;
  auto instance_renamed = a.rename(Position{8, 5}, "sameInt", instance_edits);
  REQUIRE(instance_renamed);
  CHECK(*instance_renamed == "eq");
  CHECK(instance_edits.as_array().at(0).get("edits").as_array().size() == 1);
}
...
run _ = measure "x"
  ...
  CHECK(
      actions[0].get("command").get("arguments").as_array().at(0).as_string() ==
      "E0106");
}
...
TEST_CASE("LSP imported interface contract preserves UTF-16 navigation") {
  ...
  const std::string source = "import identity from Prelude in let marker = "
                             "\"\xF0\x9F\x98\x80\" in identity 2";
  ...
  for (const auto &item : a.completions(Position{0, 0}))
    has_compare_completion =
        has_compare_completion || item.get("label").as_string() == "identity";
  CHECK(has_compare_completion);
  CHECK(
      a.hover(offset_to_position(source, source.find("identity"))).has_value());
  const auto use = offset_to_position(source, source.rfind("identity"));
  CHECK(use.line == 0);
  CHECK(use.character >
        32); // The preceding emoji occupies two UTF-16 code units.
  ...
  REQUIRE(signature);
  CHECK(signature->get("signatures")
            .as_array()
            .at(0)
            .get("label")
            .as_string()
...
TEST_CASE("Server initialize and hover") {
  ...
  CHECK(hv.get("contents").get("value").as_string().find("answer") !=
        std::string::npos);
}
...
static std::filesystem::path write_mod(const std::filesystem::path &dir,
...
TEST_CASE("Analysis local binding shadows imported name") {
  ...
  const std::string src =
      "import answer from Orig\\Mod in let answer = 1 in answer";
  a.analyze("file:///tmp/use.yona", src);
  auto pos = offset_to_position(src, src.rfind("answer"));
  auto defs = a.definition(pos);
  REQUIRE(!defs.empty());
...
TEST_CASE(
    "Analysis gives shadowed local bindings distinct lexical identities") {
  Analysis a;
  a.set_module_paths(default_module_paths(""));
  const std::string src = "let value = 1 in (let value = 2 in value, value)";
  a.analyze("file:///tmp/t.yona", src);

  const auto outer_def_offset = src.find("value");
  const auto inner_def_offset = src.find("value", outer_def_offset + 1);
  const auto inner_use_offset = src.find("value", inner_def_offset + 1);
  const auto outer_use_offset = src.rfind("value");

  const auto outer_def = offset_to_position(src, outer_def_offset);
  const auto inner_def = offset_to_position(src, inner_def_offset);
  const auto inner_use = offset_to_position(src, inner_use_offset);
  const auto outer_use = offset_to_position(src, outer_use_offset);

  auto inner_defs = a.definition(inner_use);
  REQUIRE(inner_defs.size() == 1);
  CHECK(inner_defs.front().range.start == inner_def);

  auto outer_defs = a.definition(outer_use);
  REQUIRE(outer_defs.size() == 1);
  CHECK(outer_defs.front().range.start == outer_def);

  CHECK(a.references(inner_use, true).size() == 2);
  CHECK(a.references(outer_use, true).size() == 2);
  CHECK(a.document_highlight(inner_use).size() == 2);
  CHECK(a.document_highlight(outer_use).size() == 2);

  Json edits;
  auto old_name = a.rename(inner_use, "innerValue", edits);
  REQUIRE(old_name);
  CHECK(*old_name == "value");
  REQUIRE(edits.is_array());
  REQUIRE(edits.as_array().size() == 1);
  const auto &inner_edits = edits.as_array().front().get("edits");
  REQUIRE(inner_edits.is_array());
  CHECK(inner_edits.as_array().size() == 2);

  Analysis initializer_analysis;
  initializer_analysis.set_module_paths(default_module_paths(""));
  const std::string initializer_src =
      "let value = 1 in let value = value in value";
  initializer_analysis.analyze("file:///tmp/initializer.yona", initializer_src);
  const auto initializer_outer = initializer_src.find("value");
  const auto initializer_inner =
      initializer_src.find("value", initializer_outer + 1);
  const auto initializer_rhs =
      initializer_src.find("value", initializer_inner + 1);
  const auto initializer_body = initializer_src.rfind("value");

  auto rhs_defs = initializer_analysis.definition(
      offset_to_position(initializer_src, initializer_rhs));
  REQUIRE(rhs_defs.size() == 1);
  CHECK(rhs_defs.front().range.start ==
        offset_to_position(initializer_src, initializer_outer));

  auto body_defs = initializer_analysis.definition(
      offset_to_position(initializer_src, initializer_body));
  REQUIRE(body_defs.size() == 1);
  CHECK(body_defs.front().range.start ==
        offset_to_position(initializer_src, initializer_inner));
}
...
TEST_CASE(
    "Analysis rename of an imported name does not edit the source module") {
  ...
  Json edits;
  auto old =
      a.rename(offset_to_position(src, src.rfind("answer")), "ans", edits);
  REQUIRE(old);
  CHECK(*old == "answer");
  REQUIRE(edits.is_array());
...
static bool has_completion_label(const std::vector<Json> &items,
...
TEST_CASE("Analysis publishes nested unreachable-pattern warnings") {
  Analysis a;
  a.set_module_paths(default_module_paths(""));
  a.analyze("file:///tmp/overlap.yona",
            "case Some 1 of Some _ -> 1; Some 1 -> 2; None -> 0 end");
  auto diags = a.diagnostics();
  const auto warning =
      std::find_if(diags.begin(), diags.end(), [](const LspDiagnostic &d) {
        return d.severity == 2 &&
               d.message.find("unreachable pattern") != std::string::npos;
      });
