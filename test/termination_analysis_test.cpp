...
yona::compiler::termination_analysis::Result analyze_expression(std::string_view source) {
  yona::parser::Parser parser;
  auto parsed = parser.parse_expression(source, "<termination-test>");
  REQUIRE(parsed.has_value());
  return yona::compiler::termination_analysis::analyze(*parsed.value());
}

yona::compiler::termination_analysis::Result analyze_module(std::string_view source) {
  yona::parser::Parser parser;
  auto parsed = parser.parse_module(source, "<termination-test>");
  REQUIRE(parsed.has_value());
  return yona::compiler::termination_analysis::analyze(*parsed.value());
}
...
loop n = [x for x = loop n]
  ...
  TEST_CASE("same-named functions in separate lexical scopes do not form a cycle") {
    const auto result = analyze_expression(R"(
(
...
loop (Succ rest) = if true -> loop rest
    ...
    REQUIRE(parsed.has_value());
    REQUIRE(parsed.value()->functions.size() == 1);
    REQUIRE(parsed.value()->functions.front()->bodies.size() == 1);
    REQUIRE(dynamic_cast<yona::ast::BodyWithGuards *>(parsed.value()->functions.front()->bodies.front()) != nullptr);
    const auto result = yona::compiler::termination_analysis::analyze(*parsed.value());
  ...
  TEST_CASE("lambda aliases resolve recursive calls by their lexical binding name") {
    const auto result = analyze_module(R"(
