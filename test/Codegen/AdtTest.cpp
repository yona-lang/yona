...
static string compile_and_run_adt(const string &mod_source,
                                  const string &expr_source) {
  // Step 1: Compile the module
  parser::Parser p1;
  auto mod_result = p1.parseModule(mod_source, "adt_test.yona");
  ...
  if (!yona::test::link::append_runtime_objects(link_objs))
    return "RT_COMPILE_ERROR";

  fs::path exe_path = yona::test::link::scratch_root() /
                      ("adt_test_exe" + yona::test::link::exe_suffix());
  if (!yona::test::link::link_objs_to_exe(link_objs, exe_path))
    return "LINK_ERROR";
...
unwrap x = x
    ...
    auto result = parser.parseModule(source, "opt.yona");
...
id x = x
    ...
    auto result = parser.parseModule(source, "color.yona");
    REQUIRE(result.has_value());
    auto *mod = result.value().get();
    CHECK(mod->adt_declarations.size() == 1);
...
id x = x
    ...
    auto result = parser.parseModule(source, "types.yona");
    REQUIRE(result.has_value());
    auto *mod = result.value().get();
...
red = Red
    ...
    CHECK(ir.find("YonaTestColorRed") != string::npos);
  }
...
check opt = case opt of
    ...
    auto result = parser.parseModule(source, "match.yona");
    REQUIRE(result.has_value());
...
id x = x
    ...
    auto result = parser.parseModule(source, "callback.yona");
    REQUIRE(result.has_value());
    auto *mod = result.value().get();
...
id x = x
    ...
    auto result = parser.parseModule(source, "stream.yona");
    REQUIRE(result.has_value());
    auto *mod = result.value().get();
    ...
    CHECK(fn_field.return_types[0].name == "Stream");
    REQUIRE(fn_field.return_types[0].type_arguments.size() == 1);
    CHECK(fn_field.return_types[0].type_arguments[0].name == "a");
  }
...
id x = x
    ...
    auto result = parser.parseModule(source, "reducer.yona");
    REQUIRE(result.has_value());
    auto *mod = result.value().get();
    auto &ft = mod->adt_declarations[0]->variants[0]->field_type_names[0];
    CHECK(ft.is_function_type == true);
  ...
  TEST_CASE("Closure stored in ADT field") {
    // A thunk (zero-arg closure capturing a value) stored in a recursive ADT
    // field and extracted via pattern matching, then called
    string mod_source = R"(
module Test\Thunk
...
unbox b = case b of
    ...
    parser::Parser p1;
    auto mod_result = p1.parseModule(mod_source, "thunk.yona");
    ...
    REQUIRE(mod_codegen.emit_interface_file(
        (yona_lib / "Test" / "Thunk.yonai").string()));

    // Step 2: Compile expression that imports from the module
    parser::Parser p2;
    string expr_source =
        "import Test\\Thunk in let n = 99 in let t = \\-> n in unbox (MkBox t)";
    istringstream stream(expr_source);
    auto expr_result = p2.parseExpression(stream.str(), "<stream>");
    REQUIRE(expr_result);
    ...
    fs::path exe_path = yona::test::link::scratch_root() /
                        ("thunk_test_exe" + yona::test::link::exe_suffix());
...
takeStream n s = if n <= 0 then [] else case s of End -> []; Next h t -> h :: (takeStream (n - 1) (t ())) end
    ...
    parser::Parser p1;
    auto mod_result = p1.parseModule(mod_source, "stream2.yona");
    REQUIRE(mod_result.has_value());
    ...
    fs::create_directories(yona_lib / "Test");
    REQUIRE(mod_codegen.emit_interface_file(
        (yona_lib / "Test" / "Stream2.yonai").string()));
