...
import yatta.ast.expression.LexicalScopeNode;

import java.util.*;

/**
 * YattaLanguage Language lexical scope. There can be a block scope, or function scope.
 */
public final class LexicalScope {
  ...
  /**
   * Create a new block Yatta lexical scope.
   *
  ...
  /**
   * Create a new functional Yatta lexical scope.
   *
    ...
    NodeUtil.forEachChild(block, new NodeVisitor() {
      ...
      public boolean visit(Node node) {
        ...
        if (node instanceof WriteLocalVariableNode) {
          ...
          // A different Yatta node - we're done.
          return false;
        } else {
