...
public final class FileBuiltinModule implements BuiltinModule {
  ...
  abstract static class FileOpenNode extends BuiltinNode {
    ...
    public Object open(Seq uriSeq, Seq modeSeq, @CachedContext(YattaLanguage.class) Context context) {
      try {
        ...
        AsynchronousFileChannel asynchronousFileChannel = AsynchronousFileChannel.open(Paths.get(uri), openOptions, context.ioExecutor);
      ...
      try {
        asynchronousFileChannel.read(buffer, position, buffer, new CompletionHandler<>() {
          ...
          public void completed(Integer result, ByteBuffer attachment) {
            ...
            if (!fulfilled) {
              readline(new Tuple(new NativeObject(asynchronousFileChannel), output, position + length), context, interopLibrary).map(res -> {
