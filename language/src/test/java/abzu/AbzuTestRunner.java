...
public class AbzuTestRunner extends ParentRunner<AbzuTestRunner.TestCase> {

  private static final String SOURCE_SUFFIX = ".abzu";
  ...
  protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
    AbzuTestSuite suite = c.getAnnotation(AbzuTestSuite.class);
    if (suite == null) {
      throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", AbzuTestSuite.class.getSimpleName(), c.getName(), AbzuTestRunner.class.getSimpleName()));
    }

    String[] paths = suite.value();

    Class<?> testCaseDirectory = c;
    if (suite.testCaseDirectory() != AbzuTestSuite.class) {
  ...
  public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
    ...
    AbzuTestRunner suite = new AbzuTestRunner(testClass);
    if (args.length > 0) {
