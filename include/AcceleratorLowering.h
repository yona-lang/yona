...
/// Classify `yona_Std_IntArray__map` / `__filter` / `__foldl` and the
/// FloatArray twins. `args` are juxtaposition arguments in application order
/// (function first).
...
std::vector<AccelMatch> collect_transparent_matches_module(ast::ModuleDecl* mod);
