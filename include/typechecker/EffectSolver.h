#ifndef YONA_TYPECHECKER_EFFECT_SOLVER_H
#define YONA_TYPECHECKER_EFFECT_SOLVER_H
...
struct EffectRef {
  ...
  [[nodiscard]] bool valid() const noexcept { return owner != 0 && index != UINT32_MAX; }
...
struct EffectNormalForm {
  ...
  [[nodiscard]] bool empty() const noexcept { return known_labels.empty() && tails.empty(); }
...
public:
  ...
  EffectRef mask(EffectRef source, std::initializer_list<std::string> excluded_labels);
...
#endif // YONA_TYPECHECKER_EFFECT_SOLVER_H
