#ifndef YONA_SEMANTICS_REFINEMENTCHECKER_H
#define YONA_SEMANTICS_REFINEMENTCHECKER_H
...
struct Interval {
  ...
  static Interval above(int64_t lo) {
    return {lo, (std::numeric_limits<int64_t>::max)()};
  }
  static Interval below(int64_t hi) {
    return {(std::numeric_limits<int64_t>::min)(), hi};
  }
...
struct FactEnv {
  ...
  bool satisfies(const types::RefinePredicate &pred,
...
///
/// Both dependencies must outlive the checker. Registered predicates are
/// retained with shared ownership. Registration, lookup, and check() are not
/// synchronized with one another; callers must serialize them and access to
/// the borrowed diagnostics/checker. Violations are emitted as diagnostics.
class RefinementChecker {
public:
  /// \p tc optional; when set, warns on discarded algebraic values (see \c
  /// warn_if_discarded_adt).
  ...
  /// Look up a refined type by name (returns nullptr if not refined). The
  /// pointer is borrowed until checker destruction. Registering the same name
  /// may replace the object value observed through that pointer.
  const RefinedTypeInfo *lookup(const std::string &name) const;

  /// Check a borrowed AST tree, which must remain alive and immutable for the
...
private:
  ...
  /// If \p tc was provided and \p expr has a named-sum type (App other than
  /// Seq/Set/Dict), emit \c WarningFlag::UnmatchedAdt (enable with \c -Wall or
  /// \c enable_warning).
  void warn_if_discarded_adt(ast::AstNode *expr);

  /// Extract the variable name from an argument expression (if it's an
  /// identifier).
  static std::string arg_var_name(ast::AstNode *node);

  /// Narrow a FactEnv from a comparison condition node.
  static void narrow_from_condition(ast::AstNode *cond, FactEnv &then_facts,
                                    FactEnv &else_facts);

  /// Establish facts from a let-bound expression.
  void establish_let_facts(const std::string &name, ast::AstNode *expr,
                           FactEnv &facts);
...
#endif /* YONA_SEMANTICS_REFINEMENTCHECKER_H */
