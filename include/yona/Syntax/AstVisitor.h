#ifndef YONA_SYNTAX_ASTVISITOR_H
#define YONA_SYNTAX_ASTVISITOR_H
...
/// Templated visitor over borrowed AST nodes.
///
/// The base retains no node. Every pointer passed to visit() remains owned by
/// its AST and is valid only while that tree is alive. Runtime dispatch throws
/// std::runtime_error for null, incomplete, or unknown concrete node types.
/// Derived visitors define any additional failure and synchronization rules;
/// a visitor instance is not implicitly safe for concurrent calls merely
/// because visit() is const.
template <typename ResultType> class AstVisitor {
protected:
  // Helper method to dispatch based on runtime type when we have a base pointer
  // This avoids infinite recursion when accept() is called on base class
  // pointers
...
#endif /* YONA_SYNTAX_ASTVISITOR_H */
