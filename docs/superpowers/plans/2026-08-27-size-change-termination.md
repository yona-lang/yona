...
**Tech Stack:** C++23, AST (`include/ast.h`), doctest, CMake/Ninja, CLI integration, LSP diagnostics.
...
- `include/TerminationAnalysis.h`: compiler-independent public result and relation API.
- `src/TerminationAnalysis.cpp`: AST walk, recursive call graph, SCC decomposition, relation composition, and proof result.
- `cli/main.cpp`: replace local termination walk with analyser invocation and E0203 rendering.
- `test/yona_script_test.cpp`: end-to-end strict-mode acceptance/rejection regressions.
- `test/lsp_test.cpp`: strict-mode diagnostic publication regression if LSP exposes strict configuration; otherwise assert compiler diagnostics remain the shared source and document the CLI-only gate.
...
**Files:**
- Create: `include/TerminationAnalysis.h`
- Create: `src/TerminationAnalysis.cpp`
...
- [ ] **Step 1: Replace `require_structural_termination` with an adapter.**
...
- [ ] **Step 2: Add exact diagnostic tests.**
...
- [ ] **Step 3: Run strict-mode regression coverage.**
...
- Modify: `test/lsp_test.cpp` only if a strict-mode configuration path exists

- [ ] **Step 1: Update the TODO and status wording.**
...
- [ ] **Step 2: Update E0203 and public CLI documentation.**
...
- [ ] **Step 5: Make the single final commit.**

```bash
git add CHANGELOG.md docs site/src/content/docs include/TerminationAnalysis.h \
  src/TerminationAnalysis.cpp cli/main.cpp test/ CMakeLists.txt
