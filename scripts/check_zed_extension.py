...
def main() -> None:
    ...
    config = tomllib.loads(
        (EXTENSION / "languages" / "yona" / "config.toml").read_text()
    )
    ...
    require(
        {"yona", "yonai"}.issubset(config["path_suffixes"]),
        "missing Yona suffix",
    )
    require(
        re.fullmatch(r"[0-9a-f]{40}", grammar["rev"]) is not None,
        "grammar revision must be a 40-character commit SHA",
    )
    require(
        grammar["rev"] != "0" * 40, "grammar revision is still a placeholder"
    )
    ...
    for query in (
        ...
        require(
            (EXTENSION / "languages" / "yona" / query).is_file(),
            f"missing {query}",
        )
    ...
    require(
        configured < path, "configured binary must take precedence over PATH"
    )
    require('"--stdio"' in source, "yls must be launched with --stdio")
    require(
        (GRAMMAR / "src" / "parser.c").is_file(), "generated parser is missing"
    )
