...
def read_message(proc: subprocess.Popen[bytes]) -> dict:
    ...
    if length is None:
        raise SystemExit(
            f"yls-yona response missing Content-Length: {header_text!r}"
        )
...
def read_response(proc: subprocess.Popen[bytes], expected_id: int) -> dict:
    ...
    for _ in range(8):
        ...
        if "method" in msg:
            continue
        raise SystemExit(
            f"unexpected message while waiting for id {expected_id}: {msg}"
        )
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: smoke_yls_yona.py <path-to-yls-yona>", file=sys.stderr)
        return 2
    yls = sys.argv[1]
    ...
    if "language server" not in (help_run.stdout + help_run.stderr).lower():
        print(
            "yls-yona --help did not mention the language server",
            file=sys.stderr,
        )
        return 1
    ...
    if caps.get("hoverProvider") is True:
        print(
            "yls-yona must not claim hoverProvider (C++ yls remains the editor default)",
            file=sys.stderr,
        )
        proc.kill()
        return 1
    ...
    if info.get("name") != "yls-yona":
        print(
            f"initialize serverInfo.name should be yls-yona: {init}",
            file=sys.stderr,
        )
        proc.kill()
        return 1
    ...
    if hover.get("result") is not None:
        print(
            f"unknown/unimplemented method should return null: {hover}",
            file=sys.stderr,
        )
        proc.kill()
        return 1
