...
def parse_cases(path: Path) -> list[Case]:
    ...
    if not suites and _local(root.tag) == "testsuites":
        suites = [el for el in root.iter() if el is not root and _local(el.tag) == "testsuite"]
    for suite in suites:
        ...
        for el in suite:
            ...
            cases.append(Case(name=name, suite=group, status=status, message=message))
    return cases
...
def load_reports(paths: list[Path], title: str) -> list[Report]:
    ...
    detailed_labels = {label for label, cases in leftovers if cases and not is_ctest_wrapper(cases)}
    ...
    for suite, suite_cases in group_suites(cases).items():
        ...
        if show_detail:
            ...
            for c in suite_cases:
                lines.append(f"| {md_cell(c.name)} | {c.status} | {md_cell(c.message)} |\n")
        else:
