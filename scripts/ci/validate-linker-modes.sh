...
YONAC_CC="${CC}" "${YONAC}" --sysroot "${BUILD_DIR}" --linker-mode system -e "1 + 2" -o "${BUILD_DIR}/ci_link_system"
[[ "$("${BUILD_DIR}/ci_link_system")" == "3" ]]

if [[ "${INPROC_ACTIVE}" == "TRUE" ]]; then
  YONAC_REQUIRE_INPROCESS_LLD=1 YONAC_CC="${CC}" "${YONAC}" --sysroot "${BUILD_DIR}" --linker-mode inprocess -e "1 + 2" -o "${BUILD_DIR}/ci_link_inprocess"
else
  YONAC_CC="${CC}" "${YONAC}" --sysroot "${BUILD_DIR}" --linker-mode inprocess -e "1 + 2" -o "${BUILD_DIR}/ci_link_inprocess"
fi
