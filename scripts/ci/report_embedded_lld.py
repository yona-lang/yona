...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "usage: report_embedded_lld.py <CMakeCache.txt> <title>",
            file=sys.stderr,
        )
        return 2
    ...
    if not enabled:
        print(
            "Missing YONA_INPROCESS_LLD_AVAILABLE in CMakeCache.txt",
            file=sys.stderr,
        )
        return 1
    if not reason:
        print(
            "Missing YONA_INPROCESS_LLD_REASON in CMakeCache.txt",
            file=sys.stderr,
        )
        return 1
