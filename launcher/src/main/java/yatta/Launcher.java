...
import yatta.parser.YattaLexer;
import yatta.parser.YattaParser;
...
public final class Launcher extends AbstractLanguageLauncher {
  ...
  private static final String MIME_TYPE = "application/x-yatta";
  private final static Source SHUTDOWN_SOURCE = Source.newBuilder("yatta", "shutdown", "shutdown").internal(true).buildLiteral();
  ...
  private final DefaultLexerAndParserFactory<YattaLexer, YattaParser> lexerAndParserFactory = new DefaultLexerAndParserFactory<>(YattaLexer::new, YattaParser::new, (state) -> !Launcher.DISALLOWED_COMPLETION_STATES.contains(state));

  public Launcher() {
    this.argumentParser = ArgumentParsers.newFor(PROGRAM_NAME).build().defaultHelp(true).description("Yatta Language Interpreter");
    ...
    argumentParser.addArgument("-V", "--version").help("Print Yatta version and exit").action(storeTrue());
    argumentParser.addArgument("--show-version").help("Print Yatta version and start REPL").action(storeTrue()).dest("showVersion");
    ...
    argumentParser.epilog("Other environment variables:\n" +
        "  YATTA_STDLIB_HOME                     location of the standard library (provided by Yatta distribution by default)\n" +
        "  YATTA_PATH                            ':'-separated list of directories prefixed to the default module search path");
  }
  ...
  protected void launch(Context.Builder contextBuilder) {
    if (verboseFlag) {
      contextBuilder.option("log.yatta.level", "FINE");
    }
...
//    contextBuilder.option("yatta.TerminalWidth", Integer.toString(consoleHandler.getTerminalWidth()));
//    contextBuilder.option("yatta.TerminalHeight", Integer.toString(consoleHandler.getTerminalHeight()));
  ...
  public ConsoleHandler createConsoleHandler(InputStream inStream, OutputStream outStream, Context context) {
    ...
    return new JLineConsoleHandler(Paths.get(languageHome, "yatta.nanorc"), inStream, outStream, (buffer) -> {
  ...
  public int readEvalPrint(Context context, ConsoleHandler consoleHandler) {
    System.out.println("Welcome to Yatta REPL. Type in an expression and press enter to execute or Ctrl-D to exit.");
    System.out.println();
    int lastStatus = 0;
    try {
      NORMAL:
  ...
  private static List<String> getDefaultEnvironmentArgs() {
    ...
    String envArgsOpt = System.getenv("YATTA_ARGS");
  ...
  private static void printStackTrace(PolyglotException e) {
    ...
    Object[] yattaExceptionTuple = new Object[(int) e.getGuestObject().getArraySize()];
    for (int i = 0; i < e.getGuestObject().getArraySize(); i++) {
      yattaExceptionTuple[i] = e.getGuestObject().getArrayElement(i).as(Object.class);
    }
