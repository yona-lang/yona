...
if (!existsSync(API_DIR)) {
	console.error(`sync-stdlib: missing ${API_DIR} — run python3 scripts/gendocs.py from the repo root first`);
	process.exit(1);
}
...
function transform(raw, { isIndex }) {
	...
	const description = (para ?? '').replace(/\s+/g, ' ').replace(/[*_`|]/g, '').slice(0, 160);
...
for (const file of files) {
	...
	const outName = isIndex ? 'index.md' : `${basename(file, '.md').toLowerCase()}.md`;
