...
function urlFor(file) {
	let rel = relative(DOCS, file).replace(/\\/g, '/').replace(/\.(md|mdx)$/, '');
...
writeFileSync(join(OUT_DIR, 'llms-small.txt'), core.map(dumpPage).join('\n---\n\n'));
writeFileSync(join(OUT_DIR, 'llms-full.txt'), pages.map(dumpPage).join('\n---\n\n'));
